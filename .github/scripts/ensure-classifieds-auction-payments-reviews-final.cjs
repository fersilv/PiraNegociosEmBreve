...
patch('pages/ClassifiedsAuctionsLivePageV2.tsx', (input) => {
  ...
  if (!source.includes('type AuctionCreateForm =')) {
    ...
    const addition = `\ntype AuctionCreateForm = {\n  listingId: string; startPrice: string; minIncrement: string; startsAt: string; endsAt: string;\n  onlinePaymentEnabled: boolean; auctionFeePayer: 'SELLER' | 'BUYER'; paymentMethods: Array<'PIX' | 'CARD'>;\n  cardMaxInstallments: number; fulfillmentModes: Array<'ARRANGE' | 'PICKUP' | 'DELIVERY'>; deliveryFee: string; deliveryNote: string;\n};\nconst emptyAuctionForm: AuctionCreateForm = { listingId: '', startPrice: '', minIncrement: '5', startsAt: '', endsAt: '', onlinePaymentEnabled: false, auctionFeePayer: 'SELLER', paymentMethods: ['PIX','CARD'], cardMaxInstallments: 12, fulfillmentModes: ['ARRANGE','PICKUP'], deliveryFee: '', deliveryNote: '' };\n`;
    if (source.includes(anchor)) source = source.replace(anchor, `${anchor}${addition}`);
  }

  const oldState = `  const [createForm, setCreateForm] = useState({ listingId: '', startPrice: '', minIncrement: '5', startsAt: '', endsAt: '' });`;
  if (source.includes(oldState)) {
    source = source.replace(oldState, `  const [createForm, setCreateForm] = useState<AuctionCreateForm>({ ...emptyAuctionForm });\n  const [auctionPaymentDefaults, setAuctionPaymentDefaults] = useState<any>(null);`);
  }
  ...
  if (!source.includes("api.get('/classifieds/auctions/payment-defaults')")) {
    ...
    const effect = `  useEffect(() => {\n    if (!createOpen || !canCreate) return;\n    let active = true;\n    api.get('/classifieds/auctions/payment-defaults').then((response) => {\n      if (!active) return;\n      const defaults = response.data || {};\n      setAuctionPaymentDefaults(defaults);\n      setCreateForm((current) => ({ ...current, auctionFeePayer: defaults.auctionFeePayer === 'BUYER' ? 'BUYER' : 'SELLER', paymentMethods: Array.isArray(defaults.paymentMethods) && defaults.paymentMethods.length ? defaults.paymentMethods : current.paymentMethods, cardMaxInstallments: Number(defaults.cardMaxInstallments || current.cardMaxInstallments), fulfillmentModes: Array.isArray(defaults.fulfillmentModes) && defaults.fulfillmentModes.length ? defaults.fulfillmentModes : current.fulfillmentModes }));\n    }).catch(() => setAuctionPaymentDefaults(null));\n    return () => { active = false; };\n  }, [createOpen, canCreate]);\n\n`;
    if (source.includes(anchor)) source = source.replace(anchor, `${effect}${anchor}`);
  }

  const oldCreate = `      const startsAt = createForm.startsAt ? new Date(\`${'${createForm.startsAt}'}:00-03:00\`).toISOString() : '';\n      const endsAt = createForm.endsAt ? new Date(\`${'${createForm.endsAt}'}:00-03:00\`).toISOString() : '';\n      const response = await api.post('/classifieds/me/auctions', {\n        listingId: createForm.listingId,\n        startPrice: parseMoneyInput(createForm.startPrice),\n        minIncrement: parseMoneyInput(createForm.minIncrement),\n        startsAt: startsAt || undefined,\n        endsAt,\n      });\n      invalidatePublicAuctions();\n      setCreateOpen(false);\n      setCreateForm({ listingId: '', startPrice: '', minIncrement: '5', startsAt: '', endsAt: '' });\n      await loadList(true);\n      if (response.data?.id) navigate(\`/classificados/leiloes/${'${response.data.id}'}\`);`;
  const newCreate = `      const startsAt = createForm.startsAt ? new Date(\`${'${createForm.startsAt}'}:00-03:00\`).toISOString() : '';\n      const endsAt = createForm.endsAt ? new Date(\`${'${createForm.endsAt}'}:00-03:00\`).toISOString() : '';\n      const response = await api.post('/classifieds/me/auctions', {\n        listingId: createForm.listingId,\n        startPrice: parseMoneyInput(createForm.startPrice),\n        minIncrement: parseMoneyInput(createForm.minIncrement),\n        startsAt: startsAt || undefined,\n        endsAt,\n      });\n      const createdAuctionId = String(response.data?.id || '');\n      if (createdAuctionId && createForm.onlinePaymentEnabled) {\n        const parsedDelivery = parseMoneyInput(createForm.deliveryFee);\n        await api.patch(\`/classifieds/auctions/${'${createdAuctionId}'}/seller-settlement\`, { onlinePaymentEnabled: true, auctionFeePayer: createForm.auctionFeePayer, paymentMethods: createForm.paymentMethods, cardMaxInstallments: createForm.cardMaxInstallments, fulfillmentModes: createForm.fulfillmentModes, deliveryFeeCents: Number.isFinite(parsedDelivery) ? Math.max(0, Math.round(parsedDelivery * 100)) : 0, deliveryNote: createForm.deliveryNote || null });\n      }\n      invalidatePublicAuctions();\n      setCreateOpen(false);\n      setCreateForm({ ...emptyAuctionForm });\n      await loadList(true);\n      if (createdAuctionId) navigate(\`/classificados/leiloes/${'${createdAuctionId}'}\`);`;
  if (!source.includes('const createdAuctionId = String(response.data?.id')) {
    if (source.includes(oldCreate)) source = source.replace(oldCreate, newCreate);
  }

  source = source.replaceAll(`form: { listingId: string; startPrice: string; minIncrement: string; startsAt: string; endsAt: string }`, `form: AuctionCreateForm`);
  source = source.replaceAll(`React.SetStateAction<{ listingId: string; startPrice: string; minIncrement: string; startsAt: string; endsAt: string }>`, `React.SetStateAction<AuctionCreateForm>`);

  if (!source.includes('feeRule={auctionPaymentDefaults?.feeRule}')) {
    const anchor = `            creating={creating}\n            onCreate={() => void createAuction()}`;
    if (source.includes(anchor)) source = source.replace(anchor, `            creating={creating}\n            feeRule={auctionPaymentDefaults?.feeRule || null}\n            paymentConnected={auctionPaymentDefaults?.mercadoPagoConnected === true}\n            companyAddress={auctionPaymentDefaults?.companyAddress || ''}\n            onCreate={() => void createAuction()}`);
  }

  const oldSignature = `function CreateAuctionPanel({ listings, form, setForm, creating, onCreate, onClose }: { listings: ClassifiedListing[]; form: AuctionCreateForm; setForm: React.Dispatch<React.SetStateAction<AuctionCreateForm>>; creating: boolean; onCreate: () => void; onClose: () => void })`;
  if (source.includes(oldSignature)) {
    source = source.replace(oldSignature, `function CreateAuctionPanel({ listings, form, setForm, creating, feeRule, paymentConnected, companyAddress, onCreate, onClose }: { listings: ClassifiedListing[]; form: AuctionCreateForm; setForm: React.Dispatch<React.SetStateAction<AuctionCreateForm>>; creating: boolean; feeRule?: any; paymentConnected?: boolean; companyAddress?: string; onCreate: () => void; onClose: () => void })`);
  }
  ...
  const info = `<div className="mt-4 grid grid-cols-2 gap-2"><InfoBox label="Incremento mínimo" value={money(auction.minIncrement)} /><InfoBox label="Próximo mínimo" value={money(auction.nextMinimum)} /></div>`;
  if (source.includes(info) && !source.includes('<AuctionFeeNotice auction={auction} />')) source = source.replace(info, `${info}<AuctionFeeNotice auction={auction} />`);

  const bidGate = `<div id="auction-bid-box" className="border-t border-white/[.07] p-5 sm:p-6">{ended ?`;
  if (source.includes(bidGate)) source = source.replace(bidGate, `<div id="auction-bid-box" className="border-t border-white/[.07] p-5 sm:p-6">{scheduled ? <div><div className="rounded-2xl border border-blue-300/20 bg-blue-400/[.07] p-4"><p className="text-xs font-black text-blue-100">Este leilão está agendado</p><p className="mt-2 text-[10px] leading-5 text-white/45">Os lances abrem em {compactCountdown(startsIn)}. Nada de leilão flash: duração mínima de 60 minutos.</p></div><AuctionReminderButton auction={auction} loggedIn={loggedIn} /></div> : ended ?`);

  const main = `<main className="min-h-[calc(100vh-70px)] bg-[#090605]">`;
  if (source.includes(main) && !source.includes('<AuctionPresencePing auction={auction}')) source = source.replace(main, `${main}<AuctionPresencePing auction={auction} loggedIn={loggedIn} />`);

  if (!source.includes('function AuctionFeeNotice(')) {
    const anchor = `function MiniStat({ label, value }`;
    const helpers = `function AuctionFeeNotice({ auction }: { auction: PublicClassifiedAuction }) {\n  if (!auction.onlinePaymentEnabled) return null;\n  const settlement = auction.settlement; const buyerPays = (settlement?.auctionFeePayer || auction.auctionFeePayer) === 'BUYER'; const percentage = settlement?.auctionFeePercentage ?? auction.auctionFeePercentage;\n  return <div className={\`mt-3 rounded-xl border px-3 py-2 text-[9px] leading-4 \${buyerPays ? 'border-amber-300/20 bg-amber-300/[.07] text-amber-100' : 'border-emerald-300/15 bg-emerald-300/[.05] text-emerald-100'}\`}>{buyerPays ? <><strong>Arremate + taxa de leilão{percentage == null ? '' : ` de \${Number(percentage).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}%`}.</strong> Eventual entrega é cobrada separadamente.</> : <><strong>Sem taxa de leilão adicional para o arrematante.</strong> A empresa absorve a comissão.</>}{settlement?.pickupAddress && <span className="mt-1 block text-white/50">Retirada: {settlement.pickupAddress}</span>}</div>;\n}\nfunction AuctionReminderButton({ auction, loggedIn }: { auction: PublicClassifiedAuction; loggedIn: boolean }) {\n  const [enabled, setEnabled] = useState(false); const [working, setWorking] = useState(false);\n  useEffect(() => { if (!loggedIn || !(auction.scheduled || auction.status === 'SCHEDULED')) return; let active = true; api.get(\`/classifieds/auctions/\${auction.id}/reminder\`).then((response) => active && setEnabled(response.data?.enabled === true)).catch(() => undefined); return () => { active = false; }; }, [auction.id, auction.status, auction.scheduled, loggedIn]);\n  if (!loggedIn) return <Link to={\`/login?returnTo=\${encodeURIComponent(`/classificados/leiloes/\${auction.id}`)}\`} className="mt-3 inline-flex w-full justify-center rounded-xl bg-white px-4 py-3 text-xs font-black text-[#21130f]">Entrar para ativar lembrete</Link>;\n  return <button type="button" disabled={working} onClick={async () => { setWorking(true); try { const response = await api.post(\`/classifieds/auctions/\${auction.id}/reminder\`, { enabled: !enabled }); setEnabled(response.data?.enabled === true); } finally { setWorking(false); } }} className={\`mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-black \${enabled ? 'bg-blue-500 text-white' : 'bg-white text-[#21130f]'}\`}><Clock3 className="h-4 w-4" /> {working ? 'Salvando...' : enabled ? 'Lembrete ativado' : 'Me lembre'}</button>;\n}\nfunction AuctionPresencePing({ auction, loggedIn }: { auction: PublicClassifiedAuction; loggedIn: boolean }) {\n  useEffect(() => { if (!loggedIn || auction.status !== 'OPEN') return; void api.post(\`/classifieds/auctions/\${auction.id}/presence\`).catch(() => undefined); const timer = window.setInterval(() => { if (document.visibilityState === 'visible') void api.post(\`/classifieds/auctions/\${auction.id}/presence\`).catch(() => undefined); }, 60_000); return () => window.clearInterval(timer); }, [auction.id, auction.status, loggedIn]); return null;\n}\n\n`;
