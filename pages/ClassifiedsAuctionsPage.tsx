...
import { Clock3, Crown, Gavel, Loader2, MessageCircle, Plus, ShieldCheck, Trophy, X } from 'lucide-react';
import { Link, useNavigate, useParams } from 'react-router-dom';
import { useClassifiedsWorkspace } from '../contexts/ClassifiedsWorkspaceContext';
import { api } from '../lib/api';
import type { ClassifiedLimits, ClassifiedListing } from '../types/classifieds';
...
export default function ClassifiedsAuctionsPage() {
  ...
  const [limits, setLimits] = useState<ClassifiedLimits | null>(null);
  ...
  const load = async () => {
    ...
    try {
      const requests: Promise<any>[] = [api.get('/classifieds/auctions'), api.get('/classifieds/me/limits')];
      ...
      setAuctions(list);
      setLimits(responses[1].data as ClassifiedLimits);
      if (business) {
        const mine = Array.isArray(responses[2]?.data) ? responses[2].data as ClassifiedListing[] : [];
  ...
  const canCreateAuction = Boolean(business && limits?.auctionCreation && String(limits?.plan).toUpperCase() === 'ELITE');

  const createAuction = async () => {
    if (working || !canCreateAuction) return;
    setWorking(true); setError('');
    try {
      const endsAt = localToIso(form.endsAt);
      const response = await api.post('/classifieds/me/auctions', { ...form, endsAt });
      setCreateOpen(false);
  ...
  const cancel = async () => {
    if (!detail || working || !canCreateAuction) return;
    setWorking(true); setError('');
    try {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-[10px] font-black uppercase tracking-[.18em] text-amber-700">Disputa por tempo</p><h1 className="mt-1 font-serif text-3xl font-black">Leilões</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-stone-500">Qualquer perfil elegível pode participar. Criar leilões é um recurso exclusivo para empresas no plano Elite.</p></div>
        {business && (canCreateAuction ? <button onClick={() => setCreateOpen(true)} className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-[#0d4542] px-5 text-sm font-black text-white"><Plus className="h-4 w-4" /> Criar leilão</button> : <Link to="/company" className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-amber-100 px-5 text-sm font-black text-amber-900 ring-1 ring-amber-200"><Crown className="h-4 w-4" /> Recurso Elite</Link>)}
      </header>

      <div className="grid gap-3 lg:grid-cols-2"><div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-900"><strong>Pagamento protegido ainda não está ativo.</strong> Vencer um leilão registra o compromisso e abre a negociação, mas o PiraNegócios não retém nem libera dinheiro nesta versão.</div><div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-xs leading-5 text-blue-900"><div className="flex gap-2"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>Para dar lance:</strong> a conta precisa ter e-mail, WhatsApp verificado e foto de perfil com o rosto. A API confere esses requisitos antes de registrar o lance.</span></div></div></div>
      {business && !canCreateAuction && <div className="rounded-2xl bg-[#0d3532] px-4 py-3 text-xs leading-5 text-white"><strong>Seu plano atual é {String(limits?.plan || 'FREE').toUpperCase()}.</strong> Você pode participar de leilões normalmente, mas somente empresas Elite podem criar leilões de seus produtos.</div>}
      {error && <div className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</div>}

      {detail ? (
        <AuctionDetail auction={detail} canManage={canCreateAuction} bidAmount={bidAmount} setBidAmount={setBidAmount} working={working} onBid={bid} onCancel={cancel} onConversation={openConversation} onBack={() => navigate('/classificados/leiloes')} />
      ) : (
        <>
          ...
          {business && <section><div className="mb-3 flex items-center justify-between"><h2 className="font-serif text-2xl font-black">Leilões da empresa</h2><span className="text-xs font-bold text-stone-400">{mine.length}</span></div>{mine.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{mine.map((auction) => <AuctionCard key={auction.id} auction={auction} onOpen={() => navigate(`/classificados/leiloes/${auction.id}`)} />)}</div> : <Empty text={canCreateAuction ? 'A empresa ainda não criou nenhum leilão.' : 'Ao assinar o Elite, os leilões criados pela empresa serão administrados aqui.'} />}</section>}
        </>
      )}

      {createOpen && canCreateAuction && <CreateAuctionModal products={products} form={form} setForm={setForm} working={working} onCreate={createAuction} onClose={() => setCreateOpen(false)} />}
    </div>
  );
}
...
function CreateAuctionModal({ products, form, setForm, working, onCreate, onClose }: { products: ClassifiedListing[]; form: { listingId: string; startPrice: string; minIncrement: string; endsAt: string }; setForm: React.Dispatch<React.SetStateAction<{ listingId: string; startPrice: string; minIncrement: string; endsAt: string }>>; working: boolean; onCreate: () => void; onClose: () => void }) {
  return <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/45 p-4"><button className="absolute inset-0" onClick={onClose} aria-label="Fechar" /><div className="relative w-full max-w-xl rounded-[28px] bg-white p-5 shadow-2xl sm:p-7"><button onClick={onClose} className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-stone-100"><X className="h-4 w-4" /></button><div className="flex items-center gap-2"><span className="rounded-full bg-amber-100 px-2.5 py-1 text-[9px] font-black uppercase tracking-[.1em] text-amber-900">Elite</span><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#397c75]">Business</p></div><h2 className="mt-2 font-serif text-2xl font-black">Criar leilão</h2><p className="mt-2 text-sm leading-6 text-stone-500">Escolha um produto publicado. Produtos com leilão aberto deixam de aceitar propostas diretas até o encerramento. Depois do primeiro lance, o leilão não pode ser cancelado.</p><div className="mt-6 space-y-4"><Field label="Produto"><select value={form.listingId} onChange={(event) => setForm((current) => ({ ...current, listingId: event.target.value }))} className={inputClass}><option value="">Selecione um produto</option>{products.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="Lance inicial"><input type="number" min="0.01" step="0.01" value={form.startPrice} onChange={(event) => setForm((current) => ({ ...current, startPrice: event.target.value }))} className={inputClass} placeholder="100,00" /></Field><Field label="Incremento mínimo"><input type="number" min="0.01" step="0.01" value={form.minIncrement} onChange={(event) => setForm((current) => ({ ...current, minIncrement: event.target.value }))} className={inputClass} /></Field></div><Field label="Encerra em"><input type="datetime-local" value={form.endsAt} onChange={(event) => setForm((current) => ({ ...current, endsAt: event.target.value }))} className={inputClass} /></Field></div><button disabled={working || !form.listingId || !form.startPrice || !form.endsAt} onClick={onCreate} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#0d4542] px-5 py-3.5 text-sm font-black text-white disabled:opacity-40">{working ? <Loader2 className="h-4 w-4 animate-spin" /> : <Gavel className="h-4 w-4" />} Abrir leilão</button></div></div>;
}
...
function localToIso(value: string) { const date = new Date(value); return Number.isFinite(date.getTime()) ? date.toISOString() : value; }
