...
@Injectable()
export class CompanyVerificationEmailService {
  async send(data: { to: string; partnerName: string; companyName: string; requestedByName: string; inviteUrl: string; expiresAt: Date }) {
    ...
    try {
      ...
      if (!response.ok) return { status: 'FAILED' as const, error: String(payload?.message || `HTTP ${response.status}`).slice(0, 500) };
      return { status: 'SENT' as const, messageId: payload?.id || null };
    } catch (error) {
      return { status: 'FAILED' as const, error: String(error instanceof Error ? error.message : error).slice(0, 500) };
    }
  }
}
