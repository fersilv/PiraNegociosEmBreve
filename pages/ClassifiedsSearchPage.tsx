    ...
    <div className="min-h-screen bg-[#f6f4f1] text-[#2d211c]">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-5 flex snap-x gap-2 overflow-x-auto pb-1">
          <Link to="/classificados/busca" className={`shrink-0 rounded-full px-4 py-2 text-xs font-bold ring-1 ${!activeCategory ? 'bg-[#2d211c] text-white ring-[#2d211c]' : 'bg-white text-[#5a463c] ring-[#4b3328]/10'}`}>Todos</Link>
        ...
        <div className="grid gap-7 lg:grid-cols-[248px_minmax(0,1fr)]">
          ...
          <section className="min-w-0">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#b06448]">{category ? category.name : 'Marketplace regional'}</p><h1 className="mt-1 font-serif text-2xl font-bold sm:text-3xl">{searchParams.get('q') ? `Resultados para “${searchParams.get('q')}”` : category ? `Anúncios em ${category.name}` : 'Todos os classificados'}</h1><p className="mt-1 text-xs font-semibold text-[#8b756a]">{loading ? 'Buscando...' : `${result.total} ${result.total === 1 ? 'anúncio encontrado' : 'anúncios encontrados'}`}</p></div>
