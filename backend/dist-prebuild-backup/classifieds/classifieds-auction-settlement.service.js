...
exports.ClassifiedsAuctionSettlementService = void 0;
...
let ClassifiedsAuctionSettlementService = class ClassifiedsAuctionSettlementService {
    dataSource;
    identities;
    entitlements;
    marketplacePayments;
    providerConfig;
    terms;
    constructor(dataSource, identities, entitlements, marketplacePayments, providerConfig, terms) {
    ...
    async sellerConfig(uid, auctionId) {
        ...
        const connected = await this.dataSource.query(`SELECT provider,status FROM company_classified_payment_connections
       WHERE "companyId"=$1 AND provider='MERCADO_PAGO' AND status='CONNECTED' LIMIT 1`, [auction.companyId]).catch(() => []);
    ...
    async configureSeller(uid, auctionId, body) {
        ...
        if (auction.settlementOrderId && ['PENDING', 'IN_PROCESS', 'APPROVED'].includes(String(auction.settlementPaymentStatus))) {
            throw new common_1.BadRequestException('As condições do recebimento não podem ser alteradas depois que o pagamento foi iniciado.');
        }
        if (enabled) {
            await this.terms.assertAccepted(uid, 'ONLINE_PAYMENT_SELLER');
            const companyRows = await this.dataSource.query(`SELECT "isVerified","verificationStatus" FROM companies WHERE id=$1 LIMIT 1`, [auction.companyId]);
            const company = companyRows[0];
            if (!company || !(company.isVerified || company.verificationStatus === 'VERIFIED')) {
                throw new common_1.ForbiddenException('Somente empresas verificadas podem receber arremates online.');
            }
            await this.marketplacePayments.sellerMercadoPagoCredentials(auction.companyId);
            const plan = await this.entitlements.companyPlan(auction.companyId);
            const feeRule = await this.resolveAuctionFeeRule(auction.companyId, plan);
            if (!feeRule)
                throw new common_1.BadRequestException('A taxa de leilão ainda não foi configurada no Admin.');
        }
        await this.dataSource.query(`UPDATE classified_auctions SET
         ...
         "updatedAt"=now()
       WHERE id=$1`, [auctionId, enabled, JSON.stringify(modes), feeCents, deliveryNote]);
        return this.sellerConfig(uid, auctionId);
    }
    async buyerConfig(uid, auctionId) {
        const auction = await this.assertWinner(uid, auctionId);
        if (!auction.onlinePaymentEnabled)
            throw new common_1.BadRequestException('O anunciante não habilitou pagamento online para este arremate.');
        const credentials = await this.marketplacePayments.sellerMercadoPagoCredentials(auction.companyId);
        if (!credentials.accessToken)
            throw new common_1.ServiceUnavailableException('A conexão Mercado Pago do anunciante está indisponível.');
        const platform = await this.providerConfig.getSecretConfig('MERCADO_PAGO');
        if (!platform.publicKey)
            throw new common_1.ServiceUnavailableException('A Public Key Mercado Pago do PiraNegócios não está configurada.');
        const buyerRows = await this.dataSource.query(`SELECT email,"displayName","fullName","socialName",address,city,state FROM users WHERE id=$1 LIMIT 1`, [uid]);
        const terms = await this.terms.status(uid);
        ...
        return {
            ...
            terms: { version: classifieds_marketplace_terms_service_1.CLASSIFIEDS_PAYMENT_TERMS_VERSION, accepted: terms.buyerAccepted === true },
    ...
    async createPayment(uid, auctionId, body) {
        ...
        if (fulfillmentMode === 'DELIVERY' && !fulfillmentData.address) {
            throw new common_1.BadRequestException('Informe o endereço de entrega.');
        }
        const buyerRows = await this.dataSource.query(`SELECT id,email FROM users WHERE id=$1 LIMIT 1`, [uid]);
        const buyer = buyerRows[0];
        if (!buyer?.email)
            throw new common_1.BadRequestException('Sua conta precisa ter e-mail válido para pagar online.');
        const plan = await this.entitlements.companyPlan(auction.companyId);
        const feeRule = await this.resolveAuctionFeeRule(auction.companyId, plan);
        if (!feeRule)
            throw new common_1.BadRequestException('A taxa de leilão ainda não foi configurada.');
        ...
        if (order && !['REJECTED', 'CANCELED', 'NOT_STARTED'].includes(String(order.paymentStatus))) {
            if (String(order.idempotencyKey || '') === idempotencyKey)
                return this.presentOrder(order);
            throw new common_1.BadRequestException('Já existe um pagamento em andamento para este arremate. Aguarde a confirmação antes de tentar novamente.');
        }
        if (!order) {
            const rows = await this.dataSource.query(`INSERT INTO classified_orders
         ...
         RETURNING *`, [auction.companyId, auction.listingId, uid, auctionId, auctionAmountCents, totalCents, platformFeeCents, sellerNetCents,
                method, fulfillmentMode, JSON.stringify(fulfillmentData), idempotencyKey, classifieds_marketplace_terms_service_1.CLASSIFIEDS_PAYMENT_TERMS_VERSION,
            ...
            const rows = await this.dataSource.query(`UPDATE classified_orders SET
         ...
         WHERE id=$1 RETURNING *`, [order.id, method, fulfillmentMode, JSON.stringify(fulfillmentData), idempotencyKey,
                new Date(Date.now() + (method === 'PIX' ? 35 * 60_000 : 24 * 60 * 60_000))]);
            order = rows[0];
        }
        await this.dataSource.query(`UPDATE classified_auctions SET "settlementOrderId"=$2,"settlementPaymentStatus"='PENDING',"updatedAt"=now() WHERE id=$1`, [auctionId, order.id]);
        const credentials = await this.marketplacePayments.sellerMercadoPagoCredentials(auction.companyId);
        const platform = await this.providerConfig.getSecretConfig('MERCADO_PAGO');
        ...
        try {
            ...
            payment = { message: text.slice(0, 500) };
        }
        if (!response.ok) {
            const ambiguous = response.status >= 500;
            await this.dataSource.query(`UPDATE classified_orders SET "paymentStatus"=$2,"providerStatusDetail"=$3,"updatedAt"=now() WHERE id=$1`, [order.id, ambiguous ? 'IN_PROCESS' : 'REJECTED', String(payment?.message || payment?.error || `HTTP ${response.status}`).slice(0, 160)]);
            await this.dataSource.query(`UPDATE classified_auctions SET "settlementPaymentStatus"=$2,"updatedAt"=now() WHERE id=$1`, [auctionId, ambiguous ? 'IN_PROCESS' : 'REJECTED']);
            if (!ambiguous)
                throw new common_1.BadRequestException(String(payment?.message || 'O Mercado Pago recusou o pagamento.').slice(0, 500));
            return { ...this.presentOrder(await this.orderForAuction(auctionId)), processing: true };
        }
        await this.applyPayment(order.id, auctionId, payment);
        return this.presentOrder(await this.orderForAuction(auctionId));
    }
    async resolveAuctionFeeRule(companyId, plan) {
        const custom = await this.dataSource.query(`SELECT * FROM classified_auction_fee_rules WHERE scope='COMPANY' AND "companyId"=$1 AND enabled=true LIMIT 1`, [companyId]).catch(() => []);
        const rule = custom[0] || (await this.dataSource.query(`SELECT * FROM classified_auction_fee_rules WHERE scope='PLAN' AND plan=$1 AND enabled=true LIMIT 1`, [plan]).catch(() => []))[0];
        if (!rule)
            return null;
        return {
    ...
    async assertSeller(uid, auctionId) {
        const identity = await this.identities.active(uid);
        if (identity.type !== 'COMPANY')
            throw new common_1.ForbiddenException('Somente a empresa anunciante pode configurar o recebimento do leilão.');
    ...
    async assertWinner(uid, auctionId) {
        const rows = await this.auctionRows(auctionId);
        const auction = rows[0];
        if (!auction)
            throw new common_1.NotFoundException('Leilão não encontrado.');
        if (auction.status !== 'ENDED' || !auction.winnerUserId || auction.finalAmount == null)
            throw new common_1.BadRequestException('Este leilão ainda não possui um arremate válido.');
        if (auction.winnerUserId !== uid)
            throw new common_1.ForbiddenException('Somente quem arrematou pode pagar este leilão.');
        return auction;
    }
    auctionRows(auctionId) {
        return this.dataSource.query(`SELECT a.*,l.title,l.slug,i.url AS image,c.name AS "companyName"
    ...
    orderForAuction(auctionId) {
        return this.dataSource.query(`SELECT * FROM classified_orders WHERE "auctionId"=$1 LIMIT 1`, [auctionId])
            .then((rows) => rows[0] || null).catch(() => null);
    }
    fulfillmentModes(value) {
        const values = Array.isArray(value) ? value : ['ARRANGE'];
        const modes = values.map(String).map((v) => v.toUpperCase()).filter((v) => ['ARRANGE', 'PICKUP', 'DELIVERY'].includes(v));
        const fallback = ['ARRANGE'];
        return [
            ...new Set(modes.length ? modes : fallback),
        ];
    }
    cleanModes(value) {
    ...
    fulfillmentMode(value, allowed) {
        const mode = String(value || allowed[0] || 'ARRANGE').toUpperCase();
        if (!allowed.includes(mode))
            throw new common_1.BadRequestException('Forma de recebimento indisponível.');
        return mode;
    }
    ...
    moneyToCents(value) {
        ...
        if (!Number.isFinite(n) || n <= 0)
            throw new common_1.BadRequestException('Valor do arremate inválido.');
        return Math.round(n * 100);
    }
    paymentPayload(order, auction, buyer, method, input, configuredBase) {
        ...
        if (!token || !paymentMethodId)
            throw new common_1.BadRequestException('O Mercado Pago não retornou um cartão tokenizado válido.');
    ...
    async applyPayment(orderId, auctionId, payment) {
        ...
        await this.dataSource.query(`UPDATE classified_orders SET "providerPaymentId"=$2,"paymentStatus"=$3,"providerStatusDetail"=$4,
         ...
         metadata=COALESCE(metadata,'{}'::jsonb) || $5::jsonb,"updatedAt"=now() WHERE id=$1`, [orderId, String(payment?.id || '') || null, status, String(payment?.status_detail || '').slice(0, 160) || null, JSON.stringify(metadata)]);
        await this.dataSource.query(`UPDATE classified_auctions SET "settlementPaymentStatus"=$2,"settlementOrderId"=$3,
         "settlementStatus"=CASE WHEN $2='APPROVED' THEN 'COMPLETED' ELSE "settlementStatus" END,"settlementUpdatedAt"=now(),"updatedAt"=now()
       WHERE id=$1`, [auctionId, status, orderId]);
    }
    async markProcessing(orderId, auctionId, detail) {
...
exports.ClassifiedsAuctionSettlementService = ClassifiedsAuctionSettlementService;
exports.ClassifiedsAuctionSettlementService = ClassifiedsAuctionSettlementService = __decorate([
...
//# sourceMappingURL=classifieds-auction-settlement.service.js.map