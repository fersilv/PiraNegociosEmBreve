...
export function ResumeDatabase() {
  ...
  if (profile?.type !== 'COMPANY') {
    return <div className="p-8 text-center text-stone-500">Acesso restrito a empresas.</div>;
  }
    ...
    return (
      <div className="max-w-2xl mx-auto py-12">
        <div className="bg-stone-50 border border-stone-200 rounded-3xl p-12 text-center shadow-sm">
          ...
          <h2 className="text-2xl font-serif font-bold text-stone-900 mb-4">Acesso Restrito (LGPD)</h2>
          <p className="text-stone-600 mb-8 leading-relaxed">
            Em conformidade com a Lei Geral de Proteção de Dados (LGPD), o acesso ao banco de currículos é restrito exclusivamente a empresas <strong>verificadas e aprovadas</strong> por nossa equipe.
          </p>
          <p className="text-stone-500 text-sm">
            Sua conta está atualmente em análise. Nossa equipe entrará em contato em breve para concluir a verificação.
          </p>
        </div>
      </div>
    );
  ...
  const filteredCandidates = candidates.filter(cand => {
    ...
    const nameMatch = cand.name?.toLowerCase().includes(kw);
    const bioMatch = cand.bio?.toLowerCase().includes(kw);
    const skillsMatch = Array.isArray(cand.skills) && cand.skills.some((s: string) => s.toLowerCase().includes(kw));
    const roleMatch = cand.role?.toLowerCase().includes(kw) || cand.headline?.toLowerCase().includes(kw);

    const matchesKeyword = !kw || nameMatch || bioMatch || skillsMatch || roleMatch;

    const candLoc = (cand.city ? `${cand.city} ${cand.state || ''}` : (cand.location || '')).toLowerCase();
    const matchesLocation = !loc || candLoc.includes(loc);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">
            Pesquise e filtre talentos cadastrados em nossa plataforma por competências, palavras-chave e localização.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-12 text-center text-stone-500 space-y-2">
          ...
          <p className="font-bold text-stone-700">Nenhum candidato encontrado</p>
          <p className="text-xs text-stone-400">Tente ajustar seus filtros de busca por palavra-chave ou localização.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-stone-200 bg-stone-50/80">
                  <th className="px-5 py-3.5 text-xs font-bold text-stone-500 uppercase tracking-wider">Candidato</th>
                  <th className="px-5 py-3.5 text-xs font-bold text-stone-500 uppercase tracking-wider">Localização</th>
                  <th className="px-5 py-3.5 text-xs font-bold text-stone-500 uppercase tracking-wider">Resumo / Competências</th>
                  <th className="px-5 py-3.5 text-xs font-bold text-stone-500 uppercase tracking-wider text-right">Ações</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-stone-100">
                {filteredCandidates.map(cand => {
                  const phoneFormatted = cand.phone ? cand.phone.replace(/\D/g, '') : '';
                  const cityState = cand.city ? `${cand.city}${cand.state ? ' - ' + cand.state : ''}` : (cand.location || 'Não informada');

                  return (
                    <tr key={cand.id} className="hover:bg-stone-50/80 transition-colors">
                      <td className="px-5 py-4 min-w-[200px]">
                        ...
                        {cand.email && <div className="text-xs text-stone-500 mt-0.5">{cand.email}</div>}
                        {cand.phone && (
                          <a 
                        ...
                        {cand.skills && cand.skills.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {cand.skills.map((skill: string, idx: number) => (
                              <span key={idx} className="bg-stone-100 text-stone-700 text-[10px] px-2 py-0.5 rounded font-medium">
                                {skill}
                              </span>
                            ))}
                          </div>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {cand.resumeURL && (
                            <button
                              type="button"
                              onClick={() => openBase64InNewTab(cand.resumeURL, `Currículo_${cand.name}`)}
