...
exports.WhatsAppOAuthService = void 0;
...
let WhatsAppOAuthService = class WhatsAppOAuthService {
    instances;
    apiKeys;
    clients;
    codes;
    tokens;
    constructor(instances, apiKeys, clients, codes, tokens) {
    ...
    authorizationServerMetadata() {
        ...
        return {
            ...
            scopes_supported: [...whatsapp_scopes_1.WHATSAPP_SCOPES, 'offline_access'],
            service_documentation: `${base}/admin/whatsapp`,
        };
    }
    async resourceMetadata(instanceId) {
        const instance = await this.instances.findOne({ where: { id: instanceId, active: true } });
        if (!instance)
            throw new common_1.NotFoundException('Número do WhatsApp não encontrado ou desativado.');
        return {
            resource: this.mcpResource(instanceId),
            authorization_servers: [this.publicBaseUrl()],
            scopes_supported: instance.allowedScopes.filter((scope) => whatsapp_scopes_1.WHATSAPP_SCOPES.includes(scope)),
    ...
    async registerClient(body) {
        ...
        const client = await this.clients.save(this.clients.create({
            clientId: `pn_mcp_${(0, crypto_1.randomBytes)(24).toString('hex')}`,
...
<button type="submit">Autorizar ChatGPT</button>
    ...
    async approveAuthorization(body) {
        ...
        if (!oauthScopes.length) {
            throw new common_1.UnauthorizedException('A chave não autoriza nenhuma das permissões solicitadas.');
        }
        const rawCode = `pn_oauth_code_${(0, crypto_1.randomBytes)(32).toString('hex')}`;
    ...
    async verifyAccessToken(instanceId, rawToken) {
        if (!rawToken.startsWith('pn_oauth_at_'))
        ...
        if (token.instanceId !== instanceId || token.resource !== this.mcpResource(instanceId)) {
            throw new common_1.UnauthorizedException('Este token não foi emitido para este recurso MCP.');
        }
        const instance = await this.instances.findOne({ where: { id: instanceId, active: true } });
        if (!instance)
            throw new common_1.UnauthorizedException('Número do WhatsApp desativado ou inexistente.');
    ...
    async issueTokens(instanceId, clientId, resource, scopes) {
        const accessToken = `pn_oauth_at_${(0, crypto_1.randomBytes)(32).toString('hex')}`;
        const refreshToken = `pn_oauth_rt_${(0, crypto_1.randomBytes)(40).toString('hex')}`;
    ...
    async requireInstanceFromResource(resource) {
        let url;
        try {
            url = new URL(resource);
        }
        catch {
            throw new common_1.BadRequestException('resource inválido.');
        }
        const base = new URL(this.publicBaseUrl());
        if (url.origin !== base.origin)
            throw new common_1.BadRequestException('resource pertence a outro domínio.');
        const match = url.pathname.match(/^\/api\/whatsapp\/mcp\/([^/]+)\/?$/);
        if (!match)
            throw new common_1.BadRequestException('resource não é um endpoint MCP do WhatsApp.');
        const instanceId = decodeURIComponent(match[1]);
        const instance = await this.instances.findOne({ where: { id: instanceId, active: true } });
        if (!instance || resource !== this.mcpResource(instanceId)) {
            throw new common_1.BadRequestException('resource MCP inexistente ou não canônico.');
        }
        return instance;
    }
    requestedScopes(scope, fallback) {
        const requested = String(scope || '')
            .split(/\s+/)
            .map((value) => value.trim())
            .filter(Boolean);
        const source = requested.length ? requested : [...fallback, 'offline_access'];
        const supported = new Set([...whatsapp_scopes_1.WHATSAPP_SCOPES, 'offline_access']);
    ...
    async validateApiKey(instanceId, rawKey) {
        if (!rawKey.startsWith('pn_wa_') || rawKey.length < 45) {
            throw new common_1.UnauthorizedException('Chave de autorização do WhatsApp inválida.');
        }
        ...
        if (!key || supplied.length !== expected.length || !(0, crypto_1.timingSafeEqual)(supplied, expected)) {
            throw new common_1.UnauthorizedException('Chave de autorização do WhatsApp inválida ou revogada.');
        }
        if (key.instanceId !== instanceId)
            throw new common_1.UnauthorizedException('Esta chave pertence a outro número.');
        if (key.expiresAt && key.expiresAt.getTime() <= Date.now())
            throw new common_1.UnauthorizedException('Esta chave expirou.');
...
exports.WhatsAppOAuthService = WhatsAppOAuthService;
exports.WhatsAppOAuthService = WhatsAppOAuthService = __decorate([
    ...
    __param(3, (0, typeorm_1.InjectRepository)(whatsapp_oauth_entity_1.WhatsAppOAuthCode)),
    __param(4, (0, typeorm_1.InjectRepository)(whatsapp_oauth_entity_1.WhatsAppOAuthToken)),
...
//# sourceMappingURL=whatsapp-oauth.service.js.map