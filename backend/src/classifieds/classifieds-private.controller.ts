...
export class ClassifiedsPrivateController {
  ...
  @Get('me/context')
  context(@Req() req: any) { return this.identities.context(req.user.uid); }
  @Get('me/limits')
  limits(@Req() req: any) { return this.entitlements.limits(req.user.uid); }
  @Post('me/context/select')
  selectIdentity(@Req() req: any, @Body() body: any) { return this.identities.select(req.user.uid, body?.identity); }
  @Post('me/terms/personal')
  acceptPersonalTerms(@Req() req: any, @Body() body: any) { return this.identities.acceptPersonalTerms(req.user.uid, body?.accepted); }
  @Post('me/company-profile')
  configureCompany(@Req() req: any, @Body() body: Record<string, unknown>) { return this.identities.configureCompany(req.user.uid, body || {}); }
  @Patch('me/company-profile')
  updateCompany(@Req() req: any, @Body() body: Record<string, unknown>) { return this.identities.configureCompany(req.user.uid, body || {}); }
  @Get('me/listings')
  mine(@Req() req: any) { return this.classifieds.mine(req.user.uid); }
  @Get('me/favorites')
  favorites(@Req() req: any) { return this.classifieds.favorites(req.user.uid); }
  @Get('me/analytics')
  analytics(@Req() req: any) { return this.commerce.analytics(req.user.uid); }
  @Get('me/offers')
  offers(@Req() req: any) { return this.commerce.listOffers(req.user.uid); }
  ...
  @Get('auctions')
  auctionsList(@Req() req: any) { return this.auctions.list(req.user.uid); }
  @Get('auctions/:auctionId')
  auctionDetail(@Req() req: any, @Param('auctionId') auctionId: string) { return this.auctions.detail(req.user.uid, auctionId); }
  ...
  async bidAuction(@Req() req: any, @Param('auctionId') auctionId: string, @Body() body: any) { await this.entitlements.assertAuctionParticipant(req.user.uid); return this.auctions.bid(req.user.uid, auctionId, body?.amount); }
  @Post('me/auctions/:auctionId/cancel')
  async cancelAuction(@Req() req: any, @Param('auctionId') auctionId: string) { await this.entitlements.assertAuctionCreation(req.user.uid); return this.auctions.cancel(req.user.uid, auctionId); }
  ...
  async update(@Req() req: any, @Param('id') id: string, @Body() body: Record<string, unknown>) {
    if (body.categorySlug !== undefined && body.listingType !== undefined) await this.taxonomy.assertCompatible(body.categorySlug, body.listingType, body.attributes);
  ...
  async status(@Req() req: any, @Param('id') id: string, @Body() body: { status?: unknown }) {
    ...
    if (status === 'PUBLISHED') { const identity = await this.identities.active(req.user.uid); await this.compliance.assertSellerEligible(req.user.uid, identity); }
    if (status === 'SOLD') return this.lifecycle.markSold(req.user.uid, id);
  ...
  async conversations(@Req() req: any) { const rows = await this.chats.list(req.user.uid); return this.commerce.decorateConversations(req.user.uid, rows); }
  ...
  async startConversation(@Req() req: any, @Param('listingId') listingId: string) {
    const relation = await this.dataSource.query(
      `SELECT id FROM classified_offers WHERE "listingId"=$1 AND "buyerUserId"=$2 ORDER BY "createdAt" DESC LIMIT 1`,
      [listingId, req.user.uid],
    ).catch(() => []);
    if (!relation[0]) {
      throw new ForbiddenException('A conversa de produto é liberada quando existe uma oferta. Para uma compra normal, finalize o pedido e a empresa poderá entrar em contato se necessário.');
    }
  ...
  renameConversation(@Req() req: any, @Param('conversationId') conversationId: string, @Body() body: any) { return this.commerce.renameConversation(req.user.uid, conversationId, body?.name); }
  @Delete('me/conversations/:conversationId')
  async archiveConversation(@Req() req: any, @Param('conversationId') conversationId: string) { return this.chats.archive(conversationId, req.user.uid); }
  @Get('me/chat-labels')
  labels(@Req() req: any) { return this.commerce.companyLabels(req.user.uid); }
  @Post('me/chat-labels')
  createLabel(@Req() req: any, @Body() body: any) { return this.commerce.createCompanyLabel(req.user.uid, body?.name, body?.colorKey); }
  @Patch('me/conversations/:conversationId/labels')
  setConversationLabels(@Req() req: any, @Param('conversationId') conversationId: string, @Body() body: any) { return this.commerce.setConversationLabels(req.user.uid, conversationId, body?.labelIds); }
  @Get('me/conversations/:conversationId/messages')
  messages(@Req() req: any, @Param('conversationId') conversationId: string) { return this.chats.listMessages(conversationId, req.user.uid); }
  ...
  readConversation(@Req() req: any, @Param('conversationId') conversationId: string) { return this.chats.markRead(conversationId, req.user.uid); }
}
