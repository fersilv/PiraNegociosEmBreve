...
exports.AlignedResumeImprovementService = void 0;
...
let AlignedResumeImprovementService = class AlignedResumeImprovementService {
    settingsService;
    resumeReviewService;
    constructor(settingsService, resumeReviewService) {
    ...
    repairJsonAtParsePosition(text, error) {
        ...
        if (!Number.isInteger(position) || position < 0 || position > text.length)
            return null;
        if (/Expected ',' or '[}\]]' after/i.test(message) ||
    ...
    async systemInstruction(input, baseline) {
        const [behavior, memory] = await Promise.all([
            this.settingsService.getAiBehavior(),
            this.settingsService.findRelevantAiBrain(`currículo otimização profissional qualificação ATS resumo experiência clareza ${JSON.stringify({ input, baseline }).slice(0, 3500)}`, 5, 3500),
        ]);
        return [
    ...
    async generateRaw(config, prompt, system, maxTokens) {
    ...
    normalize(raw, profile, baseline, targets) {
        ...
        inputChanges.slice(0, 24).forEach((item, index) => {
            ...
            const stageIndex = Number.isInteger(Number(item?.stageIndex))
                ...
                : undefined;
            if ((type === 'EXPERIENCE_DESCRIPTION' || type === 'STAGE_DESCRIPTION') &&
            ...
            if (type === 'STAGE_DESCRIPTION') {
                const timeline = Array.isArray(experiences[experienceIndex]?.timeline)
                    ? experiences[experienceIndex].timeline
                    : [];
                if (stageIndex === undefined || stageIndex < 0 || stageIndex >= timeline.length) {
                    return;
                }
            }
            const before = type === 'GLOBAL_SKILLS'
                ? this.cleanSkills(item?.before)
                : this.cleanText(item?.before);
            const after = type === 'GLOBAL_SKILLS'
        ...
        for (const item of rawNeeds.slice(0, targets.length)) {
            const targetId = this.cleanText(item?.targetId, 20);
            if (!allowedTargetIds.has(targetId))
                continue;
            if (needsUserInput.some((entry) => entry.targetId === targetId))
                continue;
            const target = targets.find((entry) => entry.id === targetId);
            needsUserInput.push({
                targetId,
                suggestion: target?.text || this.cleanText(item?.suggestion, 600),
                reason: this.cleanText(item?.reason, 700) ||
        ...
        if (uncovered.length > 0) {
            throw new Error(`AI_IMPROVEMENT_TARGETS_UNCOVERED:${uncovered.map((item) => item.id).join(',')}`);
        }
        return {
    ...
    makePrompt(input, baseline, targets, retryNote = '') {
    ...
    async propose(profile) {
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
            try {
                ...
                const raw = await this.generateRaw(config, this.makePrompt(input, baseline, targets, retryNote), system, attempt === 0 ? 8000 : 10000);
    ...
    applySelected(profile, proposal, selectedIds) {
        ...
        for (const change of proposal.changes || []) {
            ...
            if (change.type === 'EXPERIENCE_DESCRIPTION' &&
            ...
            if (change.type === 'STAGE_DESCRIPTION' &&
                typeof change.after === 'string' &&
                change.experienceIndex !== undefined &&
...
exports.AlignedResumeImprovementService = AlignedResumeImprovementService;
exports.AlignedResumeImprovementService = AlignedResumeImprovementService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [settings_service_1.SettingsService,
        resume_review_service_1.ResumeReviewService])
], AlignedResumeImprovementService);
//# sourceMappingURL=aligned-resume-improvement.service.js.map