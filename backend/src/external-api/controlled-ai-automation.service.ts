...
@Injectable()
export class ControlledAiAutomationService {
  constructor(private readonly dataSource: DataSource) {}

  private safeLimit(value: unknown, max = 200, fallback = 50) {
    const parsed = Math.round(Number(value || fallback));
    return Math.min(max, Math.max(1, Number.isFinite(parsed) ? parsed : fallback));
  }

  private async optionalQuery(sql: string, params: unknown[] = []) {
    try {
  ...
  async listingModerationQueue(limitRaw?: unknown) {
    const limit = this.safeLimit(limitRaw, 200, 50);
    const rows = await this.optionalQuery(
      `SELECT
  ...
  async listingModerationContext(listingId: string, limitRaw?: unknown) {
    const limit = this.safeLimit(limitRaw, 50, 20);
    const currentRows = await this.optionalQuery(
      `SELECT l.*,
         COALESCE(json_agg(i.url ORDER BY i."sortOrder") FILTER (WHERE i.url IS NOT NULL), '[]') AS images
       FROM classified_listings l
       LEFT JOIN classified_listing_images i ON i."listingId" = l.id
    ...
    const candidates = listing.companyId
      ? await this.optionalQuery(
          `SELECT l.*,
           ...
           GROUP BY l.id ORDER BY l."updatedAt" DESC LIMIT $3`,
          [listingId, listing.companyId, limit],
        )
      : await this.optionalQuery(
          `SELECT l.*,
             COALESCE(json_agg(i.url ORDER BY i."sortOrder") FILTER (WHERE i.url IS NOT NULL), '[]') AS images
           ...
           GROUP BY l.id ORDER BY l."updatedAt" DESC LIMIT $3`,
          [listingId, listing.sellerUserId, limit],
        );
  ...
  async applyListingModeration(
    ...
    if (!['APPROVE', 'DUPLICATE'].includes(decision)) {
      throw new BadRequestException('decision deve ser APPROVE ou DUPLICATE.');
    }

    const rows = await this.optionalQuery(
      `SELECT id,"sellerUserId","companyId",status FROM classified_listings WHERE id = $1 LIMIT 1`,
      [listingId],
    );
    ...
    if (decision === 'DUPLICATE') {
      if (!duplicateOfListingId) throw new BadRequestException('duplicateOfListingId é obrigatório para DUPLICATE.');
      const originals = await this.optionalQuery(
        `SELECT id,"sellerUserId","companyId" FROM classified_listings WHERE id = $1 LIMIT 1`,
        [duplicateOfListingId],
      );
      const original = originals[0];
      if (!original) throw new BadRequestException('O anúncio original informado não existe.');
      ...
      if (!sameOwner) throw new BadRequestException('O suposto duplicado pertence a outra identidade.');
      ...
      const updated = await this.dataSource.query(
        `UPDATE classified_listings
         ...
         WHERE id=$1 RETURNING id,status,"moderationReason","duplicateOfListingId","moderationReviewedAt"`,
        [listingId, reason || 'Possível anúncio duplicado identificado por moderação externa.', duplicateOfListingId],
      );
      return updated[0];
    }
  ...
  async reviewModerationQueue(limitRaw?: unknown) {
    const limit = this.safeLimit(limitRaw, 200, 50);
    const rows = await this.optionalQuery(
  ...
  async applyReviewModeration(reviewId: string, decisionRaw: unknown, reasonRaw?: unknown) {
    const decision = String(decisionRaw || '').trim().toUpperCase();
    ...
    const rows = await this.dataSource.query(
      `UPDATE classified_reviews
       SET status=$2,"moderationReason"=$3,
           ...
           "updatedAt"=now()
       WHERE id=$1
       RETURNING id,status,"moderationReason","approvedAt","rejectedAt","publishAt"`,
  ...
  async feedbackQueue(limitRaw?: unknown) {
    const limit = this.safeLimit(limitRaw, 500, 200);
    const rows = await this.optionalQuery(
      `SELECT id,message,"pagePath",process,"profileType",status,"createdAt","updatedAt"
       FROM product_feedback_requests
       WHERE status IN ('NEW','REVIEWING','PLANNED')
  ...
  async faqSource(limitRaw?: unknown) {
    const limit = this.safeLimit(limitRaw, 500, 200);
    const rows = await this.optionalQuery(
      `SELECT id,process,"pagePath",messages,"createdAt","updatedAt"
  ...
  async applyFeedbackInsights(clusters: Array<Record<string, unknown>>) {
      ...
      for (const cluster of normalized) {
        await manager.query(
          `INSERT INTO product_feedback_insights
           ...
           VALUES ($1,$2,$3::jsonb,$4,$5,$6,'EXTERNAL_AI')`,
          [cluster.title, cluster.summary, JSON.stringify(cluster.feedbackIds), cluster.feedbackIds.length, cluster.score, cluster.reason],
        );
      }
    });
  ...
  async applyFaqs(articles: Array<Record<string, unknown>>) {
    let saved = 0;
    for (const article of articles.slice(0, 50)) {
      ...
      const conversationIds = Array.isArray(article.conversationIds)
        ...
        : [];
      if (!title || !summary || !body || conversationIds.length < 1) continue;
      await this.dataSource.query(
        `INSERT INTO support_faq_articles
         ...
         ON CONFLICT (slug) DO UPDATE SET
           ...
           "requestCount"=EXCLUDED."requestCount","updatedAt"=now()`,
        [this.slug(title), title, summary, body, JSON.stringify(conversationIds), conversationIds.length],
      );
      saved += 1;
    }
    return { saved };
