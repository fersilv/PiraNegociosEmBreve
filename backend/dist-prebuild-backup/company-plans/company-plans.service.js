...
let CompanyPlansService = class CompanyPlansService {
    ...
    payments;
    providers;
    constructor(dataSource, payments, providers) {
    ...
    commercialBenefitCatalog(plan) {
        if (plan) {
            return {
                plan,
                benefits: (0, company_plan_benefits_1.benefitCatalogForPlan)(plan),
                defaultBenefitIds: (0, company_plan_benefits_1.defaultBenefitIdsForPlan)(plan),
            };
        }
        return {
            PLUS: {
                benefits: (0, company_plan_benefits_1.benefitCatalogForPlan)('PLUS'),
                defaultBenefitIds: (0, company_plan_benefits_1.defaultBenefitIdsForPlan)('PLUS'),
            },
            ELITE: {
                benefits: (0, company_plan_benefits_1.benefitCatalogForPlan)('ELITE'),
                defaultBenefitIds: (0, company_plan_benefits_1.defaultBenefitIdsForPlan)('ELITE'),
            },
        };
    }
    resolveCommercialBenefitIds(plan, configured) {
        return (0, company_plan_benefits_1.normalizeBenefitIds)(plan, configured);
    }
    resolveCommercialBenefits(plan, configured) {
        return (0, company_plan_benefits_1.benefitDefinitions)(this.resolveCommercialBenefitIds(plan, configured));
    }
    async managedCompany(userId) {
        const rows = await this.dataSource.query(`SELECT c.*, u."isCompanyAdmin", u.type AS "userType"
       FROM users u
       ...
       LIMIT 1`, [userId]);
        if (!rows[0]) {
            throw new common_1.ForbiddenException('Você não possui uma empresa administrável vinculada a esta conta.');
        }
        return rows[0];
    }
    async trialForCompany(companyId) {
        await this.dataSource.query(`UPDATE company_plan_trials
       SET status = 'EXPIRED', "updatedAt" = now()
       WHERE "companyId" = $1 AND status = 'ACTIVE' AND "endsAt" <= now()`, [companyId]).catch(() => undefined);
        const rows = await this.dataSource.query(`SELECT * FROM company_plan_trials WHERE "companyId" = $1 LIMIT 1`, [companyId]).catch(() => []);
        return rows[0] || null;
    }
    async configuredBenefitIds(plan, purchaseMode) {
        const productCode = plan === 'ELITE' ? 'COMPANY_ELITE_MONTHLY' : 'COMPANY_PLUS_MONTHLY';
        const rows = await this.dataSource.query(`SELECT "subscriptionBenefits", "oneTimeBenefits"
       FROM payment_products WHERE code = $1 LIMIT 1`, [productCode]).catch(() => []);
        const row = rows[0] || {};
    ...
    async getCompanyPlan(companyId) {
       ...
       SET status = 'EXPIRED', "updatedAt" = now()
       WHERE "companyId" = $1 AND status IN ('ACTIVE','PAST_DUE') AND "currentPeriodEnd" <= now()`, [companyId]).catch(() => undefined);
        const [subscriptionRows, trial] = await Promise.all([
            this.dataSource.query(`SELECT * FROM company_plan_subscriptions
        ...
        const plan = trialActive ? 'ELITE' : basePlan;
        ...
        let benefitIds = [];
        if (trialActive) {
            benefitIds = (0, company_plan_benefits_1.defaultBenefitIdsForPlan)('ELITE').filter((benefit) => benefit !== 'JOB_HIGHLIGHT' && benefit !== 'AD_HIGHLIGHT');
        }
        else {
        ...
        return {
            ...
            purchaseMode,
            benefitIds,
            benefits: (0, company_plan_benefits_1.benefitDefinitions)(benefitIds),
    ...
    async getForUser(userId) {
        ...
        const planProducts = new Map([['PLUS', plusProduct], ['ELITE', eliteProduct]]);
        ...
        return {
            ...
            plans: exports.COMPANY_PLAN_CATALOG.map((plan) => {
    ...
    async createCheckout(userId, requestedPlan, payer = {}) {
        const plan = this.normalizePlan(requestedPlan);
        if (plan === 'FREE') {
            throw new common_1.BadRequestException('O plano Free não precisa de checkout.');
        }
        const company = await this.managedCompany(userId);
        ...
        if (!current.isTrial && current.basePlan === plan && current.hasPaidSubscription) {
            throw new common_1.BadRequestException(`A empresa já possui o plano ${plan} ativo.`);
        }
       ...
       WHERE id = $1`, [payment.id, JSON.stringify(paymentMetadata)]);
        ...
        try {
            const checkout = await this.providers.createCheckout(payment, payer || {}, { trialDays });
    ...
    async latestCheckout(userId) {
        const company = await this.managedCompany(userId);
        const rows = await this.dataSource.query(`SELECT p.*, pp.name AS "productName"
       FROM payments p
       ...
       LIMIT 1`, [userId, company.id]);
        return rows[0] || null;
    }
    async setCancelAtPeriodEnd(userId, enabled) {
        const company = await this.managedCompany(userId);
        const rows = await this.dataSource.query(`UPDATE company_plan_subscriptions
       SET "cancelAtPeriodEnd" = $2, "updatedAt" = now()
       WHERE "companyId" = $1 AND status IN ('ACTIVE','PAST_DUE')
       RETURNING *`, [company.id, enabled]);
        if (!rows[0])
            throw new common_1.NotFoundException('A empresa não possui uma assinatura paga ativa.');
        return rows[0];
    }
};
