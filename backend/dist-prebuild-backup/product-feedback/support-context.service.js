...
exports.SupportContextService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
const user_entity_1 = require("../users/entities/user.entity");
const support_knowledge_1 = require("./support-knowledge");
...
let SupportContextService = class SupportContextService {
    dataSource;
    constructor(dataSource) {
        this.dataSource = dataSource;
    }
    async resolve(user, input) {
        ...
        const canAdmin = user.type === user_entity_1.UserType.ADMIN;
        const canCompany = !canAdmin && Boolean(user.companyId);
        const canCandidate = !canAdmin;
        const audience = canAdmin
            ? 'ADMIN'
            : canCompany
                ? 'COMPANY'
                : 'CANDIDATE';
        ...
        const allowedAudiences = new Set(['PUBLIC']);
        if (canAdmin)
            allowedAudiences.add('ADMIN');
        if (canCandidate)
            allowedAudiences.add('CANDIDATE');
        if (canCompany)
            allowedAudiences.add('COMPANY');
        const haystack = normalizeText(`${message} ${process}`);
        const scored = support_knowledge_1.SUPPORT_KNOWLEDGE_TOPICS
        ...
        const selected = [];
        if (routeMatches[0])
            selected.push(routeMatches[0].topic);
        for (const entry of scored) {
            if (selected.some((item) => item.id === entry.topic.id))
                continue;
        ...
        if (!selected.length) {
            const fallback = support_knowledge_1.SUPPORT_KNOWLEDGE_TOPICS.find((topic) => topic.id === (canAdmin ? 'admin.overview' : canCompany ? 'company.home-profile' : 'candidate.home'));
            if (fallback)
    ...
    isRestrictedRequest(input) {
        const normalized = normalizeText(input.message);
        if (input.pagePath.startsWith('/admin') && !input.canAdmin)
            return true;
        if (input.pagePath.startsWith('/company') && !input.canCompany)
            return true;
        if (input.pagePath.startsWith('/user') && !input.canCandidate)
            return true;
        if (!input.canAdmin) {
            ...
            if (adminOperations.some((term) => normalized.includes(normalizeText(term))))
                return true;
        }
        if (!input.canCompany) {
            ...
            if (companyOperations.some((term) => normalized.includes(normalizeText(term))))
                return true;
        }
        return false;
    }
    async loadLiveFacts(user, contextIds) {
        const facts = {
            account: {
        ...
        if (user.companyId && user.type !== user_entity_1.UserType.ADMIN) {
            try {
                const rows = await this.dataSource.query(`SELECT id, name, slug, "verificationStatus", category, city, state
           FROM companies WHERE id = $1 LIMIT 1`, [user.companyId]);
                if (rows[0])
                    facts.company = rows[0];
            }
            catch {
            }
        }
        if (user.companyId && contextIds.includes('company.page')) {
            try {
                const rows = await this.dataSource.query(`SELECT status, revision, "publishedAt", "templateKey"
           FROM company_pages WHERE "companyId" = $1 LIMIT 1`, [user.companyId]);
        ...
        if (user.companyId && contextIds.includes('company.jobs')) {
           ...
           FROM jobs WHERE "companyId" = $1`, [user.companyId]);
                if (rows[0])
                    facts.companyJobs = rows[0];
            }
            catch {
            }
        }
        if (contextIds.includes('candidate.classifieds')) {
            try {
                const rows = await this.dataSource.query(`SELECT status, COUNT(*)::int AS count
           FROM classified_listings WHERE "sellerUserId" = $1 GROUP BY status`, [user.id]);
        ...
        if (contextIds.includes('candidate.payments')) {
            try {
                const rows = await this.dataSource.query(`SELECT p.id, p."productCode", pp.name AS "productName", p.status,
    ...
    renderKnowledge(topics, haystack, restrictedRequest) {
        const chunks = [];
        chunks.push('REGRA DE ACESSO: o servidor já filtrou estes subcontextos para o perfil autenticado. Use somente o conteúdo abaixo. Não complete lacunas com rotas, telas, permissões ou dados de outro perfil.');
        if (restrictedRequest) {
            chunks.push('PEDIDO RESTRITO DETECTADO: não forneça passos operacionais, nomes de controles internos, rotas privadas, dados ou credenciais da área que o perfil não pode acessar. Explique apenas que aquela função é restrita e ofereça um caminho permitido ao cliente.');
        }
        for (const topic of topics) {
            ...
            if (!procedures.length && topic.procedures[0])
                procedures.push({ procedure: topic.procedures[0], score: 0 });
            const lines = [
            ...
            for (const { procedure } of procedures) {
                ...
                if (procedure.notes?.length)
                    lines.push(`Observações: ${procedure.notes.slice(0, 1).join(' | ')}`);
            }
            ...
            if (topic.boundaries?.length)
                lines.push(`Limites: ${topic.boundaries.slice(0, 3).join(' | ')}`);
...
exports.SupportContextService = SupportContextService;
exports.SupportContextService = SupportContextService = __decorate([
...
function matchRoute(pattern, path) {
    ...
    if (pattern === '/:companySlug') {
        ...
        if (segments.length === 1 && !RESERVED_PUBLIC_ROOTS.has(segments[0]))
            return 115;
        return 0;
    }
    if (pattern === '/:companySlug/termos' || pattern === '/:companySlug/privacidade') {
        ...
        const segments = path.split('/').filter(Boolean);
        if (segments.length === 2 && segments[1] === suffix && !RESERVED_PUBLIC_ROOTS.has(segments[0]))
            return 125;
        return 0;
    }
    if (pattern.endsWith('/') && path.startsWith(pattern))
        return 130;
    if (pattern === '/user' || pattern === '/company' || pattern === '/admin')
        return 0;
    return 0;
}
//# sourceMappingURL=support-context.service.js.map