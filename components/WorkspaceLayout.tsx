    ...
    <div className={`min-h-screen ${isCompany ? "bg-[#f3f2ee]" : "user-workspace bg-[#f5efe8] text-[#201813]"}`}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 hidden w-[288px] flex-col border-r md:flex ${isCompany ? "border-white/5 bg-[#1b1b18] text-white" : "border-white/5 bg-[#2b211c] text-white shadow-[18px_0_70px_rgba(48,31,22,0.10)]"}`}>
        ...
        <div className="mx-4 mb-4 space-y-3">
          ...
          <button type="button" onClick={openClassifieds} className="flex w-full items-center gap-3 rounded-2xl border border-[#e8aa89]/30 bg-[#c96847]/18 px-4 py-3 text-left text-xs font-black text-[#ffd8c5] shadow-[0_8px_24px_rgba(0,0,0,.12)] transition hover:-translate-y-0.5 hover:bg-[#c96847]/28 hover:text-white"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#c96847] text-white"><Tags className="h-4 w-4" /></span><span className="flex-1">Ir para os Classificados</span><span className="rounded-full bg-[#c96847] px-2 py-1 text-[8px] font-black uppercase tracking-[.12em] text-white">Novo</span></button>
      ...
      <nav className={`fixed inset-x-0 bottom-0 z-30 flex items-center justify-around border-t px-2 py-2.5 shadow-[0_-12px_40px_rgba(0,0,0,.09)] backdrop-blur-xl md:hidden ${isCompany ? "border-stone-200 bg-white/96" : "border-white/[0.08] bg-[#2b211c]/96 text-white"}`}>{(isCompany ? companyNavigation.slice(0, 4) : userMobileNavigation).map((item) => <MobileNavLink key={item.to} {...item} company={isCompany} />)}<Link to="/classificados/painel" className={`flex min-w-14 flex-col items-center gap-0.5 rounded-xl border px-2 py-1.5 text-[9px] font-black ${isCompany ? "border-[#c96847]/20 bg-[#fff0e8] text-[#a84f34]" : "border-[#e8aa89]/25 bg-[#c96847]/20 text-[#ffd8c5]"}`}><Tags className="h-5 w-5" /><span>Classificados</span></Link></nav>
