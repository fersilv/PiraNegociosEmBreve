...
export const COMMERCE_COMPANY_THEME_KEYS = [
  'loja',
  'vitrine',
  'marketplace',
  'catalogo',
...
export type CommerceCompanyThemeKey =
  (typeof COMMERCE_COMPANY_THEME_KEYS)[number];

export function isCommerceCompanyTheme(
  value?: string | null,
): value is CommerceCompanyThemeKey {
  return COMMERCE_COMPANY_THEME_KEYS.includes(
    String(value || '') as CommerceCompanyThemeKey,
  );
}
...
const THEME_DEFAULTS: Record<CommerceCompanyThemeKey, ThemeDefaults> = {
  loja: {
    categoryStyle: 'image-tiles',
    productsLayout: 'carousel',
    showSearch: false,
    bannerStyle: 'full',
    featuredTitle: 'Escolhas da marca',
  },
  vitrine: {
    categoryStyle: 'image-tiles',
    productsLayout: 'grid',
  ...
  gazeta: {
    ...
    showSearch: false,
    bannerStyle: 'editorial',
    featuredTitle: 'Classificados',
  },
  mosaico: {
    categoryStyle: 'image-tiles',
    productsLayout: 'masonry',
    showSearch: false,
    bannerStyle: 'full',
    featuredTitle: 'Descobertas',
  },
  radar: {
    categoryStyle: 'tiles',
    productsLayout: 'grid',
  ...
  pregao: {
    ...
    showSearch: true,
    bannerStyle: 'compact',
    featuredTitle: 'Oportunidades abertas',
  },
};

const LABELS: Record<string, string> = {
  categories: 'Categorias',
  classifieds: 'Produtos e serviços',
  about: 'Sobre',
  contact: 'Contato',
...
function categoryEntries(
  ...
  const configured = Array.isArray(config.categories?.items)
    ? config.categories.items.filter((item) => item.label)
    : [];
  ...
  if (configured.length) {
    return configured.map((item) => {
      ...
      const match = listings.find((listing) => {
        const slug = String(listing.categorySlug || '');
        return slug && (slug === item.id || Boolean(item.href?.includes(slug)));
      });
      return {
        ...item,
        imageUrl: primaryImage(match) || undefined,
      };
    });
  }
  ...
  const unique = [
    ...new Set(
      listings
        .map((listing) => String(listing.categorySlug || '').trim())
        .filter(Boolean),
    ),
  ].slice(0, 10);
  ...
  return unique.map((slug) => {
    const match = listings.find(
      (listing) => String(listing.categorySlug || '') === slug,
    );
    return {
      id: slug,
      label: slug
      ...
      href: `/classificados/categoria/${encodeURIComponent(slug)}`,
      imageUrl: primaryImage(match) || undefined,
    };
  });
}
...
function storefrontOptions(
  ...
  return {
    ...
    searchPlaceholder:
      config.storefront?.searchPlaceholder || 'O que você procura?',
    ...
    productsLayout:
      config.storefront?.productsLayout || defaults.productsLayout,
    cardsPerRow: config.storefront?.cardsPerRow || 4,
    featuredTitle:
      config.storefront?.featuredTitle || defaults.featuredTitle,
    showProducts: config.storefront?.showProducts !== false,
    showServices: config.storefront?.showServices !== false,
  };
}

function sectionWidthClass(section: CompanyPageSection) {
  return `pc-width-${section.width || 'wide'}`;
}

function CommerceSearch({ placeholder }: { placeholder: string }) {
  return (
    <form className="pc-search" action="/classificados/busca" method="get">
  ...
  return (
    <div className="pc-brand">
      {logo ? (
        ...
        <span>{String(company.name || 'L').slice(0, 1)}</span>
      )}
      <strong>{company.name}</strong>
      {companyAny.isVerified || companyAny.verificationStatus === 'VERIFIED' ? (
        <BadgeCheck className="pc-verified" />
      ) : null}
    </div>
  );
}
...
function Header({
  ...
  const links = sections.filter((section) => {
    ...
    if (!['categories', 'classifieds', 'about', 'contact'].includes(section.type)) {
      return false;
    }
    if (section.type === 'classifieds' && !hasCatalog) return false;
    return true;
  });
  ...
  return (
    <header
      className={`pc-header ${
        config.navigation?.sticky === false ? '' : 'is-sticky'
      }`}
    >
      <div className="pc-header-inner">
        <a href="#top" className="pc-brand-link">
          <BrandLockup company={company} />
        </a>
        {themeKey === 'marketplace' || themeKey === 'mercado' ? (
          <div className="pc-header-search">
            {search ? (
              <CommerceSearch
                placeholder={
                  config.storefront?.searchPlaceholder || 'Buscar nesta vitrine'
                }
              />
            ) : null}
          </div>
...
function Hero({
  ...
  const companyAny = company as any;
  const cover =
    config.cover?.enabled && config.cover?.url ? config.cover.url : '';
  const logo = companyAny.logoURL || companyAny.logoUrl;
  const location =
    companyAny.cityState ||
    [companyAny.city, companyAny.state].filter(Boolean).join(' · ');
  const options = storefrontOptions(config, themeKey);
  const imageCandidates = listings
    .map(primaryImage)
    .filter(Boolean)
    .slice(0, 4);
  ...
  if (themeKey === 'gazeta') {
    return (
      <section className="pc-hero pc-hero-gazeta">
        <div className="pc-gazeta-rule">
          <span>{location || 'Classificados locais'}</span>
          <span>{new Date().getFullYear()}</span>
        </div>
        <p className="pc-gazeta-kicker">
          {config.hero?.eyebrow || 'Edição comercial'}
        </p>
        <h1>{title}</h1>
        <div className="pc-gazeta-bottom">
          <p>{subtitle}</p>
          {listings.length ? (
            <a href="#vitrine">
              Ver anúncios <ArrowRight />
            </a>
          ) : null}
        </div>
      </section>
  ...
  if (themeKey === 'pregao') {
    return (
      <section className="pc-hero pc-hero-pregao">
        <div className="pc-pregao-top">
          <span>MARKET / {String(company.name || '').toUpperCase()}</span>
          <span>● ONLINE</span>
        </div>
        <div className="pc-pregao-grid">
          <div>
            <p className="pc-mono-label">VITRINE VERIFICADA</p>
            <h1>{title}</h1>
            <p>{subtitle}</p>
            {search ? (
              <CommerceSearch placeholder={options.searchPlaceholder} />
            ) : null}
          </div>
          <div className="pc-pregao-stats">
            <div>
              <small>ITENS</small>
              <b>{listings.length}</b>
            </div>
            <div>
              ...
              <b>
                {listings.filter((item) => item.listingType !== 'SERVICE').length}
              </b>
            </div>
            <div>
  ...
  if (
    themeKey === 'marketplace' ||
    themeKey === 'mercado' ||
    themeKey === 'classificados-pro'
  ) {
    return (
      <section
        className={`pc-hero pc-hero-discovery pc-hero-${themeKey}`}
      >
        <div className="pc-discovery-copy">
          <p className="pc-eyebrow">
            {config.hero?.eyebrow ||
              (themeKey === 'classificados-pro'
                ? 'Classificados da empresa'
                : 'Loja oficial')}
          </p>
          <h1>{title}</h1>
          <p>{subtitle}</p>
          ...
          <div className="pc-discovery-meta">
            <span>
              <BadgeCheck /> Empresa verificada
            </span>
            {location ? (
              <span>
                <MapPin /> {location}
        ...
        <div className="pc-discovery-art">
          {cover ? (
            <img src={cover} alt="" />
          ) : imageCandidates.length ? (
            <div className="pc-mini-collage">
              {imageCandidates.map((image, index) => (
                <img key={`${image}-${index}`} src={image} alt="" />
              ))}
            </div>
          ) : (
            <div className="pc-logo-poster">
              {logo ? <img src={logo} alt="" /> : <ShoppingBag />}
            </div>
          )}
        </div>
      </section>
  ...
  if (themeKey === 'catalogo') {
    return (
      <section className="pc-hero pc-hero-catalogo">
        <div className="pc-catalogo-index">
          <span>CATÁLOGO</span>
          <span>{String(listings.length).padStart(2, '0')} itens</span>
        </div>
        <div className="pc-catalogo-grid">
          <div>
            <p className="pc-eyebrow">
              {config.hero?.eyebrow || 'Coleção da empresa'}
            </p>
            <h1>{title}</h1>
            <p>{subtitle}</p>
            {listings.length ? (
          ...
          <div className="pc-catalogo-image">
            {cover ? (
              <img src={cover} alt="" />
            ) : imageCandidates[0] ? (
              <img src={imageCandidates[0]} alt="" />
            ) : logo ? (
              <img className="is-logo" src={logo} alt="" />
            ) : (
              <PackageOpen />
            )}
          </div>
  ...
  if (themeKey === 'mosaico') {
    return (
      <section className="pc-hero pc-hero-mosaico">
        <div className="pc-mosaico-title">
          <p className="pc-eyebrow">
            {config.hero?.eyebrow || 'Seleção visual'}
          </p>
          <h1>{title}</h1>
          <p>{subtitle}</p>
        </div>
        <div className="pc-mosaico-art">
          {[cover, ...imageCandidates]
            .filter(Boolean)
            .slice(0, 5)
            .map((image, index) => (
              <img key={`${image}-${index}`} src={image} alt="" />
            ))}
          {!cover && !imageCandidates.length ? (
            <div className="pc-logo-poster">
              {logo ? <img src={logo} alt="" /> : <ShoppingBag />}
            </div>
          ) : null}
  ...
  if (themeKey === 'radar') {
    return (
      <section className="pc-hero pc-hero-radar">
        <div className="pc-radar-rings">
          <i />
          <i />
          <i />
        </div>
        <div className="pc-radar-copy">
          <p className="pc-eyebrow">
            {config.hero?.eyebrow || 'Descoberta local'}
          </p>
          <h1>{title}</h1>
          <p>{subtitle}</p>
          {location ? (
            <div className="pc-radar-location">
              <MapPin /> {location}
            </div>
          ) : null}
          {search ? (
  ...
  return (
    <section
      className={`pc-hero pc-hero-store pc-banner-${options.bannerStyle}`}
      style={
        cover
          ? {
              backgroundImage: `linear-gradient(rgba(0,0,0,.28),rgba(0,0,0,.48)),url(${JSON.stringify(
                cover,
              )})`,
            }
          : undefined
    ...
    >
      <div className="pc-store-copy">
        <p className="pc-eyebrow">
          {config.hero?.eyebrow || 'Loja oficial'}
        </p>
        <h1>{title}</h1>
        <p>{subtitle}</p>
        <div className="pc-store-actions">
          {listings.length ? (
            <a href="#vitrine">
      ...
      {!cover ? (
        <div className="pc-store-art">
          {imageCandidates.slice(0, 3).map((image, index) => (
            <img key={`${image}-${index}`} src={image} alt="" />
          ))}
          {!imageCandidates.length ? (
            <div className="pc-logo-poster">
...
function PromoStrip({
  ...
  return (
    <div className="pc-promo-strip">
      ...
      {primary && secondary ? <span>{secondary}</span> : null}
    </div>
  );
}
...
function Categories({
  ...
  entries: CompanyPageCategoryLink[];
  style: CompanyCategoryStyle;
}) {
  if (!entries.length) return null;
  return (
    <div className={`pc-categories pc-categories-${style}`}>
      {entries.map((item, index) => (
        <a key={item.id || `${item.label}-${index}`} href={item.href || '#vitrine'}>
          <div className="pc-category-media">
            {item.imageUrl ? (
...
function listingHref(listing: ClassifiedListing) {
  return `/classificados/anuncio/${encodeURIComponent(String(listing.slug || ''))}`;
}
...
function ProductCard({
  ...
  const category = String(listing.categorySlug || 'item');

  if (themeKey === 'gazeta') {
    return (
      <Link to={listingHref(listing)} className="pc-gazeta-row">
        <span className="pc-gazeta-number">
          {String(index + 1).padStart(2, '0')}
        </span>
        <div>
          <small>
            {listing.listingType === 'SERVICE' ? 'SERVIÇO' : category}
          </small>
          <h3>{listing.title}</h3>
        </div>
        <strong>{classifiedPrice(listing)}</strong>
        <ArrowRight />
      </Link>
    );
  }
  ...
  if (themeKey === 'pregao') {
    return (
      <Link to={listingHref(listing)} className="pc-pregao-row">
        ...
        <strong>{classifiedPrice(listing)}</strong>
        <span className="pc-up">VER ↗</span>
      </Link>
    );
  }

  if (themeKey === 'classificados-pro') {
    return (
      <Link to={listingHref(listing)} className="pc-listing-row">
        <div className="pc-listing-thumb">
  ...
  return (
    <Link
      ...
      className={`pc-product-card ${listing.isFeatured ? 'is-featured' : ''}`}
    >
      <div className="pc-product-image">
        ...
        {listing.isFeatured ? <span>DESTAQUE</span> : null}
      </div>
...
function ListingShelf({
  ...
  return (
    <div className="pc-shelf">
      ...
      <div
        className={`pc-products pc-products-${layout}`}
        style={{ ['--pc-columns' as any]: cardsPerRow }}
      >
...
function ClassifiedsSection({
  ...
  return (
    <div className="pc-catalog-body">
      ...
      themeKey !== 'pregao' ? (
        <ListingShelf
          title={options.featuredTitle}
          items={featured}
          layout={
            options.productsLayout === 'list' ? 'grid' : options.productsLayout
          }
        ...
        <ListingShelf
          title={
            services.length
              ...
              : featured.length
                ? 'Todos os produtos'
                : options.featuredTitle
          }
          items={products}
          layout={options.productsLayout}
          cardsPerRow={options.cardsPerRow}
          themeKey={themeKey}
        />
      ) : null}
      ...
      {options.showServices ? (
        <ListingShelf
          title={products.length ? 'Serviços' : options.featuredTitle}
          items={services}
          layout={options.productsLayout}
          cardsPerRow={options.cardsPerRow}
          themeKey={themeKey}
...
function ContactSection({
  ...
  const rows = [
    [
      ...
      config.contacts?.whatsapp
        ? `https://wa.me/${String(config.contacts.whatsapp).replace(/\D/g, '')}`
        : '',
    ],
    [
      'Telefone',
      config.contacts?.phone || companyAny.phone,
      config.contacts?.phone || companyAny.phone
        ? `tel:${String(config.contacts?.phone || companyAny.phone).replace(/[^+\d]/g, '')}`
        : '',
    ],
    [
      'E-mail',
      config.contacts?.email || companyAny.email,
      config.contacts?.email || companyAny.email
        ? `mailto:${config.contacts?.email || companyAny.email}`
        : '',
    ],
    [
      'Site',
      config.contacts?.website || companyAny.website,
      cleanUrl(config.contacts?.website || companyAny.website),
    ],
...
function JobsSection({ jobs }: { jobs: PublicJobLike[] }) {
  if (!jobs.length) return null;
  return (
    <div className="pc-jobs">
      {jobs.slice(0, 6).map((job: any, index) => (
        <Link
          key={job.id || index}
          to={
            job.slug
        ...
        >
          <small>{job.workModel || 'Oportunidade'}</small>
          <h3>{job.title}</h3>
          <span>
...
function GenericSection({
  ...
  const companyAny = company as any;
  const widthClass = sectionWidthClass(section);

  if (section.type === 'about') {
    return (
      <section id="sobre" className={`pc-section pc-about ${widthClass}`}>
        <p className="pc-section-kicker">Sobre</p>
        <h2>{config.about?.title || 'Sobre a marca'}</h2>
        <p>{config.about?.text || companyAny.description || ''}</p>
      </section>
    );
  }
  ...
  if (section.type === 'contact') {
    return (
      <section id="contato" className={`pc-section ${widthClass}`}>
        <p className="pc-section-kicker">Contato</p>
        <h2>Fale com {company.name}</h2>
        <ContactSection company={company} config={config} />
      </section>
    );
  }

  if (section.type === 'jobs' && jobs.length) {
    return (
      <section id="vagas" className={`pc-section ${widthClass}`}>
        <p className="pc-section-kicker">Carreiras</p>
        <h2>{config.jobs?.title || 'Trabalhe com a gente'}</h2>
  ...
  if (section.type === 'socials') {
    ...
    return (
      <section className={`pc-section pc-social-section ${widthClass}`}>
        <p className="pc-section-kicker">Acompanhe</p>
        <div className="pc-socials">
          {links.map(([label, url]) => (
            <a
              key={String(label)}
              href={cleanUrl(String(url || ''))}
              target="_blank"
              rel="noreferrer"
            >
              {label}
  ...
  if (section.type === 'legal') {
    ...
    return (
      <section className={`pc-section pc-legal ${widthClass}`}>
        {legal.termsEnabled ? (
          <details>
            <summary>{legal.termsTitle || 'Termos de uso'}</summary>
            <p>{legal.termsBody}</p>
          </details>
        ) : null}
        {legal.privacyEnabled ? (
          <details>
            <summary>
...
export function CommerceCompanyThemeRenderer({
  themeKey,
  company,
  jobs,
  config,
  ...
  const categories = useMemo(
    () => categoryEntries(config, items),
    [config.categories?.items, items],
  );
  const visible = sections.filter(
    (section) => section.enabled !== false && section.type !== 'identity',
  );
  ...
  const hasCatalog = loading || items.length > 0;

  return (
    <div
      className={`pc-theme pc-theme-${themeKey} ${preview ? 'is-preview' : ''}`}
      style={rootStyle}
    >
      <style>{COMMERCE_CSS}</style>
      <PromoStrip
        primary={options.promoText}
        secondary={options.secondaryPromoText}
      />
      <Header
        company={company}
        config={config}
      ...
      <main id="top">
        ...
        {visible.map((section) => {
          ...
          if (section.type === 'categories') {
            if (config.categories?.enabled === false || !categories.length) {
              return null;
            }
            return (
              <section
                key={section.id}
                id="categorias"
                className={`pc-section pc-section-categories ${widthClass}`}
              >
                <div className="pc-section-heading">
                  <p className="pc-section-kicker">Navegue</p>
                  <h2>{config.categories?.title || 'Categorias'}</h2>
                </div>
                <Categories entries={categories} style={options.categoryStyle} />
              </section>
            );
          }
          ...
          if (section.type === 'classifieds') {
            ...
            return (
              <section
                key={section.id}
                id="vitrine"
                className={`pc-section pc-section-catalog ${widthClass}`}
              >
                <div className="pc-section-heading">
                  <p className="pc-section-kicker">Vitrine oficial</p>
                  <h2>{options.featuredTitle}</h2>
                  <p>
                    Produtos e serviços publicados por {company.name} nos
                    Classificados.
                  </p>
                </div>
                <ClassifiedsSection
      ...
      <footer className="pc-company-footer">
        <BrandLockup company={company} />
        <span>
          {config.footer?.text ||
            'Vitrine, atendimento e informações da empresa.'}
        </span>
      </footer>
    </div>
  );
