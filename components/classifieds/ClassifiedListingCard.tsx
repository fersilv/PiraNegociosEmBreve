...
export function ClassifiedListingCard({
  ...
  const target = auction
    ? `/classificados/leiloes/${encodeURIComponent(auction.id)}`
    : `${detailBasePath}/${encodeURIComponent(listing.slug)}`;
  ...
  return (
    ...
    >
      ...
      <div className={`relative overflow-hidden ${auction ? 'bg-[#21120e]' : 'bg-[#eee8e2]'} ${compact ? 'aspect-[1.12/1]' : 'aspect-square sm:aspect-[4/3]'}`}>
        {image ? <img src={image} alt={listing.title} loading="lazy" className={`h-full w-full object-cover transition duration-500 group-hover:scale-[1.035] ${auction ? 'brightness-[.82] saturate-[1.08]' : ''}`} /> : <div className="flex h-full items-center justify-center text-[#9e8d84]"><ImageIcon className="h-9 w-9" /></div>}
        ...
        <div className="absolute left-2.5 top-2.5 flex flex-col items-start gap-1.5 sm:left-3 sm:top-3">
          {auction ? <span className="inline-flex items-center gap-1.5 rounded-full bg-[#ff633c] px-3 py-1.5 text-[9px] font-black uppercase tracking-[.13em] text-white shadow-[0_8px_25px_rgba(255,99,60,.35)]"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-white motion-reduce:animate-none" /> Leilão ao vivo</span> : <>{pricing.promotionActive && <span className="rounded-full bg-[#d45442] px-2.5 py-1 text-[9px] font-black uppercase tracking-[.12em] text-white shadow-sm">Oferta</span>}{listing.isFeatured && <span className="rounded-full bg-[#2d211c]/90 px-2.5 py-1 text-[9px] font-black uppercase tracking-[.12em] text-white backdrop-blur">Destaque</span>}{listing.commerceConfig?.onlineCheckout?.enabled && <span className="rounded-full bg-blue-600/95 px-2.5 py-1 text-[8px] font-black uppercase tracking-[.1em] text-white">Compra online</span>}</>}
        </div>
      ...
      <div className={`p-3 sm:p-4 ${auction ? 'bg-gradient-to-b from-[#fff8f4] to-white' : ''}`}>
        {auction ? <>
          <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[.12em] text-[#d15132]"><Gavel className="h-3.5 w-3.5" /> Disputa aberta</div>{listing.sellerVerifiedSnapshot && <ShieldCheck className="h-4 w-4 text-emerald-600" aria-label="Anunciante verificado" />}</div>
          <h3 className="mt-2 overflow-hidden text-[13px] font-black leading-[1.3] text-[#2d211c] sm:text-[15px]" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{listing.title}</h3>
          <div className="mt-3 flex items-center justify-between gap-3 border-t border-[#e8d7ce] pt-3"><span className="text-[9px] font-bold text-[#8c776c]">Próximo a partir de <strong className="text-[#3e2a21]">{money(auction.nextMinimum)}</strong></span><span className="rounded-full bg-[#2d211c] px-3 py-1.5 text-[9px] font-black text-white">Entrar no leilão</span></div>
        </> : <>
