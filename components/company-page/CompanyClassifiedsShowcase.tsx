...
export function CompanyClassifiedsShowcase({ companyId, companyName, variant = 'default' }: { companyId?: string; companyName?: string; variant?: ShowcaseVariant }) {
  ...
  return (
    <section id="vitrine" className={store ? 'border-t border-white/10 bg-[#121216] text-white' : 'border-t border-stone-200 bg-[#f7f6f3] text-stone-900'}>
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-[.18em] ${store ? 'text-[#f1ff3d]' : 'text-stone-400'}`}><ShoppingBag className="h-4 w-4" /> {store ? 'Loja da empresa' : 'Vitrine no PiraNegócios'}</div>
