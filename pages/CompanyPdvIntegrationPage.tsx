...
type IntegrationStatus = {
  ...
  products?: { total?: number; linked?: number; visible?: number; conflicts?: number; unavailable?: number };
};
...
type LocalProduct = {
  id:string; title:string; price?:number|string|null; status:string; image?:string|null; pdvProductId?:string|null; syncEnabled?:boolean; visible?:boolean; syncPrice?:boolean; syncStock?:boolean; updatedAt?:string; remoteAvailable?:boolean; conflictState?:string; conflictSnapshot?:any;
};

type CategoryIntegration = {
  remote:Array<{id:string;name:string;parentId?:string|null;active?:boolean}>;
  local:Array<{slug:string;name:string;parentSlug?:string|null;isActive?:boolean}>;
  links:Array<{pdvCategoryId:string;categorySlug:string}>;
};
...
export function CompanyPdvIntegrationPage() {
  ...
  const [categoryData,setCategoryData]=useState<CategoryIntegration>({remote:[],local:[],links:[]});
    ...
    try{
      ...
      if(data.connected){
        const [p,l,c]=await Promise.all([
          api.get('/pdv-integration/products',{params:{companyId}}),
          api.get('/pdv-integration/local-products',{params:{companyId}}),
          api.get('/pdv-integration/categories',{params:{companyId}}).catch(()=>({data:{remote:[],local:[],links:[]}})),
        ]);
        setProducts(Array.isArray(p.data)?p.data:[]); setLocalProducts(Array.isArray(l.data)?l.data:[]);
        setCategoryData({remote:Array.isArray(c.data?.remote)?c.data.remote:[],local:Array.isArray(c.data?.local)?c.data.local:[],links:Array.isArray(c.data?.links)?c.data.links:[]});
      } else { setProducts([]); setLocalProducts([]); setCategoryData({remote:[],local:[],links:[]}); }
  ...
  const resolveConflict=async(product:RemoteProduct,strategy:'PDV'|'PIRA')=>{setWorking(`resolve:${product.id}:${strategy}`);setError('');setMessage('');try{await api.post(`/pdv-integration/products/${encodeURIComponent(product.id)}/resolve`,{companyId,strategy});setMessage(strategy==='PDV'?`Conflito de ${product.name} resolvido usando os dados do PDV.`:`Conflito de ${product.name} resolvido usando os dados do PiraNegócios.`);await load();}catch(e:any){setError(e?.response?.data?.message||'Não foi possível resolver o conflito.');}finally{setWorking('');}};
  ...
  const mapCategory=async(pdvCategoryId:string,categorySlug:string)=>{setWorking(`category:${pdvCategoryId}`);setError('');setMessage('');try{await api.patch(`/pdv-integration/categories/${encodeURIComponent(pdvCategoryId)}`,{companyId,categorySlug});setMessage('Mapeamento de categoria salvo.');await load();}catch(e:any){setError(e?.response?.data?.message||'Não foi possível mapear esta categoria.');}finally{setWorking('');}};
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-16">
    <section className="overflow-hidden rounded-[28px] border border-stone-200 bg-stone-950 p-7 text-white shadow-sm sm:p-9">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-emerald-300">Integrações · PDV Inteligente</p><h1 className="mt-2 text-3xl font-black tracking-[-.045em] sm:text-5xl">Um catálogo, dois motores.</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-white/55">O PDV continua sendo a operação de caixa. O PiraNegócios sincroniza o que você permitir sem substituir regras de checkout, publicação, pagamento ou entrega.</p></div><div className="flex flex-wrap gap-2"><span className={`rounded-full px-3 py-2 text-[10px] font-black ${status.connected?'bg-emerald-400/15 text-emerald-300':'bg-white/10 text-white/55'}`}>{status.connected?'CONECTADO':'DESCONECTADO'}</span>{status.connected&&<span className="rounded-full bg-white/10 px-3 py-2 text-[10px] font-black text-white/65">{linkedCount} VINCULADOS</span>}{Boolean(status.products?.conflicts)&&<span className="rounded-full bg-amber-400/15 px-3 py-2 text-[10px] font-black text-amber-200">{status.products?.conflicts} CONFLITO(S)</span>}</div></div>
    </section>
