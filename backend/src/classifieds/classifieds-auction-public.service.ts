...
export class ClassifiedsAuctionPublicService {
  ...
  async detail(auctionId:string) {
    ...
    return {
      ...this.present(row),
      bids:bids.map((b:any)=>({id:b.id,amount:Number(b.amount),createdAt:b.createdAt,bidderName:this.maskName(b.bidderName)})),
      settlement:{
  ...
  private present(row:any){
    const current=row.currentBid==null?Number(row.startPrice):Number(row.currentBid);const starts=new Date(row.startsAt).getTime();const ends=new Date(row.endsAt).getTime();
  ...
  private fee(baseCents:number,row:any){let value=Math.round(Math.max(0,baseCents)*Number(row.auctionFeeRateBps||0)/10000);value=Math.max(Number(row.auctionFeeMinimumCents||0),value);if(row.auctionFeeMaximumCents!=null)value=Math.min(Number(row.auctionFeeMaximumCents),value);return Math.max(0,Math.min(baseCents,value));}
  private feePayer(value:unknown):'SELLER'|'BUYER'{return String(value||'').toUpperCase()==='BUYER'?'BUYER':'SELLER';}
  private feeDisclosure(row:any,feeCents:number|null){if(this.feePayer(row.auctionFeePayer)==='SELLER')return'A taxa de leilão é absorvida pela empresa anunciante.';const pct=Number(row.auctionFeeRateBps||0)/100;return feeCents==null?`Arremate + taxa de leilão de ${pct.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}%.`:`Arremate + taxa de leilão de ${(feeCents/100).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})} (${pct.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}%).`;}
  private pickupAddress(row:any){if(row.pickupAddressSnapshot)return row.pickupAddressSnapshot;const cityState=[row.companyCity,row.companyState].filter(Boolean).join('/');return[row.companyAddress,cityState].filter(Boolean).join(', ')||null;}
  private paymentMethods(value:unknown){const raw=Array.isArray(value)?value:[];const methods=[...new Set(raw.map(String).map(v=>v.toUpperCase()).filter(v=>v==='PIX'||v==='CARD'))];return methods.length?methods:['PIX','CARD'];}
