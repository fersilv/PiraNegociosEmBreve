...
import { Menu, Tags, User, X } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 border-b border-[#4b3328]/10 bg-[#fffaf5]/90 backdrop-blur-2xl">
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-0.5 lg:flex">
          ...
          <Link to="/vagas" className="rounded-xl px-3 py-2 text-sm font-semibold text-[#5e4a40] transition hover:bg-[#f2e8df] hover:text-[#2d211c]">Vagas</Link>
          <Link to="/carreiras" className="rounded-xl px-3 py-2 text-sm font-semibold text-[#5e4a40] transition hover:bg-[#f2e8df] hover:text-[#2d211c]">Carreiras</Link>
          <Link to="/como-funciona" className="rounded-xl px-3 py-2 text-sm font-semibold text-[#5e4a40] transition hover:bg-[#f2e8df] hover:text-[#2d211c]">Como funciona</Link>
      ...
      {mobileOpen && <div className="border-t border-[#4b3328]/10 bg-[#fffaf5] px-4 py-4 lg:hidden"><div className="mx-auto flex max-w-7xl flex-col gap-1">
        ...
        <Link onClick={() => setMobileOpen(false)} to="/como-funciona" className="rounded-xl px-3 py-3 text-sm font-bold text-[#4f3b31] hover:bg-[#f2e8df]">Como funciona</Link>
        <Link onClick={() => setMobileOpen(false)} to="/para-empresas" className="rounded-xl px-3 py-3 text-sm font-bold text-[#4f3b31] hover:bg-[#f2e8df]">Para empresas</Link>
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-[#4b3328]/10 pt-3">{user ? <Link onClick={() => setMobileOpen(false)} to={workspacePath} className="col-span-2 rounded-xl bg-[#2d211c] px-4 py-3 text-center text-sm font-bold text-white">Meu espaço</Link> : <><Link onClick={() => setMobileOpen(false)} to="/login" className="rounded-xl border border-[#4b3328]/10 bg-white px-4 py-3 text-center text-sm font-bold text-[#4f3b31]">Entrar</Link><Link onClick={() => setMobileOpen(false)} to="/login?returnTo=%2Fclassificados%2Fpublicar" className="rounded-xl bg-[#c96847] px-4 py-3 text-center text-sm font-bold text-white">Anunciar</Link></>}</div>
      </div></div>}
    </header>
  );
}
