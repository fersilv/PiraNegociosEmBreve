...
export function AdminCompanyPlanEditor({ companyId, companyName }: { companyId: string; companyName?: string }) {
  ...
  if (loading) {
    return <div className="flex items-center gap-2 rounded-xl border border-stone-200 bg-stone-50 p-4 text-xs font-semibold text-stone-500"><Loader2 className="h-4 w-4 animate-spin" /> Carregando plano da empresa...</div>;
  }
