...
import { SmartAddressFields } from '../components/classifieds/SmartAddressFields';
...
const emptyAddress = {
  ...
  latitude: null as number | null, longitude: null as number | null, placeId: null as string | null,
  isDefault: false, active: true,
};

const emptyLocation = {
  name: 'Loja / retirada', zipCode: '', street: '', number: '', complement: '', neighborhood: '', city: '', state: '',
  latitude: null as number | null, longitude: null as number | null, placeId: null as string | null,
  allowsPickup: true, allowsDeliveryOrigin: true, isDefaultPickup: false, isDefaultDeliveryOrigin: false,
  pickupInstructions: '', active: true,
};
...
export default function ClassifiedsLogisticsPage() {
  ...
  const createAddress = async () => {
    ...
    try {
      ...
      setNotice('Endereço salvo e pronto para ser usado nas cotações.');
      await load();
  ...
  const createLocation = async () => {
    ...
    try {
      ...
      setNotice('Ponto da empresa salvo e pronto para cálculo de frete.');
      await load();
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#b06448]">Comércio e recebimento</p><h1 className="mt-1 font-serif text-3xl font-black">Logística e endereços</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-stone-500">Digite o CEP para preencher o endereço automaticamente. Se não souber o CEP, informe UF, cidade e rua e use “localizar endereço”.</p></div>
      ...
      <section className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-stone-200 sm:p-6">
        <div className="flex items-center gap-3"><MapPin className="h-5 w-5 text-[#b06448]" /><div><h2 className="font-serif text-xl font-black">Meus endereços</h2><p className="text-xs text-stone-500">O endereço padrão é pré-selecionado ao consultar frete quando você estiver logado.</p></div></div>
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-stone-300 p-4"><p className="text-xs font-black text-stone-600">Adicionar endereço</p><SmartAddressFields value={addressForm} onChange={setAddressForm as any} labelKey="label" /><label className="mt-3 flex items-center gap-2 text-xs font-bold text-stone-600"><input type="checkbox" checked={addressForm.isDefault} onChange={(event) => setAddressForm((current) => ({ ...current, isDefault: event.target.checked }))} /> Tornar padrão</label><button disabled={working} onClick={() => void createAddress()} className="mt-4 inline-flex h-11 items-center gap-2 rounded-2xl bg-stone-900 px-4 text-xs font-black text-white disabled:opacity-50"><Plus className="h-4 w-4" /> Salvar endereço</button></div>
      </section>
      ...
      {business && settings && <>
        ...
        <section className="rounded-[28px] bg-white p-5 shadow-sm ring-1 ring-stone-200 sm:p-6"><div className="flex items-center gap-3"><PackageCheck className="h-5 w-5 text-[#276b64]" /><div><h2 className="font-serif text-xl font-black">Pontos de retirada e origem</h2><p className="text-xs text-stone-500">O endereço padrão de origem é usado automaticamente no cálculo de frete do anúncio.</p></div></div><div className="mt-4 grid gap-3 md:grid-cols-2">{locations.filter((item: any) => item.active).map((item: any) => <div key={item.id} className="rounded-2xl bg-stone-50 p-4 ring-1 ring-stone-200"><div className="flex justify-between gap-3"><div><p className="text-sm font-black">{item.name}</p><p className="mt-1 text-xs leading-5 text-stone-500">{item.street}, {item.number} · {item.neighborhood}<br />{item.city}/{item.state} · {item.zipCode}</p></div><div className="flex flex-col items-end gap-1">{item.isDefaultPickup && <Tag>RETIRADA PADRÃO</Tag>}{item.isDefaultDeliveryOrigin && <Tag>ORIGEM PADRÃO</Tag>}</div></div><p className="mt-2 text-[10px] font-bold text-stone-400">{item.allowsPickup ? 'Retirada habilitada' : 'Sem retirada'} · {item.allowsDeliveryOrigin ? 'Pode ser origem de entrega' : 'Não é origem'}</p><button disabled={working} onClick={() => void removeLocation(item.id)} className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-[10px] font-black text-red-600">Desativar</button></div>)}</div><div className="mt-5 rounded-2xl border border-dashed border-stone-300 p-4"><p className="text-xs font-black">Adicionar ponto da empresa</p><SmartAddressFields value={locationForm} onChange={setLocationForm as any} labelKey="name" /><div className="mt-3 grid gap-2 sm:grid-cols-2"><Toggle label="Permitir retirada" checked={locationForm.allowsPickup} onChange={(value) => setLocationForm((current) => ({ ...current, allowsPickup: value }))} /><Toggle label="Usar como origem de entrega" checked={locationForm.allowsDeliveryOrigin} onChange={(value) => setLocationForm((current) => ({ ...current, allowsDeliveryOrigin: value }))} /><Toggle label="Retirada padrão" checked={locationForm.isDefaultPickup} onChange={(value) => setLocationForm((current) => ({ ...current, isDefaultPickup: value }))} /><Toggle label="Origem padrão" checked={locationForm.isDefaultDeliveryOrigin} onChange={(value) => setLocationForm((current) => ({ ...current, isDefaultDeliveryOrigin: value }))} /></div><label className="mt-3 block"><span className="text-[10px] font-black uppercase text-stone-400">Instruções de retirada</span><textarea rows={2} value={locationForm.pickupInstructions} onChange={(event) => setLocationForm((current) => ({ ...current, pickupInstructions: event.target.value }))} className="mt-1 w-full rounded-xl border border-stone-200 px-3 py-2 text-sm" /></label><button disabled={working} onClick={() => void createLocation()} className="mt-4 inline-flex h-11 items-center gap-2 rounded-2xl bg-stone-900 px-4 text-xs font-black text-white"><Plus className="h-4 w-4" /> Salvar ponto</button></div></section>
