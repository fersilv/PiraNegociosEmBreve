...
function initialSections(config: CompanyPageConfig) {
  ...
  const result: CompanyPageSection[] = saved.length ? saved : capabilities.recommendedSections.map((type) => ({ id: type, type, enabled: true, locked: type === 'identity' }));
...
function hydrate(raw: CompanyPageConfig | null | undefined, company: PublicCompanyLike): CompanyPageConfig {
  ...
  const next: CompanyPageConfig = {
    ...
    storefront: {
      ...
      showSearch: config.storefront?.showSearch ?? true,
      searchPlaceholder: config.storefront?.searchPlaceholder || 'O que você procura?',
      bannerStyle: config.storefront?.bannerStyle || 'full',
      categoryStyle: config.storefront?.categoryStyle || 'image-tiles',
      productsLayout: config.storefront?.productsLayout || 'carousel',
      cardsPerRow: config.storefront?.cardsPerRow || 4,
      featuredTitle: config.storefront?.featuredTitle || 'Destaques',
...
function ensureThemeSections(config: CompanyPageConfig, themeKey: string) {
  ...
  capabilities.recommendedSections.forEach((type) => {
    if (!next.some((section) => section.type === type)) next.push({ id: `${type}-${Date.now()}-${next.length}`, type, enabled: true, locked: type === 'identity' });
  });
