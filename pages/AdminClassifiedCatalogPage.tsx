...
export default function AdminClassifiedCatalogPage() {
  ...
  const mutateListing = async (id: string, action: 'archive' | 'restore' | 'delete') => {
    const labels = { archive: 'arquivar', restore: 'restaurar', delete: 'excluir permanentemente' } as const;
    if (action !== 'restore' && !window.confirm(`Deseja ${labels[action]} este anúncio?${action === 'delete' ? ' Esta ação não pode ser desfeita.' : ''}`)) return;
    setWorking(`${action}-${id}`); setMessage('');
    try {
      if (action === 'delete') await api.delete(`/admin/classifieds-catalog/listings/${id}`);
      else await api.post(`/admin/classifieds-catalog/listings/${id}/${action}`);
      setSelected(null);
      setMessage(action === 'archive' ? 'Anúncio arquivado e removido dos relatórios ativos.' : action === 'restore' ? 'Anúncio restaurado.' : 'Anúncio excluído permanentemente.');
      await refresh();
    } catch (error: any) {
      setMessage(error?.response?.data?.message || `Não foi possível ${labels[action]} o anúncio.`);
    } finally { setWorking(''); }
  };

  const mutateAuction = async (auction: Auction, action: 'cancel' | 'archive' | 'restore' | 'delete') => {
    const destructive = action === 'delete';
    if (action !== 'restore' && !window.confirm(`${action === 'cancel' ? 'Cancelar' : action === 'archive' ? 'Arquivar' : 'Excluir permanentemente'} o leilão de “${auction.title}”?${destructive ? ' Esta ação não pode ser desfeita.' : ''}`)) return;
    setWorking(`${action}-${auction.id}`); setMessage('');
    try {
      ...
      setMessage(action === 'archive' ? 'Leilão arquivado e removido das visões operacionais.' : action === 'restore' ? 'Leilão restaurado para consulta histórica.' : action === 'cancel' ? 'Leilão cancelado.' : 'Leilão excluído permanentemente.');
      await refresh();
    } catch (error: any) {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6">
    <header className="overflow-hidden rounded-[30px] bg-[#191916] p-6 text-white shadow-[0_20px_60px_rgba(28,25,20,.15)] sm:p-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#ed9f8d]">Marketplace · operação</p><h1 className="mt-2 font-serif text-3xl font-black sm:text-4xl">Central dos Classificados</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-white/55">Modere e administre todos os produtos, serviços e leilões. Arquivados ficam fora dos relatórios operacionais, mas continuam disponíveis para consulta e restauração.</p></div>
        <button onClick={() => void refresh()} className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-white px-4 text-xs font-black text-stone-900"><RefreshCw className="h-4 w-4" /> Atualizar tudo</button>
      </div>
    </header>
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      <Metric label="Publicados" value={summary?.published || 0} icon={<CheckCircle2 className="h-4 w-4 text-emerald-600" />} />
      <Metric label="Pausados" value={summary?.paused || 0} icon={<PauseCircle className="h-4 w-4 text-amber-600" />} />
      <Metric label="Em revisão" value={summary?.pendingReview || 0} icon={<ShieldCheck className="h-4 w-4 text-violet-600" />} />
      <Metric label="Leilões ativos" value={summary?.activeAuctions || 0} icon={<Gavel className="h-4 w-4 text-blue-600" />} />
      <Metric label="Arquivados" value={(summary?.archivedListings || 0) + (summary?.archivedAuctions || 0)} icon={<Archive className="h-4 w-4 text-stone-500" />} />
    </section>

    <section className="overflow-hidden rounded-[28px] bg-white ring-1 ring-stone-200 shadow-sm">
      <div className="border-b border-stone-100 p-4 sm:p-5">
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row">
          <label className="relative min-w-0 flex-1"><Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400" /><input value={query} onChange={event => { setQuery(event.target.value); setPage(1); }} placeholder="Buscar por anúncio, empresa ou anunciante..." className="h-11 w-full rounded-2xl border border-stone-200 bg-stone-50 pl-11 pr-4 text-sm font-semibold outline-none focus:border-stone-400" /></label>
          {!archived && <select value={status} onChange={event => { setStatus(event.target.value); setPage(1); }} className="h-11 rounded-2xl border border-stone-200 bg-white px-4 text-xs font-black text-stone-600 outline-none"><option value="ALL">Todos os status</option>{showingAuctions ? <><option value="SCHEDULED">Agendados</option><option value="OPEN">Abertos</option><option value="ENDED">Encerrados</option><option value="CANCELED">Cancelados</option></> : <><option value="PUBLISHED">Publicados</option><option value="PENDING_REVIEW">Em revisão</option><option value="PAUSED">Pausados</option><option value="DRAFT">Rascunhos</option><option value="SOLD">Vendidos</option></>}</select>}
        </div>
      </div>
...
function ListingList({ rows, open, detailLoading }: { rows: Listing[]; open: (id: string) => void; detailLoading: boolean }) {
  ...
  return <div className="divide-y divide-stone-100">{rows.map(row => <button key={row.id} onClick={() => open(row.id)} className="grid w-full gap-4 p-4 text-left hover:bg-stone-50 sm:p-5 lg:grid-cols-[72px_minmax(0,1.4fr)_minmax(150px,.7fr)_130px_auto] lg:items-center"><Thumb src={row.image} /><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-[9px] font-black uppercase tracking-[.12em] text-stone-400">{row.listingType === 'SERVICE' ? 'Serviço' : 'Produto'}</span>{row.isFeatured && <span className="inline-flex items-center gap-1 rounded-full bg-violet-50 px-2 py-1 text-[9px] font-black text-violet-700"><Sparkles className="h-3 w-3" /> Destaque</span>}</div><p className="mt-1 truncate text-sm font-black text-stone-950">{row.title}</p><p className="mt-1 truncate text-xs text-stone-400">{row.companyName || row.sellerName || 'Anunciante'} · {row.categoryName || row.categorySlug}</p></div><div><p className="text-xs font-bold text-stone-700">{row.city} - {row.state}</p><p className="mt-1 text-[10px] text-stone-400">{row.sellerEmail || 'Conta empresarial'}</p></div><StatusBadge value={row.status} /> <span className="inline-flex items-center gap-1 text-[10px] font-black text-stone-500">{detailLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Eye className="h-3.5 w-3.5" />} Administrar</span></button>)}</div>;
}

function AuctionList({ rows, working, mutate }: { rows: Auction[]; working: string; mutate: (row: Auction, action: 'cancel'|'archive'|'restore'|'delete') => void }) {
  ...
  return <div className="divide-y divide-stone-100">{rows.map(row => <div key={row.id} className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[72px_minmax(0,1.4fr)_160px_130px_auto] lg:items-center"><Thumb src={row.image} /><div className="min-w-0"><p className="truncate text-sm font-black">{row.title}</p><p className="mt-1 text-xs text-stone-400">{row.companyName}</p><p className="mt-1 text-[10px] font-bold text-stone-500">{Number(row.bidCount || 0)} lance(s) · atual {money(row.currentBid || row.startPrice)}</p></div><div className="text-xs text-stone-500"><p><strong>Início:</strong> {dateTime(row.startsAt)}</p><p className="mt-1"><strong>Fim:</strong> {dateTime(row.endsAt)}</p></div><StatusBadge value={row.status} /><div className="flex flex-wrap justify-end gap-2">{['SCHEDULED','OPEN'].includes(row.status) && <ActionButton disabled={Boolean(working)} onClick={() => mutate(row,'cancel')} icon={<XCircle className="h-3.5 w-3.5" />} label="Cancelar" /> }<ActionButton disabled={Boolean(working)} onClick={() => mutate(row,'archive')} icon={<Archive className="h-3.5 w-3.5" />} label="Arquivar" /><ActionButton destructive disabled={Boolean(working)} onClick={() => mutate(row,'delete')} icon={<Trash2 className="h-3.5 w-3.5" />} label="Excluir" /></div></div>)}</div>;
}
