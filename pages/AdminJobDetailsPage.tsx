...
export function AdminJobDetailsPage() {
  ...
  const moderate = async (action: "APPROVE" | "REJECT") => {
    ...
    if (
      action === "REJECT" &&
      !window.confirm(
        `Rejeitar a vaga “${data.job.title}”? Ela não será publicada.`,
      )
    )
      return;
    setSaving(true);
    setError("");
    try {
      await api.put(`/admin/jobs/${data.job.id}/moderation`, { action });
      await load();
    } catch (requestError: any) {
      setError(
        requestError?.response?.data?.message ||
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 admin-standalone-page">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link
          to="/admin/vagas"
          className="inline-flex items-center gap-2 rounded-xl border border-stone-200 bg-white px-3.5 py-2.5 text-sm font-bold text-stone-600 shadow-sm"
        >
          <ArrowLeft className="h-4 w-4" /> Voltar para vagas
        </Link>
        <div className="flex flex-wrap gap-2">
          {job.slug && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-xl border border-stone-200 bg-white px-3.5 py-2.5 text-sm font-bold text-stone-700 shadow-sm"
            >
          ...
          {job.moderationStatus === "PENDING" ? (
            <>
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3.5 py-2.5 text-sm font-bold text-white shadow-sm disabled:opacity-50"
              >
                {saving ? (
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-3.5 py-2.5 text-sm font-bold text-white shadow-sm disabled:opacity-50"
              >
            ...
            >
              ...
              {job.active ? "Encerrar vaga" : "Reabrir vaga"}
            </button>
          )}
        </div>
      </div>
      ...
      <header className="relative overflow-hidden rounded-[28px] bg-[#171714] p-5 text-white shadow-[0_24px_70px_rgba(38,33,29,.16)] sm:p-7">
        ...
        <div className="relative z-10">
          ...
          <h1 className="mt-3 max-w-4xl font-serif text-3xl font-bold tracking-tight sm:text-4xl">
            {job.title}
          </h1>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Metric
          ...
          label="Modelo"
          value={
            [job.type, job.workModel].filter(Boolean).join(" · ") ||
            "Não informado"
          }
        />
        <Metric
        ...
        <Metric
          ...
          label="Candidaturas"
          value={String(data.applicationCount || 0)}
        />
      </section>
        ...
        <div className="space-y-5">
          <Section title="Sobre a vaga" icon={<FileText className="h-5 w-5" />}>
            <RichText
              ...
              empty="Descrição não informada."
            />
          </Section>
          <Section title="Requisitos" icon={<Briefcase className="h-5 w-5" />}>
            <RichText
              value={job.requirements}
              empty="Nenhum requisito separado foi informado."
            />
          </Section>
        </div>
        ...
        <aside className="space-y-4">
          <section className="rounded-[22px] border border-stone-200 bg-[#fffdfa] p-5 shadow-sm">
            ...
            <Info
              label="Origem"
              value={
                job.isExternalListing ? "Vaga externa" : "Empresa da plataforma"
              }
            />
            <Info
              label="Candidatura"
          ...
          {job.acceptsPlatformApplications === false && (
            <section className="rounded-[22px] border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950">
              ...
              {job.applicationWhatsApp && (
                <p className="mt-3">
                  WhatsApp: <strong>{job.applicationWhatsApp}</strong>
                </p>
              )}
              {job.applicationEmail && (
                <p className="mt-2">
                  E-mail: <strong>{job.applicationEmail}</strong>
                </p>
              )}
              {job.externalApplicationInstructions && (
                <p className="mt-3 whitespace-pre-wrap leading-6 text-amber-800">
                  {job.externalApplicationInstructions}
                </p>
              )}
            </section>
          ...
          {job.sourceUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center justify-between gap-3 rounded-[22px] border border-stone-200 bg-white p-4 text-sm font-bold text-stone-700 shadow-sm"
            >
...
function Metric({
  ...
  return (
    <div className="rounded-[20px] border border-stone-200 bg-[#fffdfa] p-4 shadow-sm">
      ...
      <p className="mt-2 break-words text-sm font-bold text-stone-900">
        {value}
      </p>
    </div>
  );
...
function Info({
  ...
  return (
    <div className="mt-4 border-t border-stone-100 pt-3 first:border-t-0 first:pt-0">
      <p className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[.12em] text-stone-400">
        {icon}
        {label}
      </p>
