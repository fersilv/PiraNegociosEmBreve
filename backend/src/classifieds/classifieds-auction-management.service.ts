...
@Injectable()
export class ClassifiedsAuctionManagementService {
  constructor(
  ...
  async detail(uid: string, auctionId: string) {
    ...
    const companyId = identity.type === 'COMPANY' ? identity.company!.id : null;
    const rows = await this.dataSource.query(
      `SELECT a.*, l.title, l.slug, l.description, l.city, l.state,
      ...
      [auctionId, uid, companyId],
    );
    const auction = rows[0];
    if (!auction) throw new NotFoundException('Leilão não encontrado.');
    if (!auction.owned && !auction.won && !auction.participated) {
      throw new ForbiddenException('Este leilão não pertence ao histórico desta identidade.');
    }

    const bids = await this.dataSource.query(
      `SELECT b.id,b.amount,b."createdAt",b."bidderUserId",b."bidderCompanyId",
       ...
       WHERE b."auctionId" = $1 ORDER BY b.amount DESC,b."createdAt" ASC LIMIT 100`,
      [auctionId],
    );

    let winnerContact: Record<string, unknown> | null = null;
    if (auction.owned && auction.status === 'ENDED' && auction.winnerUserId) {
      const winnerRows = await this.dataSource.query(
        `SELECT u.id,u.email,u.phone,u."whatsappPhoneE164",u."whatsappVerifiedAt",u."photoURL",
        ...
        [auction.winnerUserId, auction.winnerCompanyId || null],
      );
  ...
  async updateSettlement(uid: string, auctionId: string, rawStatus: unknown) {
    const identity = await this.identities.active(uid);
    if (identity.type !== 'COMPANY') throw new ForbiddenException('Somente a empresa anunciante pode atualizar a negociação.');
    const companyId = identity.company!.id;
    const status = String(rawStatus || '').trim().toUpperCase() as AuctionSettlementStatus;
    ...
    const rows = await this.dataSource.query(
      `UPDATE classified_auctions SET "settlementStatus"=$3,"settlementUpdatedAt"=now(),"updatedAt"=now()
       WHERE id=$1 AND "companyId"=$2 AND status='ENDED' AND "winnerUserId" IS NOT NULL
       RETURNING id,"settlementStatus","settlementUpdatedAt"`,
      [auctionId, companyId, status],
    );
    if (!rows[0]) throw new NotFoundException('Arrematação não encontrada para esta empresa.');
    return rows[0];
  }

  async assertCooldown(listingId: string) {
    const rows = await this.dataSource.query(
      `SELECT id,status,"closedAt",COALESCE("closedAt","updatedAt") + interval '48 hours' AS "cooldownUntil"
    ...
    if (rows[0]) {
      ...
      throw new BadRequestException(`Este produto entra em quarentena de leilão por 48 horas. Novo leilão disponível após ${until.toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}.`);
    }
    return { ok: true };
  }
