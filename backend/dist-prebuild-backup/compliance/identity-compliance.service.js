...
exports.IdentityComplianceService = exports.IDENTITY_COMPLIANCE_CONSENT_VERSION = void 0;
const common_1 = require("@nestjs/common");
const crypto_1 = require("crypto");
const promises_1 = require("fs/promises");
const path_1 = require("path");
const typeorm_1 = require("typeorm");
exports.IDENTITY_COMPLIANCE_CONSENT_VERSION = '2026-08-26-simple';
const REQUIRED_STANDARD_FILES = ['SELFIE'];
...
let IdentityComplianceService = class IdentityComplianceService {
    dataSource;
    constructor(dataSource) {
        this.dataSource = dataSource;
    }
    async resolveUserId(firebaseUid, email) {
        const rows = await this.dataSource.query(`SELECT id
       FROM users
       WHERE id::text=$1 OR ($2::varchar IS NOT NULL AND lower(email)=lower($2::varchar))
       ORDER BY CASE WHEN id::text=$1 THEN 0 ELSE 1 END
       LIMIT 1`, [firebaseUid, String(email || '').trim().toLowerCase() || null]);
        if (!rows[0]?.id)
            throw new common_1.NotFoundException('Usuário não encontrado. Atualize seu cadastro antes de iniciar a verificação.');
        return String(rows[0].id);
    }
    async myStatus(uid) {
        const users = await this.dataSource.query(`SELECT u.id,u.email,u."displayName",u."fullName",u."socialName",u.phone,u."whatsappPhoneE164",u."whatsappVerifiedAt",u."companyId",
       ...
       FROM users u LEFT JOIN companies c ON c.id=u."companyId" WHERE u.id::text=$1 LIMIT 1`, [uid]);
        const user = users[0];
        if (!user)
            throw new common_1.NotFoundException('Usuário não encontrado.');
        const verifications = await this.dataSource.query(`SELECT v.*,COALESCE(d.docs,'[]'::json) AS documents
       ...
       WHERE v."userId"=$1 ORDER BY v."createdAt" DESC`, [uid]).catch(() => []);
        const membership = user.companyId ? (await this.dataSource.query(`SELECT * FROM company_memberships WHERE "companyId"=$1 AND "userId"=$2 AND status='ACTIVE' LIMIT 1`, [user.companyId, uid]).catch(() => []))[0] || null : null;
        const authorizations = user.companyId ? await this.dataSource.query(`SELECT id,"partnerName","partnerEmail","partnerPhone","qsaQualification",status,"grantFullPowers",permissions,"submittedAt","reviewedAt","reviewReason","expiresAt","createdAt"
       FROM company_verification_authorizations WHERE "companyId"=$1 ORDER BY "createdAt" DESC LIMIT 20`, [user.companyId]).catch(() => []) : [];
        return {
            consentVersion: exports.IDENTITY_COMPLIANCE_CONSENT_VERSION,
            user: {
                id: user.id,
            ...
            verifications: verifications.map((row) => this.presentVerification(row)),
        };
    }
    async saveProfile(uid, body) {
        ...
        if (['PENDING', 'APPROVED'].includes(row.status))
            throw new common_1.BadRequestException('Esta verificação já foi enviada e não pode ser alterada agora.');
        let selectedQsaName = null;
        let selectedQsaQualification = null;
        ...
        if (context === 'COMPANY' && relationship === 'PARTNER') {
            ...
            if (qsa.length) {
                const match = qsa.find((item) => this.normalizeName(item?.name) === this.normalizeName(selectedQsaName));
                if (!match)
                    throw new common_1.BadRequestException('Selecione seu nome entre os sócios retornados pela consulta do CNPJ.');
                selectedQsaName = String(match.name || '').trim();
                selectedQsaQualification = String(match.qualification || '').trim() || null;
            }
        }
        const rows = await this.dataSource.query(`UPDATE identity_verifications SET
         relationship=$2,
         ...
         "updatedAt"=now()
       WHERE id=$1 RETURNING *`, [row.id, relationship, selectedQsaName, selectedQsaQualification, declaresAtLeast25Percent]);
        return this.presentVerification(rows[0]);
    }
    async replacePartners(uid, partnersRaw) {
        ...
        const cleaned = partners.map((item) => ({
        ...
        if (cleaned.some((item) => !item.name || item.participationPercentage == null))
            throw new common_1.BadRequestException('Preencha nome e participação dos sócios declarados.');
        await this.dataSource.transaction(async (manager) => {
            ...
            for (const item of cleaned) {
                await manager.query(`INSERT INTO company_partner_declarations
           ("companyId","declaredByUserId",name,email,phone,"participationPercentage","hasAdministrativePowers","isBeneficialOwner")
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [companyId, uid, item.name, item.email, item.phone, item.participationPercentage, item.hasAdministrativePowers, Number(item.participationPercentage) > 25]);
            }
        });
        return this.myStatus(uid);
    }
    async uploadDocument(uid, kindRaw, file, body) {
        const kind = String(kindRaw || '').trim().toUpperCase();
        if (!VALID_DOCUMENT_KINDS.has(kind))
            throw new common_1.BadRequestException('Tipo de arquivo inválido.');
        if (!file?.buffer?.length)
            throw new common_1.BadRequestException('Arquivo não recebido.');
        if (!VALID_MIMES.has(file.mimetype))
            throw new common_1.BadRequestException('Envie JPG, PNG, WEBP ou PDF.');
        if (kind === 'SELFIE' && file.mimetype === 'application/pdf')
            throw new common_1.BadRequestException('A selfie precisa ser uma imagem.');
        if (file.size > 12 * 1024 * 1024)
            throw new common_1.BadRequestException('O arquivo deve ter no máximo 12 MB.');
        const context = this.context(body.context);
        const companyId = context === 'COMPANY' ? await this.companyForUser(uid) : null;
        const verification = await this.upsertVerification(uid, companyId, context);
        if (['PENDING', 'APPROVED'].includes(verification.status))
            throw new common_1.BadRequestException('Esta verificação já foi enviada.');
        ...
        const storageKey = `${verification.id}-${kind.toLowerCase()}-${(0, crypto_1.randomBytes)(12).toString('hex')}.bin`;
        const dir = this.vaultDir();
        await (0, promises_1.mkdir)(dir, { recursive: true, mode: 0o700 });
        const finalPath = (0, path_1.join)(dir, storageKey);
        const tempPath = `${finalPath}.tmp-${process.pid}-${Date.now()}`;
        await (0, promises_1.writeFile)(tempPath, ciphertext, { mode: 0o600 });
        await (0, promises_1.rename)(tempPath, finalPath);
        try {
            const existing = await this.dataSource.query(`SELECT "storageKey" FROM identity_verification_documents WHERE "verificationId"=$1 AND kind=$2 LIMIT 1`, [verification.id, kind]);
            const rows = await this.dataSource.query(`INSERT INTO identity_verification_documents
         ...
         RETURNING id,kind,"mimeType","originalName","sizeBytes","uploadedAt"`, [verification.id, kind, storageKey, file.mimetype, String(file.originalname || kind).slice(0, 240), file.size, sha256, iv.toString('base64'), tag.toString('base64')]);
            if (existing[0]?.storageKey && existing[0].storageKey !== storageKey)
                await (0, promises_1.unlink)((0, path_1.join)(dir, existing[0].storageKey)).catch(() => undefined);
            return rows[0];
        }
        catch (error) {
            await (0, promises_1.unlink)(finalPath).catch(() => undefined);
            throw error;
        }
    }
    async submit(uid, body) {
        ...
        if (verification.status === 'APPROVED')
            return this.presentVerification(verification);
        const users = await this.dataSource.query(`SELECT email,"whatsappVerifiedAt" FROM users WHERE id=$1 LIMIT 1`, [uid]);
        if (!users[0]?.email)
            throw new common_1.BadRequestException('Sua conta precisa ter um e-mail antes da verificação. Em login social, o e-mail da conta já é suficiente.');
        if (!users[0]?.whatsappVerifiedAt)
            throw new common_1.BadRequestException('Valide seu telefone/WhatsApp antes de solicitar a análise.');
        ...
        const kinds = new Set(documents.map((item) => item.kind));
        const missing = REQUIRED_STANDARD_FILES.filter((kind) => !kinds.has(kind));
        if (missing.length)
            throw new common_1.BadRequestException('Tire uma selfie atual antes de solicitar a análise.');
        if (context === 'COMPANY') {
            ...
            if (!company?.hasCnpj || !company?.cnpj || !company?.cnpjSnapshot) {
                throw new common_1.BadRequestException('Consulte o CNPJ da empresa antes de solicitar a verificação empresarial.');
            }
            if (verification.relationship !== 'PARTNER') {
                throw new common_1.BadRequestException('Se você não é o sócio responsável, use a opção de enviar autorização ao sócio.');
            }
            if (verification.declaresAtLeast25Percent !== true) {
                throw new common_1.BadRequestException('Se você não possui 25% ou mais, indique um sócio responsável para autorizar a empresa.');
            }
            const qsa = Array.isArray(company.cnpjSnapshot?.qsa) ? company.cnpjSnapshot.qsa : [];
            if (qsa.length && !verification.selectedQsaName)
                throw new common_1.BadRequestException('Selecione qual sócio do QSA é você.');
        }
        if (body.accepted !== true || String(body.consentVersion || '') !== exports.IDENTITY_COMPLIANCE_CONSENT_VERSION) {
            throw new common_1.BadRequestException('Leia e aceite a versão vigente dos termos de verificação cadastral.');
        }
        const rows = await this.dataSource.query(`UPDATE identity_verifications SET status='PENDING',"consentVersion"=$2,"consentAcceptedAt"=now(),"submittedAt"=now(),
       "reviewedAt"=NULL,"reviewedByUserId"=NULL,"reviewReason"=NULL,"updatedAt"=now() WHERE id=$1 RETURNING *`, [verification.id, exports.IDENTITY_COMPLIANCE_CONSENT_VERSION]);
    ...
    async adminList(statusRaw) {
        const status = String(statusRaw || 'PENDING').toUpperCase();
        const filter = status === 'ALL' ? '' : `WHERE v.status=$1`;
        const params = status === 'ALL' ? [] : [status];
        return this.dataSource.query(`SELECT v.id,v."userId",v."companyId",v.context,v.relationship,v."declaresAtLeast25Percent",v."selectedQsaName",v.status,v."submittedAt",v."createdAt",
       ...
       ${filter} ORDER BY CASE v.status WHEN 'PENDING' THEN 0 WHEN 'NEEDS_CHANGES' THEN 1 ELSE 2 END,COALESCE(v."submittedAt",v."createdAt") ASC LIMIT 500`, params);
    }
    async adminDetail(verificationId) {
        const rows = await this.dataSource.query(`SELECT v.*,COALESCE(u."socialName",u."displayName",u."fullName",u.email) AS "userName",u.email,u.phone,u."whatsappPhoneE164",u."whatsappVerifiedAt",u.city,u.state,u.address,
       ...
       WHERE v.id=$1 LIMIT 1`, [verificationId]);
        if (!rows[0])
            throw new common_1.NotFoundException('Verificação não encontrada.');
        const [documents, auths, logs] = await Promise.all([
            ...
            rows[0].companyId ? this.dataSource.query(`SELECT id,"partnerName","partnerEmail","partnerPhone","qsaQualification",status,"grantFullPowers",permissions,"submittedAt","reviewedAt","reviewReason" FROM company_verification_authorizations WHERE "companyId"=$1 ORDER BY "createdAt" DESC`, [rows[0].companyId]) : Promise.resolve([]),
            this.dataSource.query(`SELECT l.id,l."documentId",l."actorUserId",l.action,l."createdAt",COALESCE(u."displayName",u."fullName",u.email) AS "actorName"
         FROM compliance_document_access_logs l LEFT JOIN users u ON u.id=l."actorUserId"
         JOIN identity_verification_documents d ON d.id=l."documentId" WHERE d."verificationId"=$1 ORDER BY l."createdAt" DESC LIMIT 100`, [verificationId]).catch(() => []),
        ]);
        return { verification: this.presentVerification(rows[0]), profile: rows[0], documents, authorizations: auths, accessLogs: logs };
    }
    async readDocument(actorUserId, documentId, ip) {
        ...
        const ciphertext = await (0, promises_1.readFile)((0, path_1.join)(this.vaultDir(), document.storageKey)).catch(() => null);
        if (!ciphertext)
            throw new common_1.NotFoundException('Arquivo criptografado não encontrado no cofre.');
        const decipher = (0, crypto_1.createDecipheriv)('aes-256-gcm', this.encryptionKey(), Buffer.from(document.ivBase64, 'base64'));
        ...
        try {
            ...
            throw new common_1.ServiceUnavailableException('Falha de integridade ao abrir o arquivo.');
        }
        if ((0, crypto_1.createHash)('sha256').update(plaintext).digest('hex') !== document.sha256)
            throw new common_1.ServiceUnavailableException('O arquivo não passou na verificação de integridade.');
        const ipHash = ip ? (0, crypto_1.createHash)('sha256').update(ip).digest('hex') : null;
    ...
    async adminReview(actorUserId, verificationId, body) {
        ...
        if (decision !== 'APPROVE' && !reason)
            throw new common_1.BadRequestException('Informe o motivo para reprovação ou solicitação de ajustes.');
        ...
        const rows = await this.dataSource.query(`UPDATE identity_verifications SET status=$2,"reviewedAt"=now(),"reviewedByUserId"=$3,"reviewReason"=$4,"updatedAt"=now() WHERE id=$1 RETURNING *`, [verificationId, status, actorUserId, reason]);
        if (decision === 'APPROVE') {
        ...
        if (current.context === 'COMPANY' && current.companyId) {
            ...
            if (decision === 'APPROVE' && validatesCompany) {
                ...
                await this.dataSource.query(`UPDATE companies SET "verificationStatus"='VERIFIED',"isVerified"=true,"complianceStatus"='APPROVED',"complianceGraceDeadline"=NULL,"complianceSuspendedAt"=NULL,"complianceSuspensionReason"=NULL,"updatedAt"=now() WHERE id=$1`, [current.companyId]);
    ...
    async assertSellerEligible(uid, identity) {
        if (identity?.type === 'COMPANY' && identity.company?.id) {
            ...
            if (!company || !(company.isVerified || company.verificationStatus === 'VERIFIED') || company.complianceStatus !== 'APPROVED') {
                throw new common_1.ForbiddenException('A empresa precisa concluir a verificação simplificada antes de publicar no Marketplace.');
            }
            const membership = await this.assertMembership(uid, companyId);
            if (membership.status !== 'ACTIVE')
                throw new common_1.ForbiddenException('Seu vínculo com a empresa não está ativo.');
            if (membership.permissions?.marketplace === false)
                throw new common_1.ForbiddenException('Seu perfil não possui permissão para publicar no Marketplace.');
            return true;
        }
        if (!(await this.hasApprovedIdentity(uid))) {
            throw new common_1.ForbiddenException('Antes da primeira publicação, conclua a verificação simples com selfie e telefone validado. A análise pode levar até 48 horas.');
        }
        return true;
    }
    ...
    async restoreCompanyResources(companyId) {
    ...
    async upsertVerification(uid, companyId, context) {
        const rows = await this.dataSource.query(`INSERT INTO identity_verifications("userId","companyId",context,relationship)
       VALUES ($1,$2,$3,$4)
       ON CONFLICT ("userId",COALESCE("companyId",'00000000-0000-0000-0000-000000000000'::uuid),context)
       DO UPDATE SET "updatedAt"=identity_verifications."updatedAt" RETURNING *`, [uid, companyId, context, context === 'PERSONAL' ? 'PERSONAL' : 'EMPLOYEE']);
        return rows[0];
    }
    async companyForUser(uid) {
        ...
        if (!companyId)
            throw new common_1.BadRequestException('Sua conta não está vinculada a uma empresa.');
        await this.assertMembership(uid, companyId);
        return companyId;
    }
    async assertMembership(uid, companyId) {
        ...
        if (companies[0]?.ownerId !== uid)
            throw new common_1.ForbiddenException('Seu vínculo com a empresa não está ativo.');
        const inserted = await this.dataSource.query(`INSERT INTO company_memberships("companyId","userId",role,"isPartner",permissions,status)
       VALUES ($1,$2,'PRIMARY_ADMIN',false,'{"companyProfile":true,"recruitment":true,"marketplace":true,"finance":true,"team":true}'::jsonb,'ACTIVE')
       ON CONFLICT ("companyId","userId") DO UPDATE SET status='ACTIVE' RETURNING *`, [companyId, uid]);
        return inserted[0];
    }
    async assertPrimaryAdmin(uid, companyId) {
        const membership = await this.assertMembership(uid, companyId);
        if (membership.role !== 'PRIMARY_ADMIN')
            throw new common_1.ForbiddenException('Somente o administrador principal pode alterar esses dados.');
        return membership;
    }
    context(value) {
    ...
    relationship(value, context) {
        ...
        if (!['EMPLOYEE', 'PARTNER'].includes(relationship))
            throw new common_1.BadRequestException('Informe se você é sócio(a) ou funcionário(a).');
        return relationship;
    }
    percentage(value) {
        ...
        if (!Number.isFinite(number) || number <= 0 || number > 100)
            throw new common_1.BadRequestException('Percentual societário inválido.');
    ...
    vaultDir() {
        return (0, path_1.resolve)(process.env.IDENTITY_VAULT_DIR || '/var/lib/piranegocios/identity-vault');
    }
    encryptionKey() {
        const raw = String(process.env.IDENTITY_VAULT_ENCRYPTION_KEY || '').trim();
        if (!raw)
            throw new common_1.ServiceUnavailableException('IDENTITY_VAULT_ENCRYPTION_KEY não configurada. Gere uma chave aleatória de 32 bytes antes de habilitar verificação por selfie.');
        const candidates = [Buffer.from(raw, 'base64'), Buffer.from(raw, 'hex')];
        const key = candidates.find((item) => item.length === 32);
        if (!key)
            throw new common_1.ServiceUnavailableException('IDENTITY_VAULT_ENCRYPTION_KEY precisa representar exatamente 32 bytes em base64 ou hex.');
        return key;
    }
};
exports.IdentityComplianceService = IdentityComplianceService;
exports.IdentityComplianceService = IdentityComplianceService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource])
], IdentityComplianceService);
//# sourceMappingURL=identity-compliance.service.js.map