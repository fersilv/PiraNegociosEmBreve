...
exports.ClassifiedsAuctionEngagementService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("typeorm");
const notifications_service_1 = require("../notifications/notifications.service");
let ClassifiedsAuctionEngagementService = class ClassifiedsAuctionEngagementService {
    dataSource;
    notifications;
    timer = null;
    ...
    async reminderStatus(uid, auctionId) {
        await this.assertAuction(auctionId);
        const rows = await this.dataSource.query(`SELECT enabled,"lastPresenceAt" FROM classified_auction_reminders WHERE "auctionId"=$1 AND "userId"=$2 LIMIT 1`, [auctionId, uid]).catch(() => []);
        return { enabled: rows[0]?.enabled === true, lastPresenceAt: rows[0]?.lastPresenceAt || null };
    }
    async setReminder(uid, auctionId, enabledRaw) {
        ...
        if (!['SCHEDULED', 'OPEN'].includes(auction.status) || new Date(auction.endsAt).getTime() <= Date.now()) {
            throw new common_1.BadRequestException('Este leilão já terminou e não aceita novos lembretes.');
        }
        const enabled = enabledRaw !== false;
        await this.dataSource.query(`INSERT INTO classified_auction_reminders("auctionId","userId",enabled,"updatedAt")
       VALUES ($1,$2,$3,now())
       ON CONFLICT ("auctionId","userId") DO UPDATE SET enabled=EXCLUDED.enabled,"updatedAt"=now()`, [auctionId, uid, enabled]);
        return { enabled };
    }
    async presence(uid, auctionId) {
        const auction = await this.assertAuction(auctionId);
        if (!['SCHEDULED', 'OPEN'].includes(auction.status))
            return { recorded: false };
        await this.dataSource.query(`INSERT INTO classified_auction_reminders("auctionId","userId",enabled,"lastPresenceAt","updatedAt")
       VALUES ($1,$2,false,now(),now())
       ON CONFLICT ("auctionId","userId") DO UPDATE SET "lastPresenceAt"=now(),"updatedAt"=now()`, [auctionId, uid]);
    ...
    async activateScheduled() {
        const rows = await this.dataSource.query(`UPDATE classified_auctions
    ...
    async sendPreStart() {
        const rows = await this.dataSource.query(`SELECT r.id,r."userId",a.id AS "auctionId",a."startsAt",l.title
       ...
       ORDER BY a."startsAt" ASC LIMIT 200`).catch(() => []);
        let sent = 0;
    ...
    async sendClosureThanks() {
        const auctions = await this.dataSource.query(`SELECT a.id,a."winnerUserId",a."finalAmount",l.title
       ...
       ORDER BY a."closedAt" ASC NULLS LAST LIMIT 30`).catch(() => []);
        let done = 0;
        for (const auction of auctions) {
            const bidders = await this.dataSource.query(`SELECT DISTINCT "bidderUserId" AS "userId" FROM classified_auction_bids WHERE "auctionId"=$1`, [auction.id]).catch(() => []);
            for (const bidder of bidders) {
                const winner = bidder.userId === auction.winnerUserId;
                const inserted = await this.dataSource.query(`INSERT INTO classified_auction_closure_notifications("auctionId","userId",kind)
           VALUES ($1,$2,$3) ON CONFLICT DO NOTHING RETURNING id`, [auction.id, bidder.userId, winner ? 'WINNER' : 'PARTICIPANT']).catch(() => []);
                if (!inserted[0])
                    continue;
            ...
            await this.dataSource.query(`INSERT INTO classified_auction_closure_notifications("auctionId","userId",kind)
         VALUES ($1,NULL,'PARTICIPANTS_COMPLETE') ON CONFLICT DO NOTHING`, [auction.id]).catch(() => undefined);
            done += 1;
        }
        return done;
    }
    async claimReminder(id, column) {
        const allowed = new Set(['preStartSentAt', 'startSentAt', 'missYouSentAt']);
        if (!allowed.has(column))
            return false;
        const rows = await this.dataSource.query(`UPDATE classified_auction_reminders SET "${column}"=now(),"updatedAt"=now()
       WHERE id=$1 AND "${column}" IS NULL RETURNING id`, [id]).catch(() => []);
        return Boolean(rows[0]);
    }
    async assertAuction(auctionId) {
...
exports.ClassifiedsAuctionEngagementService = ClassifiedsAuctionEngagementService;
exports.ClassifiedsAuctionEngagementService = ClassifiedsAuctionEngagementService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeorm_1.DataSource,
        notifications_service_1.NotificationsService])
], ClassifiedsAuctionEngagementService);
//# sourceMappingURL=classifieds-auction-engagement.service.js.map