...
import { ArrowLeft, BadgeCheck, BellRing, CalendarClock, Crown, Gift, Loader2, Search, Sparkles, UserCog, WalletCards, Zap } from "lucide-react";
...
type TimedFeature = "JOB_MATCH_PREMIUM" | "RESUME_BOOST" | "EARLY_JOB_ALERTS";
...
export function AdminBillingSupportPage() {
  ...
  const featureName = (feature: TimedFeature) => feature === "RESUME_BOOST" ? "Impulso" : feature === "EARLY_JOB_ALERTS" ? "Vagas em primeira mão" : "Match Inteligente";

  const grantFeature = async (feature: TimedFeature, days: number) => {
    if (!selectedId) return;
    setWorking(`feature-${feature}`);
    try {
      await api.post(`/admin/payments/users/${selectedId}/entitlements/${feature}`, { durationDays: days, note });
      setMessage(`${featureName(feature)} concedido por ${days} dias.`);
      await refresh();
    } catch (error: any) {
  ...
  const revokeFeature = async (feature: TimedFeature) => {
        ...
        <>
          <section className="grid gap-4 lg:grid-cols-[1fr_.7fr]">
            <div className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-stone-400">Conta selecionada</p><h2 className="mt-1 text-xl font-bold text-stone-950">{support.user.fullName || support.user.displayName}</h2><p className="text-sm text-stone-500">{support.user.email}</p><p className={`mt-2 text-[10px] font-black uppercase tracking-wider ${support.user.isOpenToWork ? "text-emerald-600" : "text-orange-600"}`}>{support.user.isOpenToWork ? "Visível no Banco de Talentos" : "Oculto do Banco de Talentos"}</p></div><UserCog className="h-6 w-6 text-violet-500" /></div>
          ...
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <BenefitCard icon={<Zap className="h-5 w-5" />} title="Impulso de currículo" description="Dá posições patrocinadas no Banco de Talentos e nas candidaturas sem alterar a compatibilidade real." active={support.entitlements?.some((item: any) => item.feature === "RESUME_BOOST" && item.active)} expiry={support.entitlements?.find((item: any) => item.feature === "RESUME_BOOST")?.expiresAt} onGrant={() => void grantFeature("RESUME_BOOST", 15)} onRevoke={() => void revokeFeature("RESUME_BOOST")} grantLabel="+15 dias" />
            <BenefitCard icon={<BellRing className="h-5 w-5" />} title="Vagas em primeira mão" description="Aviso antecipado de vagas novas compatíveis antes do disparo geral." active={support.entitlements?.some((item: any) => item.feature === "EARLY_JOB_ALERTS" && item.active)} expiry={support.entitlements?.find((item: any) => item.feature === "EARLY_JOB_ALERTS")?.expiresAt} onGrant={() => void grantFeature("EARLY_JOB_ALERTS", 30)} onRevoke={() => void revokeFeature("EARLY_JOB_ALERTS")} grantLabel="+30 dias" />
            <div className="rounded-3xl border border-violet-200 bg-violet-50 p-5"><div className="flex items-center gap-3 text-violet-700"><CalendarClock className="h-5 w-5" /><h3 className="font-bold">Plano Destaque mensal</h3></div><p className="mt-2 text-xs leading-5 text-stone-600">Ativa/estende 30 dias de assinatura, Match, Impulso e vagas em primeira mão.</p><button disabled={working === "subscription"} onClick={() => void activateSubscription()} className="mt-5 w-full rounded-xl bg-violet-700 px-4 py-3 text-xs font-black text-white">Ativar / +30 dias</button>{support.subscriptions?.[0] && <p className="mt-3 text-[10px] font-bold text-violet-700">{support.subscriptions[0].status} · até {dateLabel(support.subscriptions[0].currentPeriodEnd)}</p>}</div>
          </section>
