...
import type {
  ...
  CompanyProductsLayout,
} from '../components/company-page/CompanyPageExtensions';
import {
  getCompanyThemeCapabilities,
  themeSupports,
} from '../components/company-page/CompanyThemeCapabilities';

type Panel = 'themes' | 'sections' | 'store' | 'style' | 'content' | 'advanced';

type EditorStoreDefaults = {
  showSearch: boolean;
...
const DEFAULT_CATEGORIES: CompanyPageCategoryLink[] = [
  { id: 'novidades', label: 'Novidades', href: '#vitrine' },
  { id: 'produtos', label: 'Produtos', href: '#vitrine' },
  { id: 'servicos', label: 'Serviços', href: '#vitrine' },
];

const STORE_DEFAULTS: Record<string, EditorStoreDefaults> = {
  loja: { showSearch: false, bannerStyle: 'full', categoryStyle: 'image-tiles', productsLayout: 'carousel', featuredTitle: 'Escolhas da marca' },
  vitrine: { showSearch: false, bannerStyle: 'split', categoryStyle: 'image-tiles', productsLayout: 'grid', featuredTitle: 'Vitrine' },
  marketplace: { showSearch: true, bannerStyle: 'compact', categoryStyle: 'circles', productsLayout: 'carousel', featuredTitle: 'Destaques para você' },
  catalogo: { showSearch: false, bannerStyle: 'editorial', categoryStyle: 'tiles', productsLayout: 'grid', featuredTitle: 'Catálogo' },
  'classificados-pro': { showSearch: true, bannerStyle: 'compact', categoryStyle: 'chips', productsLayout: 'list', featuredTitle: 'Anúncios em destaque' },
  mercado: { showSearch: true, bannerStyle: 'split', categoryStyle: 'circles', productsLayout: 'carousel', featuredTitle: 'Ofertas da loja' },
  gazeta: { showSearch: false, bannerStyle: 'editorial', categoryStyle: 'chips', productsLayout: 'list', featuredTitle: 'Classificados' },
  mosaico: { showSearch: false, bannerStyle: 'full', categoryStyle: 'image-tiles', productsLayout: 'masonry', featuredTitle: 'Descobertas' },
  radar: { showSearch: true, bannerStyle: 'split', categoryStyle: 'tiles', productsLayout: 'grid', featuredTitle: 'Perto de você' },
  pregao: { showSearch: true, bannerStyle: 'compact', categoryStyle: 'chips', productsLayout: 'list', featuredTitle: 'Oportunidades abertas' },
};

function storeDefaults(templateKey?: string | null): EditorStoreDefaults {
  return STORE_DEFAULTS[String(templateKey || 'loja')] || STORE_DEFAULTS.loja;
}

function initialSections(config: CompanyPageConfig) {
  const capabilities = getCompanyThemeCapabilities(config.templateKey);
  const saved = Array.isArray(config.sections)
    ? config.sections.map((section) => ({ ...section }))
    : [];
  const result: CompanyPageSection[] = saved.length
    ? saved
    : capabilities.recommendedSections.map((type) => ({
        id: type,
        type,
        enabled: true,
  ...
  return result.map((section) =>
    section.type === 'identity'
      ? { ...section, enabled: true, locked: true }
      : section,
  );
}

function hydrate(
  raw: CompanyPageConfig | null | undefined,
  ...
  const next: CompanyPageConfig = {
    ...config,
    version: Math.max(6, Number(config.version || 6)),
...
function ensureThemeSections(config: CompanyPageConfig, themeKey: string) {
  const capabilities = getCompanyThemeCapabilities(themeKey);
  const sections = initialSections(config);
  const next = [...sections];

  capabilities.recommendedSections.forEach((type) => {
    if (!next.some((section) => section.type === type)) {
      next.push({
        id: `${type}-${Date.now()}-${next.length}`,
        type,
        enabled: true,
        locked: type === 'identity',
      });
...
export function CompanyPageBuilderV4() {
    ...
    (async () => {
      try {
        ...
        if (nextCompany.slug) {
          secondary.push(
            api
              .get(`/public/companies/${nextCompany.slug}`)
              .then((response) => {
                if (active) {
        ...
        secondary.push(
          api
            .get(`/classifieds/company/${companyId}/listings`)
            .then((response) => {
              if (!active) return;
              ...
              const rows = Array.isArray(payload)
                ...
                : Array.isArray(payload?.items)
                  ? payload.items
                  : [];
              setClassifiedsCount(rows.length);
            })
            .catch(() => {
              if (active) setClassifiedsCount(0);
            }),
        );
        ...
        if (active) {
          setError(
            requestError?.response?.data?.message ||
              'Não foi possível carregar Minha Página.',
          );
        }
      } finally {
  ...
  const sections = useMemo(
    () => (config ? initialSections(config) : []),
    [config],
  );
  const capabilities = useMemo(
    () => getCompanyThemeCapabilities(config?.templateKey),
    [config?.templateKey],
  );
  const commerce = isCommerceCompanyTheme(String(config?.templateKey || ''));
  ...
  const saveDraft = async () => {
    if (!companyId || !config || !company || saving) return;
    setSaving(true);
    setError('');
    setMessage('');
    try {
  ...
  const publish = async () => {
    if (!companyId || !config || !company || publishing) return;
    setPublishing(true);
    setError('');
    setMessage('');
    try {
  ...
  const updateSections = (next: CompanyPageSection[]) => {
    if (config) change({ ...config, sections: next });
  };
  ...
  const dropSection = (targetId: string) => {
    ...
    if (!source || !target || source.type === 'identity' || target.type === 'identity') {
      setDraggedId(null);
      return;
    }
  ...
  const dropAtEnd = () => {
    if (!draggedId) return;
    const source = sections.find((section) => section.id === draggedId);
    if (!source || source.type === 'identity') {
      setDraggedId(null);
      return;
    }
    updateSections([
      ...sections.filter((section) => section.id !== draggedId),
      source,
    ]);
    setDraggedId(null);
  };
    ...
    return (
      <div className="mx-auto max-w-2xl px-6 py-20 text-center">
        ...
        <p className="mt-3 text-stone-500">
          Sua conta precisa estar vinculada a uma empresa.
        </p>
      </div>
    );
  }

  const missingSections = ALL_SECTION_TYPES.filter(
    (type) => type !== 'identity' && !sections.some((section) => section.type === type),
  );

  return (
    <div className="min-h-screen bg-[#e8e7e5] text-stone-950">
        ...
        <div className="flex min-h-16 items-center gap-3 px-4 sm:px-6">
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-[.18em] text-stone-400">
              Minha Página · Studio V4
            </p>
            <div className="flex items-center gap-2">
              <h1 className="truncate text-base font-bold">{company.name}</h1>
              {(company as any).isVerified ||
              (company as any).verificationStatus === 'VERIFIED' ? (
                <BadgeCheck className="h-4 w-4 text-emerald-600" />
              ) : null}
            </div>
          </div>
          <div className="ml-auto flex gap-2">
            <TopButton onClick={saveDraft} loading={saving} icon={<Save className="h-4 w-4" />}>
              Salvar
            </TopButton>
            <TopButton onClick={preview} loading={previewing} icon={<Eye className="h-4 w-4" />} secondary>
              Prévia
            </TopButton>
            <TopButton onClick={publish} loading={publishing} icon={<Send className="h-4 w-4" />} dark>
              Publicar
            </TopButton>
          </div>
        </div>
      ...
      <div className="grid min-h-[calc(100vh-64px)] xl:grid-cols-[440px_minmax(0,1fr)]">
        <aside className="border-r border-stone-200 bg-white">
          <div className="grid grid-cols-6 gap-1 border-b border-stone-200 p-3">
            ...
            <PanelButton active={panel === 'store'} onClick={() => setPanel('store')} icon={<ShoppingBag className="h-4 w-4" />} label="Loja" />
            <PanelButton active={panel === 'style'} onClick={() => setPanel('style')} icon={<Palette className="h-4 w-4" />} label="Visual" />
            <PanelButton active={panel === 'content'} onClick={() => setPanel('content')} icon={<Monitor className="h-4 w-4" />} label="Texto" />
            <PanelButton active={panel === 'advanced'} onClick={() => setPanel('advanced')} icon={<Code2 className="h-4 w-4" />} label="Código" />
          </div>
          ...
          <div className="max-h-[calc(100vh-118px)] overflow-y-auto p-5">
            ...
            {panel === 'content' ? (
              <ContentPanel config={config} onChange={change} jobsCount={jobs.length} />
            ) : null}
        ...
        <main className="min-w-0 p-3 sm:p-5 xl:p-7">
          <div className="mb-3 flex items-center justify-between text-[10px] font-black uppercase tracking-[.16em] text-stone-500">
            ...
            <span>{commerce ? `${classifiedsCount} item(ns) na vitrine` : company.name}</span>
          </div>
          <div className="mx-auto h-[calc(100vh-132px)] max-w-[1700px] overflow-auto rounded-[20px] border border-black/10 bg-white shadow-[0_24px_80px_rgba(0,0,0,.15)]">
    ...
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold disabled:opacity-50 ${
        dark
          ...
          : secondary
            ? 'hidden border border-stone-200 bg-white sm:inline-flex'
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <label className="mb-1.5 mt-4 block text-[10px] font-black uppercase tracking-[.12em] text-stone-500">
      {children}
    </label>
  );
}
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className="w-full rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-stone-500"
    />
  );
}
...
function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className={`min-h-28 w-full resize-y rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-stone-500 ${props.className || ''}`}
    />
  );
}
  ...
  return (
    <>
      <Heading
        ...
        text="O modelo muda a arquitetura. Loja, marketplace, catálogo e classificados usam estruturas próprias, assim como as famílias institucionais, gastronômicas, de moda e serviços."
      />
      <div className="space-y-1">
        {COMPANY_PAGE_TEMPLATES.map((template) => (
          <button
            ...
            key={template.key}
            onClick={() => {
              const themed = applyCompanyThemePreset(config, template.key);
              onChange(
                ensureThemeSections(
                  { ...themed, templateKey: template.key },
                  template.key,
                ),
              );
            }}
          ...
          >
            <div className="flex items-start justify-between gap-2">
              ...
              <span className={`mt-1 text-[9px] font-black uppercase tracking-widest ${config.templateKey === template.key ? 'text-stone-300' : 'text-stone-400'}`}>
                {template.eyebrow}
              </span>
            </div>
          </button>
  ...
  return (
    <>
      <Heading
        ...
        text="Segure o ícone de seis pontos e arraste. Tudo abaixo da capa pode mudar de posição. Remover um bloco não apaga os dados originais."
      />
      <div className="space-y-2">
        {sections.map((section: CompanyPageSection) => {
          const meta = SECTION_META[section.type] || {
            label: section.type,
            description: '',
          };
          const required = capabilities.requiredSections.includes(section.type);
          const draggable = !required;
          return (
            <div
              key={section.id}
              draggable={draggable}
              onDragStart={() => draggable && setDraggedId(section.id)}
              ...
              className={`rounded-2xl border p-3 transition ${
                draggedId === section.id
                  ? 'border-stone-900 bg-stone-100 opacity-55'
            ...
            >
              <div className="flex items-start gap-2">
                {draggable ? (
                  <GripVertical className="mt-0.5 h-5 w-5 shrink-0 text-stone-300" />
                ) : (
                  <span className="mt-1 h-4 w-5 shrink-0" />
                )}
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold">
                    {meta.label}
                    {required ? (
                      <span className="ml-2 text-[8px] uppercase tracking-widest text-stone-400">
                        fixa no topo
                      </span>
                    ) : null}
                  </p>
              ...
              {!required ? (
                <Toggle
                  ...
                  label={section.enabled === false ? 'Oculta' : 'Visível'}
                />
              ) : null}
              ...
              {section.type !== 'identity' && themeSupports(config.templateKey, 'sectionSizing') ? (
                <div className="grid grid-cols-2 gap-2">
                  <Select
                    value={section.width || config.width || 'wide'}
                  ...
                  <Select
                    value={section.contentMode || 'section'}
                    onChange={(event) => update(section.id, { contentMode: event.target.value })}
                  >
                    <option value="section">Bloco</option>
        ...
        {draggedId ? (
          <div
            ...
            onDrop={dropAtEnd}
            className="rounded-2xl border-2 border-dashed border-stone-300 px-4 py-5 text-center text-[10px] font-black uppercase tracking-widest text-stone-400"
          >
            Soltar aqui para mover ao final
          </div>
        ) : null}
      </div>
      ...
      {missing.length ? (
        <div className="mt-5 rounded-2xl border border-dashed border-stone-300 p-3">
          <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-stone-400">
            Adicionar seção
          </p>
      ...
      <div className="mt-5 border-t border-stone-100 pt-4">
        <Toggle
          checked={config.navigation?.enabled !== false}
          onChange={(enabled) => onChange(merge(config, 'navigation', { enabled }))}
          label="Menu de navegação"
        />
        <Toggle
          checked={config.navigation?.sticky !== false}
          onChange={(sticky) => onChange(merge(config, 'navigation', { sticky }))}
          label="Menu fixo ao rolar"
        />
      </div>
    </>
  );
...
function StorePanel({
  ...
  const defaults = storeDefaults(config.templateKey);

  const updateCategory = (
    index: number,
    values: Partial<CompanyPageCategoryLink>,
  ) => {
    onChange(
      merge(config, 'categories', {
    ...
    return (
      <>
        <Heading
          ...
          text="Essas opções aparecem nos modelos comerciais."
        />
        <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 p-5 text-sm leading-6 text-stone-500">
          ...
          Escolha Loja, Marketplace, Catálogo, Mercado, Gazeta, Mosaico,
          Radar, Pregão ou Classificados Pro para editar banner, categorias e
          vitrine.
        </div>
      </>
    );
  ...
  return (
    <>
      <Heading
        title="Loja e classificados"
        text="Controle a experiência comercial. Os produtos e serviços vêm automaticamente dos Classificados da empresa."
      />
      <div className="rounded-2xl bg-emerald-50 p-3 text-xs font-bold text-emerald-800">
        {classifiedsCount} produto(s)/serviço(s) disponíveis para a seção de vitrine.
      </div>
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'storefront', { promoText: event.target.value }))
        }
      />
      ...
      <Input
        ...
        placeholder="Ex.: 5% no Pix"
        onChange={(event) =>
          onChange(
            merge(config, 'storefront', {
              secondaryPromoText: event.target.value,
            }),
          )
        }
      />
      ...
      <Toggle
        checked={config.storefront?.showSearch ?? defaults.showSearch}
        onChange={(showSearch) =>
          onChange(merge(config, 'storefront', { showSearch }))
        }
        label="Mostrar busca na loja"
      />
      ...
      <Select
        value={config.storefront?.bannerStyle || defaults.bannerStyle}
        onChange={(event) =>
          onChange(
            merge(config, 'storefront', { bannerStyle: event.target.value }),
          )
        }
      >
        <option value="full">Campanha tela cheia</option>
      ...
      <Select
        value={config.storefront?.categoryStyle || defaults.categoryStyle}
        onChange={(event) =>
          onChange(
            merge(config, 'storefront', { categoryStyle: event.target.value }),
          )
        }
      >
        <option value="image-tiles">Cards com imagem</option>
        <option value="circles">Círculos com imagem</option>
        <option value="tiles">Cards sem imagem</option>
        <option value="chips">Chips / texto</option>
      </Select>

      <Label>Layout de produtos e serviços</Label>
      <Select
        value={config.storefront?.productsLayout || defaults.productsLayout}
        onChange={(event) =>
          onChange(
            merge(config, 'storefront', { productsLayout: event.target.value }),
          )
        }
      >
      ...
      <Input
        value={config.storefront?.featuredTitle || defaults.featuredTitle}
        onChange={(event) =>
          onChange(
            merge(config, 'storefront', { featuredTitle: event.target.value }),
          )
        }
      />

      <Toggle
        checked={config.storefront?.showProducts !== false}
        onChange={(showProducts) =>
          onChange(merge(config, 'storefront', { showProducts }))
        }
        label="Mostrar produtos"
      />
      <Toggle
        checked={config.storefront?.showServices !== false}
        onChange={(showServices) =>
          onChange(merge(config, 'storefront', { showServices }))
        }
      ...
      <div className="mt-6 border-t border-stone-100 pt-5">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-bold">Categorias personalizadas</p>
            <p className="mt-1 text-[11px] text-stone-500">
              Com ou sem imagem, você escolhe.
            </p>
          </div>
          <button
            type="button"
            onClick={addCategory}
            className="inline-flex items-center gap-1 rounded-full bg-stone-950 px-3 py-2 text-[10px] font-bold text-white"
          >
          ...
          {items.map((item, index) => (
            <div key={item.id || index} className="rounded-2xl border border-stone-200 p-3">
              ...
              <div className="mt-2">
                <Input
                  value={item.imageUrl || ''}
                  placeholder="Imagem opcional da categoria"
                  onChange={(event) =>
                    updateCategory(index, { imageUrl: event.target.value })
                  }
                />
              </div>
              <div className="mt-2">
                <Input
                  value={item.href || ''}
                  placeholder="Destino, ex.: #vitrine"
                  onChange={(event) =>
                    updateCategory(index, { href: event.target.value })
                  }
                />
              </div>
            </div>
  ...
  return (
    <>
      <Heading
        ...
        text="A estrutura vem do modelo. Aqui você adapta a marca sem transformar todos os modelos no mesmo template."
      />
              ...
              <label key={key} className="text-xs text-stone-500">
                ...
                <div className="mt-1 flex items-center gap-2 rounded-xl border border-stone-200 p-2">
                  <input
                    ...
                    onChange={(event) =>
                      onChange(merge(config, 'theme', { [key]: event.target.value }))
                    }
      ...
      <Select
        value={config.branding?.typography || 'clean'}
        onChange={(event) =>
          onChange(
            merge(config, 'branding', { typography: event.target.value }),
          )
        }
      >
      ...
      <Label>Cantos</Label>
      <Select
        value={config.branding?.corners || 'soft'}
        onChange={(event) =>
          onChange(merge(config, 'branding', { corners: event.target.value }))
        }
      >
      ...
      <Select
        value={config.width || 'wide'}
        onChange={(event) =>
          onChange({ ...config, width: event.target.value as any })
        }
      >
        <option value="compact">Compacta</option>
        <option value="standard">Padrão</option>
...
function ContentPanel({
  ...
  return (
    <>
      <Heading
        title="Conteúdo"
        text="Textos, informações institucionais e canais da empresa."
      />

      <Label>Chamada da capa</Label>
      <Input
        value={config.hero?.eyebrow || ''}
        onChange={(event) =>
          onChange(merge(config, 'hero', { eyebrow: event.target.value }))
        }
      />
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'hero', { title: event.target.value }))
        }
      />
      ...
      <Textarea
        ...
        onChange={(event) =>
          onChange(merge(config, 'hero', { subtitle: event.target.value }))
        }
      />
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'about', { title: event.target.value }))
        }
      />
      ...
      <Textarea
        ...
        onChange={(event) =>
          onChange(merge(config, 'about', { text: event.target.value }))
        }
      />
      ...
      <div className="mt-6 rounded-2xl bg-stone-50 p-3">
        ...
        <p className="mt-1 text-[11px] leading-4 text-stone-500">
          Você pode tirar a seção Vagas da página sem apagar nenhuma vaga.
        </p>
      </div>
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'contacts', { phone: event.target.value }))
        }
      />
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'contacts', { whatsapp: event.target.value }))
        }
      />
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'contacts', { email: event.target.value }))
        }
      />
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'contacts', { website: event.target.value }))
        }
      />
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'socials', { instagram: event.target.value }))
        }
      />
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'socials', { linkedin: event.target.value }))
        }
      />
      ...
      <Input
        ...
        onChange={(event) =>
          onChange(merge(config, 'footer', { text: event.target.value }))
        }
      />
    </>
  );
  ...
  return (
    <>
      <Heading
        ...
        text="Preservado para páginas que já usam HTML/CSS/JS próprio."
      />
      <Toggle
        checked={codeMode}
        onChange={(enabled) =>
          onChange({ ...config, editorMode: enabled ? 'code' : 'visual' })
        }
      ...
      {codeMode ? (
        <>
          ...
          <Textarea
            ...
            onChange={(event) =>
              onChange(merge(config, 'codePage', { html: event.target.value }))
            }
            className="font-mono"
          />
          <Label>CSS</Label>
          <Textarea
            ...
            onChange={(event) =>
              onChange(merge(config, 'codePage', { css: event.target.value }))
            }
            className="font-mono"
          />
          ...
          <Textarea
            ...
            onChange={(event) =>
              onChange(merge(config, 'codePage', { js: event.target.value }))
            }
            className="font-mono"
          />
        </>
        ...
        <div className="mt-4 rounded-2xl bg-stone-50 p-4 text-xs leading-5 text-stone-500">
          No modo visual, cada família de tema usa seu próprio renderer. O
          PiraNegócios aparece apenas no rodapé de integração.
        </div>
      )}
    </>
  );
