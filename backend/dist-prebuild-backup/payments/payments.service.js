...
let PaymentsService = class PaymentsService {
    ...
    async listCatalog(includeDisabled = false) {
        ...
        return rows.map((row) => this.presentProduct(row));
    }
    async findProduct(code, includeDisabled = false) {
        const rows = await this.dataSource.query(`SELECT * FROM payment_products WHERE code = $1 ${includeDisabled ? '' : 'AND enabled = true'} LIMIT 1`, [code]);
        if (!rows[0])
    ...
    async updateProduct(code, input) {
        ...
        if (!name)
            throw new common_1.BadRequestException('O produto precisa ter um nome.');
        if (promotionalPriceCents !== null && promotionalPriceCents > priceCents) {
            throw new common_1.BadRequestException('O preço promocional não pode ser maior que o preço normal.');
        }
        if (promotionStartsAt && promotionEndsAt && new Date(String(promotionEndsAt)).getTime() < new Date(String(promotionStartsAt)).getTime()) {
            throw new common_1.BadRequestException('O fim da promoção não pode ser anterior ao início.');
        }
       ...
       WHERE code = $1
       RETURNING *`, [code, name, description, priceCents, promotionalPriceCents, promotionStartsAt, promotionEndsAt, enabled, freeUses]);
        return this.presentProduct(rows[0]);
    }
    async getDevMode() {
        const rows = await this.dataSource.query(`SELECT value FROM settings WHERE key = 'PAYMENTS_DEV_MODE' LIMIT 1`);
        return { enabled: String(rows[0]?.value || 'false') === 'true' };
    }
    async setDevMode(enabled) {
        await this.dataSource.query(`INSERT INTO settings (key, value, description, "isPublic")
       VALUES ('PAYMENTS_DEV_MODE', $1, 'Permite simular aprovação de pagamentos Pix no painel administrativo.', false)
       ON CONFLICT (key) DO UPDATE
       SET value = EXCLUDED.value, description = EXCLUDED.description, "isPublic" = false, "updatedAt" = now()`, [enabled ? 'true' : 'false']);
        return { enabled };
    }
    ...
    async createPixPayment(userId, productCode) {
        ...
        const rows = await this.dataSource.query(`INSERT INTO payments
    ...
    async attachProviderCheckout(paymentId, checkout) {
        const rows = await this.dataSource.query(`UPDATE payments SET
         provider = $2,
         "providerPaymentId" = $3,
         "pixCopyPaste" = $4,
         "qrCodeBase64" = $5,
       ...
       RETURNING *`, [
        ...
        if (!rows[0])
            throw new common_1.NotFoundException('Pagamento pendente não encontrado.');
        return rows[0];
    }
    async cancelProviderCheckout(paymentId, error) {
        const message = error instanceof Error ? error.message : String(error || 'Falha ao criar cobrança no provedor');
        const rows = await this.dataSource.query(`UPDATE payments SET status = 'CANCELED', "canceledAt" = now(), "updatedAt" = now(),
         metadata = coalesce(metadata,'{}'::jsonb) || $2::jsonb
       WHERE id = $1 AND status = 'PENDING' RETURNING *`, [paymentId, JSON.stringify({ providerCheckoutError: message.slice(0, 1000) })]);
        return rows[0] || null;
    }
    async listUserPayments(userId) {
        return this.dataSource.query(`SELECT p.*, pp.name AS "productName", pp.description AS "productDescription"
       FROM payments p
    ...
    async listAllPayments(limit = 200) {
        ...
        return this.dataSource.query(`SELECT p.*, pp.name AS "productName", u.email, u."fullName", u."displayName"
       FROM payments p
       LEFT JOIN payment_products pp ON pp.code = p."productCode"
    ...
    featureForProduct(code) {
        if (code === 'RESUME_REANALYSIS')
            return 'RESUME_REANALYSIS';
        if (code === 'RESUME_AI_IMPROVEMENT')
            return 'RESUME_AI_IMPROVEMENT';
        if (code === 'RESUME_AI_IMPORT')
            return 'RESUME_AI_IMPORT';
        return null;
    }
    async settlePayment(paymentId, metadata, isSimulation) {
        return this.dataSource.transaction(async (manager) => {
            const rows = await manager.query(`SELECT * FROM payments WHERE id = $1 FOR UPDATE`, [paymentId]);
            ...
            if (payment.status !== 'PENDING') {
                throw new common_1.BadRequestException(`Pagamento não pode ser confirmado no status ${payment.status}.`);
            }
            const paidRows = await manager.query(`UPDATE payments
         SET status = 'PAID', "paidAt" = now(), "updatedAt" = now(), "isSimulation" = $3,
             metadata = coalesce(metadata, '{}'::jsonb) || $2::jsonb
         WHERE id = $1 RETURNING *`, [paymentId, JSON.stringify(metadata), isSimulation]);
           ...
           DO UPDATE SET credits = user_feature_credits.credits + 1, "updatedAt" = now()`, [payment.userId, feature]);
            }
            return paidRows[0];
        });
    }
    async activateCompanyPlanTrial(paymentId, input = {}) {
        const rows = await this.dataSource.query(`SELECT * FROM payments WHERE id = $1 LIMIT 1`, [paymentId]);
        const payment = rows[0];
        if (!payment)
            throw new common_1.NotFoundException('Pagamento da assinatura não encontrado.');
        if (!['COMPANY_PLUS_MONTHLY', 'COMPANY_ELITE_MONTHLY'].includes(String(payment.productCode)))
            return null;
        ...
        if (!companyId || trialDays <= 0)
            return null;
        const inserted = await this.dataSource.query(`INSERT INTO company_plan_trials
       ...
       ON CONFLICT ("companyId") DO NOTHING RETURNING *`, [companyId, payment.userId, targetPlan, trialDays, String(input.provider || payment.provider || '') || null, input.providerSubscriptionId || null, payment.id]);
        const trial = inserted[0] || (await this.dataSource.query(`SELECT * FROM company_plan_trials WHERE "companyId" = $1 LIMIT 1`, [companyId]))[0] || null;
        if (trial?.status === 'ACTIVE') {
         ...
         ON CONFLICT ("companyId") DO UPDATE SET eligible = false, "eligibleUntil" = NULL, source = 'ELITE_TRIAL', "updatedAt" = now()`, [companyId]).catch(() => undefined);
            await this.dataSource.query(`UPDATE payments SET metadata = coalesce(metadata,'{}'::jsonb) || $2::jsonb, "updatedAt" = now() WHERE id = $1`, [payment.id, JSON.stringify({ companyEliteTrialPending: false, companyEliteTrialActivated: true, companyEliteTrialStartedAt: trial.startedAt, companyEliteTrialEndsAt: trial.endsAt })]);
        }
        return trial;
    }
    async confirmPayment(paymentId, metadata = {}) {
        return this.settlePayment(paymentId, metadata, false);
    }
    async confirmProviderPayment(paymentId, metadata = {}) {
        return this.settlePayment(paymentId, metadata, false);
    }
    async simulatePayment(paymentId, adminUserId) {
        const devMode = await this.getDevMode();
        if (!devMode.enabled) {
            throw new common_1.BadRequestException('O modo DEV de pagamentos está desativado. Ative-o no painel financeiro para usar simulações.');
        }
        return this.settlePayment(paymentId, {
    ...
    async getCredits(userId) {
        const rows = await this.dataSource.query(`SELECT feature, credits FROM user_feature_credits WHERE "userId" = $1`, [userId]);
        const credits = {
            RESUME_REANALYSIS: 0,
        ...
        for (const row of rows)
            credits[row.feature] = Number(row.credits || 0);
        return credits;
    }
    async hasCredit(userId, feature) {
        const rows = await this.dataSource.query(`SELECT credits FROM user_feature_credits WHERE "userId" = $1 AND feature = $2 LIMIT 1`, [userId, feature]);
        return Number(rows[0]?.credits || 0) > 0;
    }
    async consumeCredit(userId, feature) {
        const rows = await this.dataSource.query(`UPDATE user_feature_credits
       SET credits = credits - 1, "updatedAt" = now()
       WHERE "userId" = $1 AND feature = $2 AND credits > 0
       RETURNING credits`, [userId, feature]);
        if (!rows[0])
            throw new common_1.BadRequestException('Crédito insuficiente para este recurso.');
        return Number(rows[0].credits || 0);
    }
    async grantCredit(userId, feature, quantity = 1) {
        const safeQuantity = Math.min(100, Math.max(1, Math.round(quantity)));
        await this.dataSource.query(`INSERT INTO user_feature_credits ("userId", feature, credits)
       VALUES ($1, $2, $3)
       ON CONFLICT ("userId", feature)
       DO UPDATE SET credits = user_feature_credits.credits + $3, "updatedAt" = now()`, [userId, feature, safeQuantity]);
    ...
    async recordAnalysis(userId, profile, analysis, source, paymentId) {
        const score = Math.max(0, Math.min(100, Math.round(Number(analysis?.score) || 0)));
        const rows = await this.dataSource.query(`INSERT INTO resume_analysis_history
        ("userId", source, score, analysis, "resumeSnapshot", "paymentId")
       VALUES ($1, $2, $3, $4::jsonb, $5::jsonb, $6)
       RETURNING *`, [userId, source, score, JSON.stringify(analysis), JSON.stringify(this.resumeSnapshot(profile)), paymentId || null]);
        return rows[0];
    }
    async listAnalysisHistory(userId) {
        return this.dataSource.query(`SELECT id, source, score, analysis, "createdAt" FROM resume_analysis_history
       WHERE "userId" = $1 ORDER BY "createdAt" DESC`, [userId]);
    }
    async createImprovementProposal(userId, profile, proposal, paymentId) {
        const rows = await this.dataSource.query(`INSERT INTO resume_improvement_proposals
        ("userId", "beforeSnapshot", proposal, "paymentId")
       VALUES ($1, $2::jsonb, $3::jsonb, $4)
       RETURNING *`, [userId, JSON.stringify(this.resumeSnapshot(profile)), JSON.stringify(proposal), paymentId || null]);
        return rows[0];
    }
    async getImprovementProposal(userId, id) {
        const rows = await this.dataSource.query(`SELECT * FROM resume_improvement_proposals WHERE id = $1 AND "userId" = $2 LIMIT 1`, [id, userId]);
        if (!rows[0])
            throw new common_1.NotFoundException('Proposta de melhoria não encontrada.');
        return rows[0];
    }
    async completeImprovementProposal(userId, id, selectedChangeIds, partial) {
        const rows = await this.dataSource.query(`UPDATE resume_improvement_proposals
       SET status = $3, "selectedChangeIds" = $4::jsonb, "appliedAt" = now(), "updatedAt" = now()
       WHERE id = $1 AND "userId" = $2
       RETURNING *`, [id, userId, partial ? 'PARTIAL' : 'APPLIED', JSON.stringify(selectedChangeIds)]);
        if (!rows[0])
            throw new common_1.NotFoundException('Proposta de melhoria não encontrada.');
        return rows[0];
    }
    async listImprovementHistory(userId) {
        return this.dataSource.query(`SELECT id, status, proposal, "selectedChangeIds", "appliedAt", "createdAt"
       FROM resume_improvement_proposals WHERE "userId" = $1 ORDER BY "createdAt" DESC`, [userId]);
    }
    async listPublicationHistory(userId) {
        return this.dataSource.query(`SELECT id, version, snapshot, score, status, "publishedAt", "unpublishedAt"
       FROM resume_publication_history WHERE "userId" = $1 ORDER BY version DESC`, [userId]);
    }
};
