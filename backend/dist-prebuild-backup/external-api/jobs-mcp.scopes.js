...
exports.DEFAULT_JOBS_MCP_SCOPES = exports.DEFAULT_JOBS_V2_SCOPES = exports.JOBS_MCP_SCOPES = exports.ALL_JOBS_SCOPES = exports.JOBS_CAPABILITIES = void 0;
exports.sanitizeJobsScopes = sanitizeJobsScopes;
exports.expandLegacyJobsScopes = expandLegacyJobsScopes;
exports.hasJobsScope = hasJobsScope;
exports.JOBS_CAPABILITIES = [
    {
        scope: 'jobs:read',
...
exports.ALL_JOBS_SCOPES = exports.JOBS_CAPABILITIES.map((capability) => capability.scope);
exports.JOBS_MCP_SCOPES = exports.JOBS_CAPABILITIES
    .filter((capability) => capability.channels.includes('mcp'))
    .map((capability) => capability.scope);
exports.DEFAULT_JOBS_V2_SCOPES = exports.JOBS_CAPABILITIES
    .filter((capability) => capability.defaultV2)
    .map((capability) => capability.scope);
exports.DEFAULT_JOBS_MCP_SCOPES = exports.JOBS_CAPABILITIES
    .filter((capability) => capability.defaultMcp)
    .map((capability) => capability.scope);
function sanitizeJobsScopes(value, fallback = exports.DEFAULT_JOBS_MCP_SCOPES) {
    if (!Array.isArray(value))
        return [...fallback];
    const allowed = new Set(exports.ALL_JOBS_SCOPES);
    return Array.from(new Set(value.map(String).filter((scope) => allowed.has(scope))));
}
const LEGACY_SCOPE_EQUIVALENTS = {
...
function expandLegacyJobsScopes(scopes) {
    const values = scopes instanceof Set ? new Set(scopes) : new Set(scopes);
    const expanded = new Set();
    for (const scope of values) {
        if (scope === 'jobs:read' || scope === 'jobs:write')
            continue;
    ...
    const hasExplicitGranularJobsScope = Array.from(values).some((value) => value.startsWith('jobs:') && !['jobs:read', 'jobs:write'].includes(value));
    if (!hasExplicitGranularJobsScope) {
        if (values.has('jobs:read'))
            LEGACY_READ_EQUIVALENTS.forEach((scope) => expanded.add(scope));
        if (values.has('jobs:write'))
            LEGACY_WRITE_EQUIVALENTS.forEach((scope) => expanded.add(scope));
    }
    return Array.from(expanded);
}
function hasJobsScope(scopes, scope) {
    const values = scopes instanceof Set ? scopes : new Set(scopes);
    if (values.has(scope))
        return true;
    if (scope === 'jobs:read' || scope === 'jobs:write')
        return values.has(scope);
    for (const [legacyScope, equivalents] of Object.entries(LEGACY_SCOPE_EQUIVALENTS)) {
        if (values.has(legacyScope) && equivalents.includes(scope))
            return true;
    }
    const hasGranularScope = Array.from(values).some((value) => value.startsWith('jobs:') && !['jobs:read', 'jobs:write'].includes(value));
    if (hasGranularScope)
        return false;
    if (values.has('jobs:read') && LEGACY_READ_EQUIVALENTS.has(scope))
        return true;
    if (values.has('jobs:write') && LEGACY_WRITE_EQUIVALENTS.has(scope))
        return true;
    return false;
}
//# sourceMappingURL=jobs-mcp.scopes.js.map