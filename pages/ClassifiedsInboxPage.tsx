...
export default function ClassifiedsInboxPage() {
    ...
    user.getIdToken().then((token) => {
      ...
      socket.on('chat:message', (message: ClassifiedConversationMessage) => {
        ...
        setConversations((current) => current.map((conversation) => {
          ...
          const incomingForIdentity = message.senderRole !== conversation.role;
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((current) => current.some((item) => item.id === message.id) ? current : [...current, message]);
      setConversations((current) => current.map((conversation) => conversation.id === conversationId ? { ...conversation, lastMessage: { id: message.id, senderId: message.senderId, senderRole: message.senderRole, body: message.body, createdAt: message.createdAt }, lastMessageAt: message.createdAt, unreadCount: 0 } : conversation));
      setBody('');
    } catch (requestError: any) {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="mb-5"><p className="text-[10px] font-black uppercase tracking-[.18em] text-stone-400">Negociação em tempo real</p><h1 className="mt-1 font-serif text-3xl font-black">Conversas</h1><p className="mt-2 text-sm text-stone-500">Cada conversa fica vinculada ao anúncio e à identidade Personal ou Business usada na negociação.</p></div>
...
function ConversationRow({ conversation, active, onClick }: { conversation: ClassifiedConversation; active: boolean; onClick: () => void }) {
  ...
  return <button onClick={onClick} className={`flex w-full gap-3 border-b border-stone-100 p-4 text-left transition ${active ? 'bg-stone-100' : 'hover:bg-stone-50'}`}><div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-2xl bg-stone-100">{conversation.listing?.image ? <img src={conversation.listing.image} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center"><ShoppingBag className="h-5 w-5 text-stone-300" /></div>}{conversation.unreadCount > 0 && <span className="absolute right-1 top-1 flex min-h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-black text-white">{Math.min(99, conversation.unreadCount)}</span>}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-1"><p className="truncate text-xs font-black text-stone-900">{counterpart?.name || 'Negociação'}</p>{counterpart?.verified && <BadgeCheck className="h-3.5 w-3.5 shrink-0 text-emerald-600" />}</div><p className="mt-1 truncate text-xs font-bold text-stone-600">{conversation.listing?.title || 'Anúncio'}</p><p className={`mt-1 truncate text-[11px] ${conversation.unreadCount ? 'font-bold text-stone-800' : 'text-stone-400'}`}>{conversation.lastMessage?.body || 'Conversa iniciada'}</p></div></button>;
}

function ConversationHeader({ conversation, onBack }: { conversation: ClassifiedConversation; onBack: () => void }) {
  const counterpart = conversation.role === 'BUYER' ? conversation.seller : conversation.buyer;
  return <header className="flex items-center gap-3 border-b border-stone-100 bg-white p-4"><button onClick={onBack} className="rounded-xl bg-stone-100 px-3 py-2 text-xs font-black md:hidden">Voltar</button><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><p className="truncate text-sm font-black">{counterpart?.name}</p>{counterpart?.verified && <BadgeCheck className="h-4 w-4 text-emerald-600" />}</div><Link to={conversation.listing ? `/classificados/anuncio/${conversation.listing.slug}` : '/classificados'} className="mt-0.5 block truncate text-xs text-stone-500 hover:underline">{conversation.listing?.title}</Link></div>{conversation.listing?.status === 'SOLD' && <span className="rounded-full bg-blue-50 px-3 py-1 text-[9px] font-black uppercase text-blue-700">Vendido</span>}</header>;
}

function MessageBubble({ message, ownSide, ownUser }: { message: ClassifiedConversationMessage; ownSide: boolean; ownUser: boolean }) { return <div className={`max-w-[86%] ${ownSide ? 'ml-auto' : ''}`}><div className={`rounded-[20px] px-4 py-3 text-sm leading-6 shadow-sm ${ownSide ? 'rounded-br-md bg-stone-900 text-white' : 'rounded-bl-md border border-stone-200 bg-white text-stone-800'}`}><p className={`mb-1 text-[10px] font-black ${ownSide ? 'text-white/60' : 'text-stone-400'}`}>{message.senderName}{ownSide && !ownUser ? ' · equipe' : ''}</p><p className="whitespace-pre-wrap break-words">{message.body}</p></div><p className={`mt-1 px-1 text-[9px] text-stone-400 ${ownSide ? 'text-right' : ''}`}>{new Date(message.createdAt).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</p></div>; }
