      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <Section number="04" title="Candidatura" description="Defina por onde o candidato entra e se um arquivo de currículo é realmente necessário.">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <ToggleCard active={!acceptsPlatformApplications} disabled={isInternal} onClick={() => { setAcceptsPlatformApplications(false); setRequiresResumeFile(false); }} title="Candidatura externa" description={isInternal ? "Indisponível: vagas internas recebem candidaturas obrigatoriamente pelo PiraNegócios." : "Direcione para outro canal ou processo."} />
          </div>

          {isInternal && <p className="mt-3 rounded-xl border border-violet-200 bg-violet-50 px-4 py-3 text-xs font-semibold leading-5 text-violet-800">Como esta vaga é interna, a candidatura pela plataforma é obrigatória e não pode ser desabilitada.</p>}
...
function ToggleCard({ active, disabled = false, onClick, title, description }: { active: boolean; disabled?: boolean; onClick: () => void; title: string; description: string }) { return <button type="button" disabled={disabled} onClick={onClick} className={`rounded-2xl border p-4 text-left transition ${active ? "border-stone-950 bg-stone-950 text-white" : "border-stone-200 bg-white text-stone-900"} ${disabled ? "cursor-not-allowed opacity-50" : ""}`}><div className="flex items-center justify-between"><strong className="text-sm">{title}</strong>{active && <Check className="h-4 w-4" />}</div><p className={`mt-1 text-xs leading-5 ${active ? "text-white/55" : "text-stone-500"}`}>{description}</p></button>; }
