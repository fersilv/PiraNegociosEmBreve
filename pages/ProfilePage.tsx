...
export function ProfilePage() {
  ...
  const [eduStatus, setEduStatus] = useState<
    "CONCLUIDO" | "EM_ANDAMENTO" | "TRANCADO" | "INTERROMPIDO"
  >("CONCLUIDO");
  ...
  const handleUpdateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile?.type === "CANDIDATE") {
        ...
        updates.isOpenToWork = isOpenToWork;
  ...
  const editExperience = (index: number) => {
    ...
    setExpEndDate(
      exp.current ? "" : exp.endDate === "Não informada" ? "" : exp.endDate,
    );
  ...
  const editEducation = (index: number) => {
    ...
    setEduEndYear(
      edu.status === "EM_ANDAMENTO"
        ? ""
        : edu.endYear === "Não informado"
          ? ""
          : edu.endYear,
    );
  ...
  const addEducation = () => {
    ...
    const newEdu: AcademicEducation = {
      ...
      endYear:
        eduStatus === "EM_ANDAMENTO"
          ? "Em andamento"
          : eduEndYear.trim() || "Não informado",
  ...
  const editCourse = (index: number) => {
    ...
    setCourseInstitution(
      course.institution === "Não informada" ? "" : course.institution || "",
    );
  ...
  const runAiResumeAnalysis = async () => {
    ...
    if (!candidateResumeUrl) {
      alert(
        "Faça o upload do seu currículo primeiro para que a IA possa analisá-lo.",
      );
      return;
    }
    ...
    const userLimit =
      profile?.aiAnalysisLimit !== undefined
        ? Number(profile.aiAnalysisLimit)
        : globalLimit;
    ...
    if (userLimit >= 0 && currentCount >= userLimit) {
      alert(
        `Você atingiu o limite de uso gratuito da assistente de IA (${currentCount} de ${userLimit} análises). Entre em contato com o administrador para aumentar o seu limite.`,
      );
      setAiLoading(false);
      return;
    }
  ...
  const applyAiResult = () => {
    ...
    if (aiResult.additionalPhones)
      setAdditionalPhones(aiResult.additionalPhones);
    ...
    setAiResult(null);
    alert(
      'Dados da IA aplicados! Não se esqueça de clicar em "Salvar Meu Perfil Completo" ao final da página para gravar os dados permanentemente.',
    );
  };
      ...
      <div className="bg-white rounded-3xl p-6 md:p-8 border border-stone-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-1">
          ...
          <p className="text-stone-500 text-sm max-w-xl">
            Mantenha suas informações de contato, currículo profissional,
            competências e escolaridade atualizados.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-lg font-serif font-bold text-white flex items-center gap-2">
                ...
                <span className="text-[10px] bg-terracotta-600 text-white px-2 py-0.5 rounded-full font-sans uppercase font-bold tracking-wider">
                  Mágica da IA
                </span>
              </h3>
              <p className="text-sm text-stone-300 mt-1">
                Fazer o preenchimento manual de experiências e cursos é coisa do
                passado. Use nossa Inteligência Artificial para ler seu
                documento de currículo, preencher todo o seu perfil de forma
                automática e ainda indicar sugestões de melhorias no seu
                currículo!
              </p>
            </div>
          </div>
          ...
          {!candidateResumeUrl ? (
            <div className="bg-stone-800/80 border border-stone-700 rounded-2xl p-4 flex gap-3 text-sm text-stone-300">
              ...
              <div>
                ...
                Por favor, faça o upload do seu currículo em arquivo no
                formulário abaixo. Uma vez anexado, você poderá usar a
                inteligência artificial para ler e organizar seu perfil
                automaticamente.
              </div>
            </div>
          ) : (
                ...
                <div className="bg-stone-800/50 border border-stone-700 rounded-2xl p-8 text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-stone-400">
                      Extraindo dados profissionais. Isso levará cerca de 20
                      segundos...
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-6 bg-stone-800/30 border border-stone-700/50 p-6 rounded-2xl">
                  ...
                  <div className="bg-terracotta-600/20 border border-terracotta-500/30 rounded-xl p-4 text-sm text-stone-200">
                    ...
                    <p className="text-xs text-stone-300 mt-1">
                      A IA analisou seu currículo profissional. Veja a análise
                      rápida abaixo e clique em{" "}
                      <strong>"Aplicar dados ao meu Perfil"</strong> para
                      atualizar automaticamente todas as seções abaixo de uma
                      vez só!
                    </p>
                  </div>
                  ...
                  <div className="space-y-4 text-stone-200">
                    ...
                    <div>
                      <span className="text-xs font-bold text-terracotta-400 uppercase tracking-wider block mb-2">
                        Sugestões de Otimização e Melhorias:
                      </span>
                      <ul className="space-y-2">
                        {aiResult.aiAnalysis?.suggestions?.map(
                      ...
                      <div className="space-y-3.5 text-sm">
                        <div>
                          <span className="text-xs font-bold text-stone-400 uppercase">
                            Feedback Geral do Especialista:
                          </span>
                        ...
                        <div>
                          <span className="text-xs font-bold text-stone-400 uppercase block mb-1.5">
      ...
      <div className="bg-white rounded-3xl p-6 md:p-8 border border-stone-200 shadow-sm">
        <form onSubmit={handleUpdateUser} className="space-y-8">
          ...
          {isCandidate && (
            <div className="space-y-8 pt-6 border-t border-stone-100">
              ...
              <div className="space-y-4">
                ...
                <label className="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
                  <input
                    type="checkbox"
                    checked={isOpenToWork}
                    onChange={(event) => setIsOpenToWork(event.target.checked)}
                    className="mt-0.5 h-4 w-4"
                  />
                  <span>
                    <strong>Buscando emprego</strong>
                    <small className="mt-1 block text-emerald-800">
                      Inclui seu perfil no banco de talentos para empresas
                      verificadas. Você pode desativar quando quiser; currículo
                      continua opcional até o momento de se candidatar.
                    </small>
                  </span>
                </label>
                ...
                <div>
                  <label className="block text-xs font-bold text-stone-500 uppercase tracking-widest mb-2">
                    Link do perfil do LinkedIn (Opcional)
                  </label>
                  <input
                    type="url"
                    value={linkedinURL}
                ...
                {additionalPhones.length > 0 && (
                  <div className="flex flex-wrap gap-2.5">
                    {additionalPhones.map((phoneItem, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-1.5 px-3.5 py-2 bg-stone-100 text-stone-700 text-xs font-bold rounded-xl border border-stone-200"
                      >
                        <span>{phoneItem}</span>
                        <button
                          type="button"
                ...
                {skills.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {skills.map((skillItem, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-1 px-3 py-1.5 bg-terracotta-50 text-terracotta-800 text-xs font-bold rounded-lg border border-terracotta-100/70"
                      >
                        <span>{skillItem}</span>
                        <button
                          type="button"
                  ...
                  <p className="text-xs text-stone-400 italic">
                    Nenhuma competência registrada ainda. Adicione abaixo ou
                    extraia do currículo usando IA.
                  </p>
                )}
                ...
                {experiences.length > 0 ? (
                  <div className="space-y-4">
                    {experiences.map((expItem, idx) => (
                      <div
                        key={idx}
                        className="bg-stone-50 border border-stone-200/80 p-5 rounded-2xl relative group"
                      >
                        ...
                        <p className="text-[10px] text-stone-400 font-mono mt-1 font-medium">
                        ...
                        {expItem.skills && expItem.skills.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-3">
                            {expItem.skills.map((sk, sIdx) => (
                              <span
                                key={sIdx}
                                className="text-[9px] bg-stone-100 border border-stone-200/50 text-stone-600 px-2 py-0.5 rounded-md font-medium"
                              >
                                {sk}
                              </span>
                            ))}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={
                            expCurrent ? "Trabalho Atual" : "Ex: 12/2023"
                          }
                      ...
                      <div className="col-span-full flex items-center gap-2">
                        ...
                        <label
                          htmlFor="expCurrent"
                          className="text-xs text-stone-600 select-none"
                        >
                          Atualmente trabalho nesta função
                        </label>
                      </div>
                      <div className="col-span-full">
                        <label className="block text-[10px] font-bold text-stone-500 uppercase tracking-widest mb-1.5">
                      ...
                      <div className="col-span-full">
                        <label className="block text-[10px] font-bold text-stone-500 uppercase tracking-widest mb-1.5">
                          Habilidades Aplicadas nesta Experiência (Opcional)
                        </label>
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          {expSkills.map((sk, skIdx) => (
                            <span
                              key={skIdx}
                              className="bg-terracotta-50 text-terracotta-800 text-[10px] font-bold px-2 py-0.5 rounded-md border border-terracotta-100 flex items-center gap-1"
                            >
                              {sk}
                              <button
                                type="button"
                  ...
                  <div className="space-y-4">
                    {education.map((eduItem, idx) => (
                      ...
                      >
                        ...
                        <h4 className="font-serif font-bold text-stone-950 text-sm">
                          {eduItem.degree} em {eduItem.fieldOfStudy}
                        </h4>
                        <p className="text-xs text-stone-600 font-bold mt-0.5">
                          {eduItem.institution}
                        </p>
                        ...
                        {eduItem.status && (
                          <span className="inline-block mt-2.5 text-[9px] uppercase font-bold tracking-wider px-2 py-0.5 bg-stone-100 text-stone-600 rounded">
                            ...
                            {eduItem.status === "EM_ANDAMENTO" &&
                              "Em andamento"}
                            {eduItem.status === "TRANCADO" && "Trancado"}
                            {eduItem.status === "INTERROMPIDO" &&
                              "Interrompido"}
                          </span>
                        )}
                      </div>
              ...
              <div className="space-y-4 pt-4">
                ...
                {courses.length > 0 ? (
                  ...
                  <p className="text-xs text-stone-400 italic">
                    Nenhum curso ou certificação registrado.
                  </p>
                )}
          ...
          <div className="flex justify-end pt-5 border-t border-stone-100">
            ...
            >
              {loading
                ? "Salvando Perfil Completo..."
                : "Salvar Meu Perfil Completo"}
            </button>
          </div>
        </form>
      </div>
