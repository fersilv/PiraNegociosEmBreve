...
@Injectable()
export class ClassifiedsDeliveryAwareCheckoutService {
  constructor(
  ...
  async createPayment(uid: string, listingId: string, body: Record<string, any>) {
    ...
    const prepared = await this.dataSource.transaction(async (manager) => {
      ...
      if (!this.fulfillmentModes(listing).includes('DELIVERY')) throw new BadRequestException('Entrega não está habilitada para este produto.');

      const addressRows = await manager.query(
      ...
      const originRows = await manager.query(
      ...
      if (unitPrice == null || !Number.isFinite(Number(unitPrice)) || Number(unitPrice) <= 0) throw new BadRequestException('Este produto está sem preço válido para a forma de pagamento selecionada.');
      ...
      const addressSnapshot = this.addressSnapshot(address);
      const originSnapshot = this.addressSnapshot(origin);
      const quoteSnapshot = {
        id: quote.id,
      ...
      const financialSnapshot = {
        currency: 'BRL', itemSubtotalCents, shippingCents, buyerFeeCents, totalCents,
        platformFeeCents, applicationFeeCents, sellerNetCents, deliveryPartnerPayableCents,
      ...
      const orderRows = await manager.query(
        ...
        [
          ...
          itemSubtotalCents, shippingCents, buyerFeeCents, applicationFeeCents, deliveryPartnerPayableCents,
          JSON.stringify(financialSnapshot), JSON.stringify(quoteSnapshot), JSON.stringify(addressSnapshot), JSON.stringify(originSnapshot),
        ],
      );
      ...
      await manager.query(`UPDATE delivery_quotes SET status='SELECTED' WHERE id=$1 AND status IN ('QUOTED','SELECTED')`, [quote.id]);
      await manager.query(
        `INSERT INTO classified_order_events("orderId",type,"toStatus","actorUserId",metadata) VALUES ($1,'CHECKOUT_CREATED','CREATED',$2,$3::jsonb)`,
    ...
    let providerResult: { ok: boolean; status: number; data: any };
    try {
      ...
      return this.orderById(prepared.order.id, { processing: true, message: 'O Mercado Pago ainda está processando a criação do pagamento. Não tente pagar novamente.' });
    }
    ...
    if (!providerResult.ok) {
      if (providerResult.status >= 500) {
        ...
        return this.orderById(prepared.order.id, { processing: true, message: 'O provedor está processando a solicitação. Não tente pagar duas vezes.' });
      }
  ...
  private async applyInitialProviderPayment(orderId: string, payment: any) {
    const rows = await this.dataSource.query(`SELECT * FROM classified_orders WHERE id=$1 LIMIT 1`, [orderId]);
    const before = rows[0];
    if (!before) return;
    ...
    if (mapped === 'APPROVED') {
      await this.dataSource.query(`UPDATE classified_orders SET "stockReserved"=false WHERE id=$1`, [orderId]);
      await this.dataSource.query(`UPDATE classified_order_items SET "stockReserved"=false WHERE "orderId"=$1`, [orderId]).catch(() => undefined);
    }
    const pix = payment?.point_of_interaction?.transaction_data || {};
    const metadata = { mercadoPago: { status: payment?.status || null, statusDetail: payment?.status_detail || null, paymentTypeId: payment?.payment_type_id || null, paymentMethodId: payment?.payment_method_id || null, pixCopyPaste: pix.qr_code || null, pixQrCodeBase64: pix.qr_code_base64 || null, ticketUrl: pix.ticket_url || null } };
    const nextOrderStatus = mapped === 'APPROVED' ? 'PAID' : before.status;
    await this.dataSource.query(
      `UPDATE classified_orders SET "providerPaymentId"=COALESCE($2,"providerPaymentId"),"paymentStatus"=$3,status=$4,
       "providerStatusDetail"=$5,"paidAt"=CASE WHEN $3='APPROVED' THEN COALESCE("paidAt",now()) ELSE "paidAt" END,
       metadata=COALESCE(metadata,'{}'::jsonb) || $6::jsonb,"updatedAt"=now() WHERE id=$1`,
    ...
    await this.dataSource.query(
      `INSERT INTO classified_order_events("orderId",type,"fromStatus","toStatus",metadata) VALUES ($1,'PAYMENT_RESPONSE',$2,$3,$4::jsonb)`,
  ...
  private async releaseStock(orderId: string, reason: string) {
    await this.dataSource.transaction(async (manager) => {
      ...
      const listingRows = await manager.query(`SELECT * FROM classified_listings WHERE id=$1 FOR UPDATE`, [order.listingId]);
      const listing = listingRows[0];
      const stock = this.stockQuantity(listing?.commerceConfig);
      ...
      await manager.query(`INSERT INTO classified_order_events("orderId",type,metadata) VALUES ($1,'STOCK_RELEASED',$2::jsonb)`, [orderId, JSON.stringify({ reason })]).catch(() => undefined);
    });
  }
  ...
  private assertQuoteItems(quote: any, listingId: string, quantity: number) {
    ...
    if (items.length !== 1 || String(items[0]?.listingId || '') !== listingId || Number(items[0]?.quantity || 0) !== quantity) throw new BadRequestException('A quantidade mudou depois do cálculo do frete. Calcule novamente.');
  }
  ...
  private addressText(row: any) { return `${row.street || ''}, ${row.number || ''}${row.complement ? ` · ${row.complement}` : ''} · ${row.neighborhood || ''} · ${row.city || ''}/${row.state || ''} · ${row.zipCode || ''}`.slice(0, 500); }
  private paymentMethod(value: unknown): CheckoutMethod { const method = String(value || '').trim().toUpperCase(); if (!['PIX','CARD'].includes(method)) throw new BadRequestException('Forma de pagamento inválida.'); return method as CheckoutMethod; }
  private quantity(value: unknown) { const number = Math.floor(Number(value || 1)); if (!Number.isFinite(number) || number < 1 || number > 50) throw new BadRequestException('Quantidade inválida.'); return number; }
  ...
  private uuid(value: unknown, message: string) { const id = String(value || '').trim(); if (!/^[0-9a-f-]{36}$/i.test(id)) throw new BadRequestException(message); return id; }
  private toCents(value: unknown) { const number = Number(value); if (!Number.isFinite(number) || number <= 0) throw new BadRequestException('Preço inválido para checkout.'); return Math.round(number * 100); }
  private mapPaymentStatus(value: unknown) { const status = String(value || '').toLowerCase(); if (status === 'approved') return 'APPROVED'; if (status === 'rejected') return 'REJECTED'; if (status === 'cancelled' || status === 'canceled') return 'CANCELED'; if (status === 'refunded') return 'REFUNDED'; if (status === 'in_process' || status === 'in_mediation' || status === 'authorized') return 'IN_PROCESS'; return 'PENDING'; }
  private providerError(data: any) { const cause = Array.isArray(data?.cause) ? data.cause[0] : null; return String(cause?.description || data?.message || data?.error || 'O Mercado Pago recusou o pagamento.').slice(0, 500); }
  private async platformMercadoPagoConfig() { return this.providerConfig.getSecretConfig<MercadoPagoProviderConfig>('MERCADO_PAGO'); }
}
