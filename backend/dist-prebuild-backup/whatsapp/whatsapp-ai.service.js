...
let WhatsAppAiService = class WhatsAppAiService {
    settings;
...
- Não confunda imagem comum com foto de currículo ou documento.`;
    ...
    async answerAdmin(runtime, input) {
...
Fale em português brasileiro, de forma natural, direta e informativa. Não use JSON.`;
    ...
    async explainOperationalError(input) {
        const runtime = await this.runtime();
    ...
    async runtime() {
        ...
        const provider = String(await this.settings.getValue('AI_PROVIDER') || '');
        const model = String(await this.settings.getValue('AI_MODEL') || '').trim();
        if (!enabled || !['GEMINI', 'OPENAI', 'GROQ'].includes(provider) || !model) {
            throw new common_1.ServiceUnavailableException('A inteligência artificial está desabilitada ou sem modelo configurado.');
        }
    ...
    async generate(runtime, prompt, system, json, maxTokens) {
        if (runtime.provider === 'OPENAI') {
            const client = new openai_1.default({ apiKey: runtime.apiKey });
        ...
        if (runtime.provider === 'GROQ') {
            const client = new groq_anthropic_compat_1.GroqCompat({ apiKey: runtime.apiKey });
...
exports.WhatsAppAiService = WhatsAppAiService = __decorate([
