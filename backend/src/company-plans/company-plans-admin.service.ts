...
@Injectable()
export class CompanyPlansAdminService {
  constructor(
  ...
  async get(companyId: string) {
    ...
    const [current, subscriptionRows, trialRows, productRows] = await Promise.all([
      ...
      this.dataSource.query(
        `SELECT * FROM company_plan_subscriptions WHERE "companyId" = $1 LIMIT 1`,
        [companyId],
      ).catch(() => []),
      this.dataSource.query(
      ...
      this.dataSource.query(
        `SELECT code, name, "priceCents", "promotionalPriceCents", "promotionStartsAt", "promotionEndsAt", "durationDays", enabled
  ...
  async set(
    ...
    // An explicit admin choice always wins over a temporary trial.
    await this.dataSource.query(
      `UPDATE company_plan_trials SET status = 'EXPIRED', "endsAt" = LEAST("endsAt", now()), "updatedAt" = now()
      ...
      await this.dataSource.query(
        `UPDATE company_plan_subscriptions
         SET status = 'EXPIRED', "currentPeriodEnd" = LEAST("currentPeriodEnd", now()),
             ...
             "updatedAt" = now()
         WHERE "companyId" = $1`,
        [companyId, JSON.stringify({ adminOverride: true, plan: 'FREE', grantedBy: adminUserId, changedAt: new Date().toISOString() })],
      ).catch(() => undefined);
    ...
    const products = await this.dataSource.query(
      `SELECT code, COALESCE("durationDays", 30)::int AS "durationDays" FROM payment_products WHERE code = $1 LIMIT 1`,
      [productCode],
    );
    if (!products[0]) throw new BadRequestException(`O produto financeiro do plano ${plan} não está configurado.`);
    ...
    const payerRows = await this.dataSource.query(
      `SELECT id FROM users
       ...
       LIMIT 1`,
      [company.ownerId || '', companyId],
    );
    const payerUserId = payerRows[0]?.id;
    if (!payerUserId) {
      throw new BadRequestException('A empresa precisa ter um proprietário ou administrador vinculado antes de receber um plano.');
    }
    ...
    await this.dataSource.query(
      `INSERT INTO company_plan_subscriptions
       ...
       ON CONFLICT ("companyId") DO UPDATE SET
         ...
         "updatedAt" = now()`,
      [companyId, payerUserId, plan, productCode, currentPeriodEnd, JSON.stringify(metadata)],
    );
  ...
  private plan(value: unknown): CompanyPlan {
    const plan = String(value || '').toUpperCase();
    if (!['FREE', 'PLUS', 'ELITE'].includes(plan)) throw new BadRequestException('Plano inválido.');
    return plan as CompanyPlan;
  }

  private periodEnd(value: unknown, durationDays: number) {
    if (value !== null && value !== undefined && String(value).trim()) {
      const date = new Date(String(value));
      if (Number.isNaN(date.getTime())) throw new BadRequestException('Data de vencimento inválida.');
      const max = Date.now() + 5 * 365 * 24 * 60 * 60 * 1000;
      if (date.getTime() <= Date.now()) throw new BadRequestException('O vencimento do plano deve ficar no futuro.');
      if (date.getTime() > max) throw new BadRequestException('O vencimento não pode ultrapassar 5 anos.');
      return date;
    }
    return new Date(Date.now() + durationDays * 24 * 60 * 60 * 1000);
  }

  private async setHighlight(companyId: string, eligible: boolean, eligibleUntil: Date | null, source: string) {
    await this.dataSource.query(
      `INSERT INTO company_ad_highlight_eligibility
       ...
       ON CONFLICT ("companyId") DO UPDATE SET
         ...
         "updatedAt" = now()`,
      [companyId, eligible, eligibleUntil, source],
    ).catch(() => undefined);
  }
}
